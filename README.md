...
- **dwarf-cloudsc-gpu-kernels**: GPU-enabled version of the CLODUSC dwarf
...
- **dwarf-cloudsc-gpu-scc**: GPU-enabled and optimized version of
  CLOUDSC that utilisaes the native blcoked IFS memory layout via a
  "single-column coalesced" (SCC) loop layout. Here the outher NPROMA
  block loop is mapped to the OpenACC "gang" level and the kernel uses
  an inverted loop-nest where the outer horizontal loop os mapped to
  OpenACC " vector" parallelism.
