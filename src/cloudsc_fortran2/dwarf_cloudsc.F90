...
IF(.NOT.ALLOCATED(YRCST)) STOP 'YRCST not allocated'
IF(.NOT.ALLOCATED(YRTHF)) STOP 'YRTHF not allocated'
IF(.NOT.ALLOCATED(YRECLDP)) STOP 'YRECLDP not allocated'
! Call the driver to perform the parallel loop over our kernel
CALL CLOUDSC_DRIVER( NUMOMP, NPROMA, GLOBAL_STATE%KLEV, NGPTOT, NGPTOTG, NCLV, &
