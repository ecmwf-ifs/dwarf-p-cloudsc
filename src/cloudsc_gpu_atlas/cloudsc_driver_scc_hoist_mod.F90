...
MODULE CLOUDSC_DRIVER_SCC_HOIST_MOD
...
1003 format(5x,'NUMPROC=',i0,', NUMOMP=',i0,', NGTOT=', i0,', NGPTOTG=',i0,', NPROMA=',i0,', NGPBLKS=',i0)
    ...
    ALLOCATE( &
      ZFOEALFA(NPROMA, NLEV+1, NGPBLKS), &
      ZTP1(NPROMA, NLEV, NGPBLKS), &
      ZLI(NPROMA, NLEV, NGPBLKS), &
      ZA(NPROMA, NLEV, NGPBLKS), &
      ZAORIG(NPROMA, NLEV, NGPBLKS), &
      ZLIQFRAC(NPROMA, NLEV, NGPBLKS), &
      ZICEFRAC(NPROMA, NLEV, NGPBLKS), &
      ZQX(NPROMA, NLEV, NCLV, NGPBLKS), &
      ZQX0(NPROMA, NLEV, NCLV, NGPBLKS), &
      ZPFPLSX(NPROMA, NLEV+1, NCLV, NGPBLKS), &
      ZLNEG(NPROMA, NLEV, NCLV, NGPBLKS), &
      ZQXN2D(NPROMA, NLEV, NCLV, NGPBLKS), &
      ZQSMIX(NPROMA, NLEV, NGPBLKS), &
      ZQSLIQ(NPROMA, NLEV, NGPBLKS), &
      ZQSICE(NPROMA, NLEV, NGPBLKS), &
      ZFOEEWMT(NPROMA, NLEV, NGPBLKS), &
      ZFOEEW(NPROMA, NLEV, NGPBLKS), &
      ZFOEELIQT(NPROMA, NLEV, NGPBLKS) &
    )
...
!$acc &   ZFOEEW, ZFOEELIQT)
    ...
    CALL FSET%DEVICE_DATA(19, PLUDE)
    ...
    ! moved to the device in the ``acc data`` clause below
    ...
    DO JKGLO=1,NGPTOT,NPROMA
      ...
      DO JL=1,ICEND
        CALL CLOUDSC_SCC_HOIST &
         ...
         & PT(:,:,IBL), PQ(:,:,IBL), &
         & TENDENCY_TMP_T(:,:,IBL), TENDENCY_TMP_Q(:,:,IBL), TENDENCY_TMP_A(:,:,IBL), TENDENCY_TMP_CLD(:,:,4:8,IBL), &
         & TENDENCY_LOC_T(:,:,IBL), TENDENCY_LOC_Q(:,:,IBL), TENDENCY_LOC_A(:,:,IBL), TENDENCY_LOC_CLD(:,:,4:8,IBL), &
...
END MODULE CLOUDSC_DRIVER_SCC_HOIST_MOD
