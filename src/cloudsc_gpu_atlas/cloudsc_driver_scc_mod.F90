...
1003 format(5x,'NUMPROC=',i0,', NUMOMP=',i0,', NGTOT=', i0,', NGPTOTG=',i0,', NPROMA=',i0,', NGPBLKS=',i0)
    ...
    CALL TIMER_DEVICE_ALLOC%START(1)
    TID = GET_THREAD_NUM()
    CALL TIMER_DEVICE_ALLOC%THREAD_START(TID)
    CALL FSET%ALLOCATE_DEVICE()
    CALL TIMER_DEVICE_ALLOC%THREAD_END(TID)
    CALL TIMER_DEVICE_ALLOC%END()
    ...
    CALL CLOUDSC_KERNEL( NGPTOT, NLEV, NPROMA, PTSPHY, &
      ...
      CALL TIMER_DEVICE_ALLOC%PRINT_PERFORMANCE(NPROMA, NGPBLKS, NGPTOT)
