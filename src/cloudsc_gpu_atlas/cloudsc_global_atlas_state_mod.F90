...
CONTAINS
  ...
  SUBROUTINE CLOUDSC_GLOBAL_ATLAS_STATE_LOAD(SELF, FSET, FSPACE, NPROMA, NGPTOTG)
    ...
    CALL CONFIG%SET("host_memory_pinned", .true.);
...
!      CALL MULTIFIELD_IN%FINAL()
    ELSE
      DO IVAR = 1, SIZE(IN_VAR_NAMES) - 7
          CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME=TRIM(IN_VAR_NAMES(IVAR)),   KIND=ATLAS_REAL(JPRB), OPTIONS=CONFIG))
          FIELD = FSET%FIELD(TRIM(IN_VAR_NAMES(IVAR)))
      END DO
    ENDIF

    ! special input variables
    CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME="PLSM",   KIND=ATLAS_REAL(JPRB),    LEVELS=0, OPTIONS=CONFIG))
    CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME="LDCUM",  KIND=ATLAS_LOGICAL(),     LEVELS=0, OPTIONS=CONFIG))
    CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME="KTYPE",  KIND=ATLAS_INTEGER(JPIM), LEVELS=0, OPTIONS=CONFIG))
    CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME="PAPH",   KIND=ATLAS_REAL(JPRB),    LEVELS=SELF%KLEV+1, OPTIONS=CONFIG))
    !CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME="PEXTRA", KIND=ATLAS_REAL(JPRB),    VARIABLES=MAX(1,SELF%KFLDX, OPTIONS=CONFIG)))
    CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME="PCLV",   KIND=ATLAS_REAL(JPRB),    VARIABLES=MAX(1,NCLV), OPTIONS=CONFIG))
    CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME='TENDENCY_CML_CLD', KIND=ATLAS_REAL(JPRB), VARIABLES=NCLV, OPTIONS=CONFIG))
    CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME='TENDENCY_TMP_CLD', KIND=ATLAS_REAL(JPRB), VARIABLES=NCLV, OPTIONS=CONFIG))
    ...
    ELSE
        DO IVAR = 1, SIZE(OUT_VAR_NAMES) - 6
            CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME=TRIM(OUT_VAR_NAMES(IVAR)), KIND=ATLAS_REAL(JPRB), &
                & LEVELS=SELF%KLEV+1, OPTIONS=CONFIG))
        END DO
    ENDIF

    ! special output variables
    CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME="PCOVPTOT", KIND=ATLAS_REAL(JPRB), OPTIONS=CONFIG))
    CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME='TENDENCY_LOC_T', KIND=ATLAS_REAL(JPRB), OPTIONS=CONFIG))
    CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME='TENDENCY_LOC_A', KIND=ATLAS_REAL(JPRB), OPTIONS=CONFIG))
    CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME='TENDENCY_LOC_Q', KIND=ATLAS_REAL(JPRB), OPTIONS=CONFIG))
    CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME="PRAINFRAC_TOPRFZ", KIND=ATLAS_REAL(JPRB), LEVELS=0, OPTIONS=CONFIG))
    CALL FSET%ADD(FSPACE%CREATE_FIELD(NAME='TENDENCY_LOC_CLD', KIND=ATLAS_REAL(JPRB), VARIABLES=NCLV, OPTIONS=CONFIG))

    DO IVAR = 1, SIZE(IN_VAR_NAMES)-13
