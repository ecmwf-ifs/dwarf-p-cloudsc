...
__global__ void cloudsc_c(int kidia, int kfdia, int klon, float ptsphy,
  ...
  float zda;
  //float zli[klev], za[klev];
  //float zaorig[klev];  // start of scheme value for CC
  ...
  //float zliqfrac[klev];  // cloud liquid water fraction: ql/(ql+qi)
  //float zicefrac[klev];  // cloud ice water fraction: qi/(ql+qi)
  ...
  //float zqsmix[klev];  // diagnostic mixed phase saturation
  //REAL(KIND=JPRB) :: ZQSBIN(KLON,KLEV) ! binary switched ice/liq saturation
  //float zqsliq[klev];  // liquid water saturation
  //float zqsice[klev];  // ice water saturation
  ...
  float zsolqa[5 * 5];  // explicit sources and sinks
  ...
  //float zqx[5 * klev];
  //float zqx0[5 * klev];
  //float zpfplsx[5 * (klev + 1)];
  //float zlneg[5 * klev];
  //float zqxn2d[5 * klev];

  jl = threadIdx.x; 
  ibl = blockIdx.x; //blockIdx.z;
  ...
  zepsilon = (float) 100.*DBL_EPSILON;
  ...
  for (jk = 0; jk <= klev + -1; jk += 1) {
    ...
    tendency_loc_cld[jl + klon*(jk + klev*(4 + 5*ibl))] = (float) 0.0;
  }
  ...
  // ----------------------
  for (jk = 0; jk <= klev + -1; jk += 1) {
    ztp1[jl + klon*(jk + klev*ibl)] = 
      pt[jl + klon*(jk + klev*ibl)] + ptsphy*tendency_tmp_t[jl + klon*(jk + klev*ibl)];
    zqx[jl + klon*(jk + klev*(4 + 5*ibl))] = 
      pq[jl + klon*(jk + klev*ibl)] + ptsphy*tendency_tmp_q[jl + klon*(jk + klev*ibl)];
    zqx0[jl + klon*(jk + klev*(4 + 5*ibl))] = 
      pq[jl + klon*(jk + klev*ibl)] + ptsphy*tendency_tmp_q[jl + klon*(jk + klev*ibl)];
    za[jl + klon*(jk + klev*ibl)] = 
      pa[jl + klon*(jk + klev*ibl)] + ptsphy*tendency_tmp_a[jl + klon*(jk + klev*ibl)];
    zaorig[jl + klon*(jk + klev*ibl)] = 
      pa[jl + klon*(jk + klev*ibl)] + ptsphy*tendency_tmp_a[jl + klon*(jk + klev*ibl)];
  }
  ...
  for (jm = 0; jm <= 5 - 1 + -1; jm += 1) {
    for (jk = 0; jk <= klev + -1; jk += 1) {
      zqx[jl + klon*(jk + klev*(jm + 5*ibl))] = pclv[jl + klon*(jk + klev*(jm + 5*ibl))] + 
        ptsphy*tendency_tmp_cld[jl + klon*(jk + klev*(jm + 5*ibl))];
      zqx0[jl + klon*(jk + klev*(jm + 5*ibl))] = pclv[jl + klon*(jk + klev*(jm + 5*ibl))] + 
	ptsphy*tendency_tmp_cld[jl + klon*(jk + klev*(jm + 5*ibl))];
    }
  }
  ...
  for (jm = 0; jm <= 5 + -1; jm += 1) {
    for (jk = 0; jk <= klev + 1 + -1; jk += 1) {
      zpfplsx[jl + klon*(jk + (klev + 1)*(jm + 5*ibl))] = (float) 0.0;          // precip fluxes
    }
  }
  
  for (jm = 0; jm <= 5 + -1; jm += 1) {
    for (jk = 0; jk <= klev + -1; jk += 1) {
      zqxn2d[jl + klon*(jk + klev*(jm + 5*ibl))] = (float) 0.0;          // end of timestep values in 2D
      zlneg[jl + klon*(jk + klev*(jm + 5*ibl))] = (float) 0.0;          // negative input check
    }
  }
  ...
  // ----------------------------------------------------
  for (jk = 0; jk <= klev + -1; jk += 1) {
    if (zqx[jl + klon*(jk + klev*(0 + 5*ibl))] + 
        zqx[jl + klon*(jk + klev*(1 + 5*ibl))] < (*yrecldp).rlmin || 
	za[jl + klon*(jk + klev*ibl)] < (*yrecldp).ramin) {
        
      // Evaporate small cloud liquid water amounts
      zlneg[jl + klon*(jk + klev*(0 + 5*ibl))] = zlneg[jl + klon*(jk + klev*(0 + 5*ibl))] + 
	    zqx[jl + klon*(jk + klev*(0 + 5*ibl))];
      zqadj = zqx[jl + klon*(jk + klev*(0 + 5*ibl))]*zqtmst;
      ...
      zqx[jl + klon*(jk + klev*(4 + 5*ibl))] = zqx[jl + klon*(jk + klev*(4 + 5*ibl))] + 
	zqx[jl + klon*(jk + klev*(0 + 5*ibl))];
      zqx[jl + klon*(jk + klev*(0 + 5*ibl))] = (float) 0.0;
        
      // Evaporate small cloud ice water amounts
      zlneg[jl + klon*(jk + klev*(1 + 5*ibl))] = zlneg[jl + klon*(jk + klev*(1 + 5*ibl))] + 
        zqx[jl + klon*(jk + klev*(1 + 5*ibl))];
      zqadj = zqx[jl + klon*(jk + klev*(1 + 5*ibl))]*zqtmst;
      tendency_loc_q[jl + klon*(jk + klev*ibl)] = 
        tendency_loc_q[jl + klon*(jk + klev*ibl)] + zqadj;
      tendency_loc_t[jl + klon*(jk + klev*ibl)] = 
      ...
      zqx[jl + klon*(jk + klev*(4 + 5*ibl))] = zqx[jl + klon*(jk + klev*(4 + 5*ibl))] + 
        zqx[jl + klon*(jk + klev*(1 + 5*ibl))];
      zqx[jl + klon*(jk + klev*(1 + 5*ibl))] = (float) 0.0;
        
      // Set cloud cover to zero
      za[jl + klon*(jk + klev*ibl)] = (float) 0.0;
  ...
  for (jm = 0; jm <= 5 - 1 + -1; jm += 1) {
    for (jk = 0; jk <= klev + -1; jk += 1) {
      if (zqx[jl + klon*(jk + klev*(jm + 5*ibl))] < (*yrecldp).rlmin) {
        zlneg[jl + klon*(jk + klev*(jm + 5*ibl))] = zlneg[jl + klon*(jk + klev*(jm + 5*ibl))] + 
          zqx[jl + klon*(jk + klev*(jm + 5*ibl))];
        zqadj = zqx[jl + klon*(jk + klev*(jm + 5*ibl))]*zqtmst;
        tendency_loc_q[jl + klon*(jk + klev*ibl)] = 
          tendency_loc_q[jl + klon*(jk + klev*ibl)] + zqadj;
        if (iphase[jm] == 1) {
          tendency_loc_t[jl + klon*(jk + klev*ibl)] = 
        ...
        zqx[jl + klon*(jk + klev*(4 + 5*ibl))] = zqx[jl + klon*(jk + klev*(4 + 5*ibl))] + 
          zqx[jl + klon*(jk + klev*(jm + 5*ibl))];
        zqx[jl + klon*(jk + klev*(jm + 5*ibl))] = (float) 0.0;
      }
    }
  }
  ...
  for (jk = 0; jk <= klev + -1; jk += 1) {
    ...
    //----------------------------------------
    zfoealfa[jl + klon*(jk + (klev + 1)*ibl)] = 
      ((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2))));
    zfoeewmt[jl + klon*(jk + klev*ibl)] = 
        fmin(((float)(r2es*((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2)))*
	exp((r3les*(ztp1[jl + klon*(jk + klev*ibl)] - rtt))/(ztp1[jl + klon*(jk + klev*ibl)] - r4les)) + 
	((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2))))*
	exp((r3ies*(ztp1[jl + klon*(jk + klev*ibl)] - rtt))/(ztp1[jl + klon*(jk + klev*ibl)] - r4ies))))) / 
	pap[jl + klon*(jk + klev*ibl)], (float) 0.5);
    zqsmix[jl + klon*(jk + klev*ibl)] = zfoeewmt[jl + klon*(jk + klev*ibl)];
    zqsmix[jl + klon*(jk + klev*ibl)] = zqsmix[jl + klon*(jk + klev*ibl)] / 
      ((float) 1.0 - retv*zqsmix[jl + klon*(jk + klev*ibl)]);
    
    //---------------------------------------------
    // ice saturation T<273K
    // liquid water saturation for T>273K
    //---------------------------------------------
    zalfa = ((float)(fmax((float)0.0, copysign((float)1.0, ztp1[jl + klon*(jk + klev*ibl)] - rtt))));
    zfoeew[jl + klon*(jk + klev*ibl)] = fmin((zalfa*((float)(r2es*exp((r3les*(ztp1[jl + klon*(jk + klev*ibl)] - rtt))/
      (ztp1[jl + klon*(jk + klev*ibl)] - r4les)))) + ((float) 1.0 - zalfa)*
      ((float)(r2es*exp((r3ies*(ztp1[jl + klon*(jk + klev*ibl)] - rtt))/(ztp1[jl + klon*(jk + klev*ibl)] - r4ies)))))/
      pap[jl + klon*(jk + klev*ibl)], (float) 0.5);
    zfoeew[jl + klon*(jk + klev*ibl)] = fmin((float) 0.5, zfoeew[jl + klon*(jk + klev*ibl)]);
    zqsice[jl + klon*(jk + klev*ibl)] = zfoeew[jl + klon*(jk + klev*ibl)] / 
      ((float) 1.0 - retv*zfoeew[jl + klon*(jk + klev*ibl)]);
    
    //----------------------------------
    // liquid water saturation
    //----------------------------------
    zfoeeliqt[jl + klon*(jk + klev*ibl)] = fmin(((float)(r2es*exp((r3les*(ztp1[jl + klon*(jk + klev*ibl)] - rtt))/
      (ztp1[jl + klon*(jk + klev*ibl)] - r4les)))) / pap[jl + klon*(jk + klev*ibl)], (float) 0.5);
    zqsliq[jl + klon*(jk + klev*ibl)] = zfoeeliqt[jl + klon*(jk + klev*ibl)];
    zqsliq[jl + klon*(jk + klev*ibl)] = zqsliq[jl + klon*(jk + klev*ibl)] / 
      ((float) 1.0 - retv*zqsliq[jl + klon*(jk + klev*ibl)]);
  ...
  for (jk = 0; jk <= klev + -1; jk += 1) {
    ...
    //------------------------------------------
    za[jl + klon*(jk + klev*ibl)] = 
      fmax((float) 0.0, fmin((float) 1.0, za[jl + klon*(jk + klev*ibl)]));
    
    //-------------------------------------------------------------------
    // Calculate liq/ice fractions (no longer a diagnostic relationship)
    //-------------------------------------------------------------------
    zli[jl + klon*(jk + klev*ibl)] = zqx[jl + klon*(jk + klev*(0 + 5*ibl))] + 
      zqx[jl + klon*(jk + klev*(1 + 5*ibl))];
    if (zli[jl + klon*(jk + klev*ibl)] > (*yrecldp).rlmin) {
      zliqfrac[jl + klon*(jk + klev*ibl)] = 
        zqx[jl + klon*(jk + klev*(0 + 5*ibl))] / zli[jl + klon*(jk + klev*ibl)];
      zicefrac[jl + klon*(jk + klev*ibl)] = 
        (float) 1.0 - zliqfrac[jl + klon*(jk + klev*ibl)];
    } else {
      zliqfrac[jl + klon*(jk + klev*ibl)] = (float) 0.0;
      zicefrac[jl + klon*(jk + klev*ibl)] = (float) 0.0;
    }   
  }
  ...
  //---------------------------------
  ztrpaus = (float) 0.1;
  zpaphd = (float) 1.0 / paph[jl + klon*(klev + (klev + 1)*ibl)];
  for (jk = 0; jk <= klev - 1 + -1; jk += 1) {
    zsig = pap[jl + klon*(jk + klev*ibl)]*zpaphd;
    if (zsig > (float) 0.1 && zsig < (float) 0.4 && ztp1[jl + klon*(jk + klev*ibl)] >
      ztp1[jl + klon*(1 + jk + klev*ibl)]) {
      ztrpaus = zsig;
    }
  }
  ...
  for (jk = -1 + (*yrecldp).ncldtop; jk <= klev + -1; jk += 1) {
    ...
    //---------------------------------
    for (jm = 0; jm <= 5 + -1; jm += 1) {
      zqxfg[jm] = zqx[jl + klon*(jk + klev*(jm + 5*ibl))];
    }
    ...
    zdp = paph[jl + klon*(1 + jk + (klev + 1)*ibl)] - 
      paph[jl + klon*(jk + (klev + 1)*ibl)]; // dp
    zgdp = rg / zdp; // g/dp
    zrho = pap[jl + klon*(jk + klev*ibl)] / (rd*ztp1[jl + klon*(jk + klev*ibl)]); // p/RT air density
    
    zdtgdp = ptsphy*zgdp; // dt g/dp
    zrdtgdp = zdp*((float) 1.0 / (ptsphy*rg)); // 1/(dt g/dp)
    
    if (jk + 1 > 1) {
      zdtgdpf = ptsphy*rg / (pap[jl + klon*(jk + klev*ibl)] - 
        pap[jl + klon*(-1 + jk + klev*ibl)]);
    }
    ...
    // liquid
    zfacw = r5les / (pow((ztp1[jl + klon*(jk + klev*ibl)] - r4les), (float)2));
    zcor = (float) 1.0 / ((float) 1.0 - retv*zfoeeliqt[jl + klon*(jk + klev*ibl)]);
    zdqsliqdt = zfacw*zcor*zqsliq[jl + klon*(jk + klev*ibl)];
    zcorqsliq = (float) 1.0 + ralvdcp*zdqsliqdt;
    
    // ice
    zfaci = r5ies / (pow((ztp1[jl + klon*(jk + klev*ibl)] - r4ies), (float)2));
    zcor = (float) 1.0 / ((float) 1.0 - retv*zfoeew[jl + klon*(jk + klev*ibl)]);
    zdqsicedt = zfaci*zcor*zqsice[jl + klon*(jk + klev*ibl)];
    zcorqsice = (float) 1.0 + ralsdcp*zdqsicedt;
    
    // diagnostic mixed
    zalfaw = zfoealfa[jl + klon*(jk + (klev + 1)*ibl)];
    zalfawm = zalfaw;
    zfac = zalfaw*zfacw + ((float) 1.0 - zalfaw)*zfaci;
    zcor = (float) 1.0 / ((float) 1.0 - retv*zfoeewmt[jl + klon*(jk + klev*ibl)]);
    zdqsmixdt = zfac*zcor*zqsmix[jl + klon*(jk + klev*ibl)];
    zcorqsmix = (float) 1.0 + ((float)((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*
      rtwat_rtice_r, (float)2)))*ralvdcp + ((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*
      rtwat_rtice_r, (float)2))))*ralsdcp))*zdqsmixdt;
    
    // evaporation/sublimation limits
    zevaplimmix = fmax((zqsmix[jl + klon*(jk + klev*ibl)] - 
      zqx[jl + klon*(jk + klev*(4 + 5*ibl))]) / zcorqsmix, (float) 0.0);
    zevaplimliq = fmax((zqsliq[jl + klon*(jk + klev*ibl)] - 
      zqx[jl + klon*(jk + klev*(4 + 5*ibl))]) / zcorqsliq, (float) 0.0);
    zevaplimice = fmax((zqsice[jl + klon*(jk + klev*ibl)] - 
      zqx[jl + klon*(jk + klev*(4 + 5*ibl))]) / zcorqsice, (float) 0.0);
    
    //--------------------------------
    // in-cloud consensate amount
    //--------------------------------
    ztmpa = (float) 1.0 / fmax(za[jl + klon*(jk + klev*ibl)], zepsec);
    zliqcld = zqx[jl + klon*(jk + klev*(0 + 5*ibl))]*ztmpa;
    zicecld = zqx[jl + klon*(jk + klev*(1 + 5*ibl))]*ztmpa;
    ...
    if (zqx[jl + klon*(jk + klev*(0 + 5*ibl))] < (*yrecldp).rlmin) {
      zsolqa[4 + 5*(0)] = zqx[jl + klon*(jk + klev*(0 + 5*ibl))];
      zsolqa[0 + 5*(4)] = -zqx[jl + klon*(jk + klev*(0 + 5*ibl))];
    }
    
    if (zqx[jl + klon*(jk + klev*(1 + 5*ibl))] < (*yrecldp).rlmin) {
      zsolqa[4 + 5*(1)] = zqx[jl + klon*(jk + klev*(1 + 5*ibl))];
      zsolqa[1 + 5*(4)] = -zqx[jl + klon*(jk + klev*(1 + 5*ibl))];
    }
    ...
    zfokoop = ((float)(fmin(rkoop1 - rkoop2*ztp1[jl + klon*(jk + klev*ibl)], (float)(r2es*exp((r3les*(ztp1[jl + klon*(jk + klev*ibl)] - rtt))/
      (ztp1[jl + klon*(jk + klev*ibl)] - r4les)))*(float)1.0/(float)(r2es*exp((r3ies*(ztp1[jl + klon*(jk + klev*ibl)] - rtt))/
      (ztp1[jl + klon*(jk + klev*ibl)] - r4ies))))));
    
    if (ztp1[jl + klon*(jk + klev*ibl)] >= rtt || (*yrecldp).nssopt == 0) {
      zfac = (float) 1.0;
      zfaci = (float) 1.0;
    } else {
      zfac = za[jl + klon*(jk + klev*ibl)] + zfokoop*((float) 1.0 - za[jl + klon*(jk +klev*ibl)]);
    ...
    if (za[jl + klon*(jk + klev*ibl)] > (float) 1.0 - (*yrecldp).ramin) {
      zsupsat = fmax((zqx[jl + klon*(jk + klev*(4 + 5*ibl))] - zfac*zqsice[jl + klon*(jk + klev*ibl)]) / 
        zcorqsice, (float) 0.0);
    } else {
      // Calculate environmental humidity supersaturation
      zqp1env = (zqx[jl + klon*(jk + klev*(4 + 5*ibl))] - 
        za[jl + klon*(jk + klev*ibl)]*zqsice[jl + klon*(jk + klev*ibl)]) / 
        fmax((float) 1.0 - za[jl + klon*(jk + klev*ibl)], zepsilon);
      //& SIGN(MAX(ABS(1.0_JPRB-ZA(JL,JK)),ZEPSILON),1.0_JPRB-ZA(JL,JK))
      zsupsat = fmax(((float) 1.0 - za[jl + klon*(jk + klev*ibl)])*(zqp1env - zfac*zqsice[jl + klon*(jk + klev*ibl)]) / 
        zcorqsice, (float) 0.0);
    }
    ...
    if (zsupsat > zepsec) {
      ...
      if (ztp1[jl + klon*(jk + klev*ibl)] > (*yrecldp).rthomo) {
      ...
      zsolac = ((float) 1.0 - za[jl + klon*(jk + klev*ibl)])*zfaci;
    ...
    if (psupsat[jl + klon*(jk + klev*ibl)] > zepsec) {
      if (ztp1[jl + klon*(jk + klev*ibl)] > (*yrecldp).rthomo) {
        // Turn supersaturation into liquid water
        zsolqa[0 + 5*(0)] = zsolqa[0 + 5*(0)] + psupsat[jl + klon*(jk + klev*ibl)];
        ...
        // Turn supersaturation into ice water
        zsolqa[1 + 5*(1)] = zsolqa[1 + 5*(1)] + psupsat[jl + klon*(jk + klev*ibl)];
    ...
    if (jk + 1 < klev && jk + 1 >= (*yrecldp).ncldtop) {
      ...
      if (/*ldcum[jl + klon*ibl] &&*/ plude[jl + klon*(jk + klev*ibl)] > (*yrecldp).rlmin 
        && plu[jl + klon*(1 + jk + klev*ibl)] > zepsec) {
        
        zsolac = zsolac + plude[jl + klon*(jk + klev*ibl)] / 
          plu[jl + klon*(1 + jk + klev*ibl)];
        // *diagnostic temperature split*
        zalfaw = zfoealfa[jl + klon*(jk + (klev + 1)*ibl)];
        zconvsrce[0] = zalfaw*plude[jl + klon*(jk + klev*ibl)];
        zconvsrce[1] = ((float) 1.0 - zalfaw)*plude[jl + klon*(jk + klev*ibl)];
      ...
      //if (ldcum[jl + klon*ibl]) {
        zsolqa[3 + 5*(3)] = zsolqa[3 + 5*(3)] + psnde[jl + klon*(jk + klev*ibl)]*zdtgdp;
      //}
    ...
    if (jk + 1 > (*yrecldp).ncldtop) {
      ...
      zmf = fmax((float) 0.0, (pmfu[jl + klon*(jk + klev*ibl)] + pmfd[jl + klon*(jk + klev*ibl)])*zdtgdp);
      zacust = zmf*zanewm1;
      ...
      zdtdp = zrdcp*(float) 0.5*(ztp1[jl + klon*(-1 + jk + klev*ibl)] + 
        ztp1[jl + klon*(jk + klev*ibl)]) / paph[jl + klon*(jk + (klev + 1)*ibl)];
      zdtforc = zdtdp*(pap[jl + klon*(jk + klev*ibl)] - pap[jl + klon*(-1 + jk + klev*ibl)]);
      //[#Note: Diagnostic mixed phase should be replaced below]
      zdqs = zanewm1*zdtforc*zdqsmixdt;
      
      for (jm = 0; jm <= 5 + -1; jm += 1) {
        if (!llfall[jm] && iphase[jm] > 0) {
          zlfinal = fmax((float) 0.0, zlcust[jm] - zdqs); //lim to zero
    ...
    if (jk + 1 < klev) {
      ...
      zmfdn = fmax((float) 0.0, (pmfu[jl + klon*(1 + jk + klev*ibl)] + 
    ...
    if (zli[jl + klon*(jk + klev*ibl)] > zepsec) {
      ...
      ze = zldifdt*fmax(zqsmix[jl + klon*(jk + klev*ibl)] - 
        zqx[jl + klon*(jk + klev*(4 + 5*ibl))], (float) 0.0);
      zleros = za[jl + klon*(jk + klev*ibl)]*ze;
      zleros = fmin(zleros, zevaplimmix);
      zleros = fmin(zleros, zli[jl + klon*(jk + klev*ibl)]);
      zaeros = zleros / zlicld;          //if linear term
      
      // Erosion is -ve LINEAR in L,A
      zsolac = zsolac - zaeros;          //linear
      
      zsolqa[4 + 5*(0)] = zsolqa[4 + 5*(0)] + zliqfrac[jl + klon*(jk + klev*ibl)]*zleros;
      zsolqa[0 + 5*(4)] = zsolqa[0 + 5*(4)] - zliqfrac[jl + klon*(jk + klev*ibl)]*zleros;
      zsolqa[4 + 5*(1)] = zsolqa[4 + 5*(1)] + zicefrac[jl + klon*(jk + klev*ibl)]*zleros;
      zsolqa[1 + 5*(4)] = zsolqa[1 + 5*(4)] - zicefrac[jl + klon*(jk + klev*ibl)]*zleros;
    ...
    zdtdp = zrdcp*ztp1[jl + klon*(jk + klev*ibl)] / pap[jl + klon*(jk + klev*ibl)];
    zdpmxdt = zdp*zqtmst;
    zmfdn = (float) 0.0;
    if (jk + 1 < klev) {
      zmfdn = pmfu[jl + klon*(1 + jk + klev*ibl)] + pmfd[jl + klon*(1 + jk + klev*ibl)];
    }
    zwtot = pvervel[jl + klon*(jk + klev*ibl)] + 
      (float) 0.5*rg*(pmfu[jl + klon*(jk + klev*ibl)] + pmfd[jl + klon*(jk + klev*ibl)] + zmfdn);
    ...
    zqold = zqsmix[jl + klon*(jk + klev*ibl)];
    ztold = ztp1[jl + klon*(jk + klev*ibl)];
    ztp1[jl + klon*(jk + klev*ibl)] = ztp1[jl + klon*(jk + klev*ibl)] + zdtforc;
    ztp1[jl + klon*(jk + klev*ibl)] = fmax(ztp1[jl + klon*(jk + klev*ibl)], (float) 160.0);
    llflag = true;
    
    // Formerly a call to CUADJTQ(..., ICALL=5)
    zqp = (float) 1.0 / pap[jl + klon*(jk + klev*ibl)];
    zqsat = ((float)(r2es*((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2)))*
      exp((r3les*(ztp1[jl + klon*(jk + klev*ibl)] - rtt))/(ztp1[jl + klon*(jk + klev*ibl)] - r4les)) + 
      ((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2))))*
      exp((r3ies*(ztp1[jl + klon*(jk + klev*ibl)] - rtt))/(ztp1[jl + klon*(jk + klev*ibl)] - r4ies)))))*zqp;
    zqsat = fmin((float) 0.5, zqsat);
    zcor = (float) 1.0 / ((float) 1.0 - retv*zqsat);
    zqsat = zqsat*zcor;
    zcond = (zqsmix[jl + klon*(jk + klev*ibl)] - zqsat) / 
      ((float) 1.0 + zqsat*zcor*((float)(((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - 
      rtice)*rtwat_rtice_r, (float)2)))*r5alvcp)*((float)1.0/pow(ztp1[jl + klon*(jk + klev*ibl)] - r4les, (float)2)) + 
      (((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2))))*r5alscp)*
      ((float)1.0/pow(ztp1[jl + klon*(jk + klev*ibl)] - r4ies, (float)2)))));
    ztp1[jl + klon*(jk + klev*ibl)] = ztp1[jl + klon*(jk + klev*ibl)] + 
      ((float)((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2)))*ralvdcp + 
      ((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2))))*ralsdcp))*zcond;
    zqsmix[jl + klon*(jk + klev*ibl)] = zqsmix[jl + klon*(jk + klev*ibl)] - zcond;
    zqsat = ((float)(r2es*((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2)))*
      exp((r3les*(ztp1[jl + klon*(jk + klev*ibl)] - rtt))/(ztp1[jl + klon*(jk + klev*ibl)] - r4les)) + 
      ((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2))))*
      exp((r3ies*(ztp1[jl + klon*(jk + klev*ibl)] - rtt))/(ztp1[jl + klon*(jk + klev*ibl)] - r4ies)))))*zqp;
    ...
    zcor = (float) 1.0 / ((float) 1.0 - retv*zqsat);
    zqsat = zqsat*zcor;
    zcond1 = (zqsmix[jl + klon*(jk + klev*ibl)] - zqsat) / 
      ((float) 1.0 + zqsat*zcor*((float)(((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - 
      rtice)*rtwat_rtice_r, (float)2)))*r5alvcp)*((float)1.0/pow(ztp1[jl + klon*(jk + klev*ibl)] - r4les, (float)2)) + 
      (((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2))))*r5alscp)*
      ((float)1.0/pow(ztp1[jl + klon*(jk + klev*ibl)] - r4ies, (float)2)))));
    ztp1[jl + klon*(jk + klev*ibl)] = ztp1[jl + klon*(jk + klev*ibl)] + 
      ((float)((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2)))*ralvdcp + 
      ((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2))))*ralsdcp))*zcond1;
    zqsmix[jl + klon*(jk + klev*ibl)] = zqsmix[jl + klon*(jk + klev*ibl)] - zcond1;
    
    zdqs = zqsmix[jl + klon*(jk + klev*ibl)] - zqold;
    zqsmix[jl + klon*(jk + klev*ibl)] = zqold;
    ztp1[jl + klon*(jk + klev*ibl)] = ztold;
    ...
    if (zdqs > (float) 0.0) {
      ...
      zlevap = za[jl + klon*(jk + klev*ibl)]*fmin(zdqs, zlicld);
      zlevap = fmin(zlevap, zevaplimmix);
      zlevap = fmin(zlevap, fmax(zqsmix[jl + klon*(jk + klev*ibl)] - 
        zqx[jl + klon*(jk + klev*(4 + 5*ibl))], (float) 0.0));
      
      // For first guess call
      zlevapl = zliqfrac[jl + klon*(jk + klev*ibl)]*zlevap;
      zlevapi = zicefrac[jl + klon*(jk + klev*ibl)]*zlevap;
      
      zsolqa[4 + 5*(0)] = zsolqa[4 + 5*(0)] + zliqfrac[jl + klon*(jk + klev*ibl)]*zlevap;
      zsolqa[0 + 5*(4)] = zsolqa[0 + 5*(4)] - zliqfrac[jl + klon*(jk + klev*ibl)]*zlevap;
      
      zsolqa[4 + 5*(1)] = zsolqa[4 + 5*(1)] + zicefrac[jl + klon*(jk + klev*ibl)]*zlevap;
      zsolqa[1 + 5*(4)] = zsolqa[1 + 5*(4)] - zicefrac[jl + klon*(jk + klev*ibl)]*zlevap;
    ...
    if (za[jl + klon*(jk + klev*ibl)] > zepsec && zdqs <= -(*yrecldp).rlmin) {
      
      zlcond1 = fmax(-zdqs, (float) 0.0);          //new limiter
      
      //old limiter (significantly improves upper tropospheric humidity rms)
      if (za[jl + klon*(jk + klev*ibl)] > (float) 0.99) {
        zcor = (float) 1.0 / ((float) 1.0 - retv*zqsmix[jl + klon*(jk + klev*ibl)]);
        zcdmax = (zqx[jl + klon*(jk + klev*(4 + 5*ibl))] - zqsmix[jl + klon*(jk + klev*ibl)]) / 
          ((float) 1.0 + zcor*zqsmix[jl + klon*(jk + klev*ibl)]*((float)(((float)
          (fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2)))*r5alvcp)*
          ((float)1.0/pow(ztp1[jl + klon*(jk + klev*ibl)] - r4les, (float)2)) + 
          (((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jl + klon*(jk + klev*ibl)])) - rtice)*rtwat_rtice_r, (float)2))))
          *r5alscp)*(1.0/pow(ztp1[jl + klon*(jk + klev*ibl)] - r4ies, (float)2)))));
      } else {
        zcdmax = (zqx[jl + klon*(jk + klev*(4 + 5*ibl))] - za[jl + klon*(jk + klev*ibl)]*zqsmix[jl + klon*(jk + klev*ibl)]) / 
          za[jl + klon*(jk + klev*ibl)];
      }
      zlcond1 = fmax(fmin(zlcond1, zcdmax), (float) 0.0);
      // end old limiter
      
      zlcond1 = za[jl + klon*(jk + klev*ibl)]*zlcond1;
    ...
    if (zdqs <= -(*yrecldp).rlmin && za[jl + klon*(jk + klev*ibl)] < (float) 1.0 - zepsec) {
      
      //---------------------------
      // Critical relative humidity
      //---------------------------
      zrhc = (*yrecldp).ramid;
      zsigk = pap[jl + klon*(jk + klev*ibl)] / paph[jl + klon*(klev + (klev + 1)*ibl)];
      // Increase RHcrit to 1.0 towards the surface (eta>0.8)
      if (zsigk > (float) 0.8) {
        zrhc = (*yrecldp).ramid + ((float) 1.0 - (*yrecldp).ramid)*(pow(((zsigk - (float) 0.8) / 
          (float) 0.2), (float)2));
      }
      ...
      if ((*yrecldp).nssopt == 0) {
        // No scheme
        zqe = (zqx[jl + klon*(jk + klev*(4 + 5*ibl))] - za[jl + klon*(jk + klev*ibl)]*
          zqsice[jl + klon*(jk + klev*ibl)]) / fmax(zepsec, (float) 1.0 - za[jl + klon*(jk + klev*ibl)]);
        zqe = fmax((float) 0.0, zqe);
      } else if ((*yrecldp).nssopt == 1) {
        // Tompkins
        zqe = (zqx[jl + klon*(jk + klev*(4 + 5*ibl))] - za[jl + klon*(jk + klev*ibl)]*
          zqsice[jl + klon*(jk + klev*ibl)]) / fmax(zepsec, (float) 1.0 - 
        za[jl + klon*(jk + klev*ibl)]);
        zqe = fmax((float) 0.0, zqe);
      } else if ((*yrecldp).nssopt == 2) {
        // Lohmann and Karcher
        zqe = zqx[jl + klon*(jk + klev*(4 + 5*ibl))];
      } else if ((*yrecldp).nssopt == 3) {
        // Gierens
        zqe = 
          zqx[jl + klon*(jk + klev*(4 + 5*ibl))] + zli[jl + klon*(jk + klev*ibl)];
      }
      ...
      if (zqe >= zrhc*zqsice[jl + klon*(jk + klev*ibl)]*zfac && 
        zqe < zqsice[jl + klon*(jk + klev*ibl)]*zfac) {
        // note: not **2 on 1-a term if ZQE is used.
        // Added correction term ZFAC to numerator 15/03/2010
        zacond = -((float) 1.0 - za[jl + klon*(jk + klev*ibl)])*zfac*zdqs / 
          fmax((float) 2.0*(zfac*zqsice[jl + klon*(jk + klev*ibl)] - zqe), zepsec);
        
        zacond = fmin(zacond, (float) 1.0 - za[jl + klon*(jk + klev*ibl)]); //PUT THE LIMITER BACK
        ...
        zzdl = (float) 2.0*(zfac*zqsice[jl + klon*(jk + klev*ibl)] - zqe) / 
          fmax(zepsec, (float) 1.0 - za[jl + klon*(jk + klev*ibl)]);
        // Added correction term ZFAC 15/03/2010
        if (zfac*zdqs < -zzdl) {
          // ZLCONDLIM=(ZA(JL,JK)-1.0_JPRB)*ZDQS(JL)-ZQSICE(JL,JK)+ZQX(JL,JK,NCLDQV)
          zlcondlim = (za[jl + klon*(jk + klev*ibl)] - (float) 1.0)*zfac*zdqs - zfac*
            zqsice[jl + klon*(jk + klev*ibl)] + zqx[jl + klon*(jk + klev*(4 + 5*ibl))];
          zlcond2 = fmin(zlcond2, zlcondlim);
        }
        zlcond2 = fmax(zlcond2, (float) 0.0);
        
        if (zlcond2 < (*yrecldp).rlmin || ((float) 1.0 - 
          za[jl + klon*(jk + klev*ibl)]) < zepsec) {
          zlcond2 = (float) 0.0;
          zacond = (float) 0.0;
        }
    ...
    if (idepice == 1) {
      ...
      if (za[jl + klon*(-1 + jk + klev*ibl)] < (*yrecldp).rcldtopcf && 
        za[jl + klon*(jk + klev*ibl)] >= (*yrecldp).rcldtopcf) {
        zcldtopdist = (float) 0.0;
      } else {
      ...
      //--------------------------------------------------------------
      if (ztp1[jl + klon*(jk + klev*ibl)] < rtt && zqxfg[0] > (*yrecldp).rlmin) {
        // T<273K
        
        zvpice = ((float)(r2es*exp((r3ies*(ztp1[jl + klon*(jk + klev*ibl)] - rtt))/
          (ztp1[jl + klon*(jk + klev*ibl)] - r4ies))))*rv / rd;
        zvpliq = zvpice*zfokoop;
        zicenuclei = (float) 1000.0*exp((float) 12.96*(zvpliq - zvpice) / zvpliq - (float) 0.639);
        
        //------------------------------------------------
        //   2.4e-2 is conductivity of air
        //   8.8 = 700**1/3 = density of ice to the third
        //------------------------------------------------
        zadd = rlstt*(rlstt / (rv*ztp1[jl + klon*(jk + klev*ibl)]) - (float) 1.0) / 
          ((float) 2.4E-2*ztp1[jl + klon*(jk + klev*ibl)]);
        zbdd = rv*ztp1[jl + klon*(jk + klev*ibl)]*pap[jl + klon*(jk + klev*ibl)] / 
          ((float) 2.21*zvpice);
        zcvds = (float) 7.8*(pow((zicenuclei / zrho), (float) 0.666))*(zvpliq - 
        ...
        // new value of ice:
        //------------------
        zinew = pow(((float) 0.666*zcvds*ptsphy + (pow(zice0, (float) 0.666))), (float) 1.5);
        
        //---------------------------
        // grid-mean deposition rate:
        //---------------------------
        zdepos = fmax(za[jl + klon*(jk + klev*ibl)]*(zinew - zice0), (float) 0.0);
        ...
        zinfactor = fmin(zicenuclei / (float) 15000., (float) 1.0);
        zdepos = zdepos*fmin(zinfactor + ((float) 1.0 - zinfactor)*((*yrecldp).rdepliqrefrate + 
          zcldtopdist / (*yrecldp).rdepliqrefdepth), (float) 1.0);
      ...
      if (ztp1[jl + klon*(jk + klev*ibl)] < rtt && zqxfg[0] > (*yrecldp).rlmin) {
        ...
        zaplusb = (*yrecldp).rcl_apb1*zvpice - (*yrecldp).rcl_apb2*zvpice*
          ztp1[jl + klon*(jk + klev*ibl)] + pap[jl + klon*(jk + klev*ibl)]*
          (*yrecldp).rcl_apb3*(pow(ztp1[jl + klon*(jk + klev*ibl)], (float) 3.));
        zcorrfac = pow(((float) 1.0 / zrho), (float) 0.5);
        zcorrfac2 = (pow((ztp1[jl + klon*(jk + klev*ibl)] / (float) 273.0), (float)1.5))*
          ((float) 393.0 / (ztp1[jl + klon*(jk + klev*ibl)] + (float) 120.0));
        
        zpr02 = zrho*zice0*(*yrecldp).rcl_const1i / (ztcg*zfacx1i);
        
        zterm1 = (zvpliq - zvpice)*(pow(ztp1[jl + klon*(jk + klev*ibl)], (float) 2.0))*
          zvpice*zcorrfac2*ztcg*(*yrecldp).rcl_const2i*zfacx1i / (zrho*zaplusb*zvpice);
        zterm2 = (float) 0.65*(*yrecldp).rcl_const6i*(pow(zpr02, (*yrecldp).rcl_const4i)) + 
          (*yrecldp).rcl_const3i*(pow(zcorrfac, (float) 0.5))*(pow(zrho, (float) 0.5))*
          (pow(zpr02, (*yrecldp).rcl_const5i)) / (pow(zcorrfac2, (float) 0.5));
        
        zdepos = fmax(za[jl + klon*(jk + klev*ibl)]*zterm1*zterm2*ptsphy, (float) 0.0);
        ...
        // to increase deposition rate with decreasing temperatures
        zinfactor = fmin(zicenuclei / (float) 15000., (float) 1.0);
        zdepos = zdepos*fmin(zinfactor + ((float) 1.0 - zinfactor)*
          ((*yrecldp).rdepliqrefrate + zcldtopdist / (*yrecldp).rdepliqrefdepth), (float) 1.0);
    ...
    for (jm = 0; jm <= 5 + -1; jm += 1) {
      if (llfall[jm] || jm + 1 == 2) {
        ...
        //------------------------
        if (jk + 1 > (*yrecldp).ncldtop) {
          zfallsrce[jm] = zpfplsx[jl + klon*(jk + (klev + 1)*(jm + 5*ibl))]*zdtgdp;
        ...
        if ((*yrecldp).laericesed && jm + 1 == 2) {
          ...
          zvqx[1] = (float) 0.002*(pow(zre_ice, (float) 1.0));
        }
        zfall = zvqx[jm]*zrho;
        //-------------------------------------------------
    ...
    if (zqpretot > zepsec) {
      zcovptot = (float) 1.0 - (((float) 1.0 - zcovptot)*((float) 1.0 - 
        fmax(za[jl + klon*(jk + klev*ibl)], za[jl + klon*(-1 + jk + klev*ibl)])) / 
	((float) 1.0 - fmin(za[jl + klon*(-1 + jk + klev*ibl)], (float) 1.0 - (float) 1.E-06)));
      zcovptot = fmax(zcovptot, (*yrecldp).rcovpmin);
      zcovpclr = fmax((float) 0.0, zcovptot - za[jl + klon*(jk + klev*ibl)]);          // clear sky proportion
    ...
    if (ztp1[jl + klon*(jk + klev*ibl)] <= rtt) {
      ...
      if (zicecld > zepsec) {
        ...
        zzco = ptsphy*(*yrecldp).rsnowlin1*exp((*yrecldp).rsnowlin2*
          (ztp1[jl + klon*(jk + klev*ibl)] - rtt));
        
        if ((*yrecldp).laericeauto) {
          zlcrit = picrit_aer[jl + klon*(jk + klev*ibl)];
          // 0.3 = N**0.333 with N=0.027
          zzco = zzco*(pow(((*yrecldp).rnice / pnice[jl + klon*(jk + klev*ibl)]), (float) 0.333));
        } else {
          zlcrit = (*yrecldp).rlcritsnow;
        }
        
        zsnowaut = zzco*((float) 1.0 - exp(-(pow((zicecld / zlcrit), (float)2))));
    ...
    if (zliqcld > zepsec) {
      ...
      if (iwarmrain == 1) {
        ...
        if ((*yrecldp).laerliqautolsp) {
          ...
          zzco = zzco*(pow(((*yrecldp).rccn / pccn[jl + klon*(jk + klev*ibl)]), (float) 0.333));
        } else {
        ...
        //------------------------------------------------------------------
        zprecip = (zpfplsx[jl + klon*(jk + (klev + 1)*(3 + 5*ibl))] + 
          zpfplsx[jl + klon*(jk + (klev + 1)*(2 + 5*ibl))]) / fmax(zepsec, zcovptot);
        zcfpr = (float) 1.0 + (*yrecldp).rprc1*sqrt(fmax(zprecip, (float) 0.0));
        ...
        if ((*yrecldp).laerliqcoll) {
          ...
          zcfpr = zcfpr*(pow(((*yrecldp).rccn / pccn[jl + klon*(jk + klev*ibl)]), (float) 0.333));
        }
        ...
        if (zliqcld / zlcrit < (float) 20.0) {
          ...
          zrainaut = zzco*((float) 1.0 - exp(-(pow((zliqcld / zlcrit), (float)2))));
        } else {
          zrainaut = zzco;
        }
        ...
        if (zliqcld > zlcrit) {
          ...
          zrainaut = (float) 1.5*za[jl + klon*(jk + klev*ibl)]*ptsphy*
            (*yrecldp).rcl_kkaau*(pow(zliqcld, (*yrecldp).rcl_kkbauq))*
          ...
          zrainacc = (float) 2.0*za[jl + klon*(jk + klev*ibl)]*ptsphy*(*yrecldp).rcl_kkaac*
    ...
    if (iwarmrain > 1) {
      ...
      if (ztp1[jl + klon*(jk + klev*ibl)] <= rtt && zliqcld > zepsec) {
        
        // Fallspeed air density correction
        zfallcorr = pow(((*yrecldp).rdensref / zrho), (float) 0.4);
        ...
        if (zsnowcld > zepsec && zcovptot > (float) 0.01) {
          ...
          zsnowrime = (float) 0.3*zcovptot*ptsphy*(*yrecldp).rcl_const7s*
            zfallcorr*(pow((zrho*zsnowcld*(*yrecldp).rcl_const1s), (*yrecldp).rcl_const8s));
          
          // Limit snow riming term
          zsnowrime = fmin(zsnowrime, (float) 1.0);
    ...
    if (zicetot > zepsec && ztp1[jl + klon*(jk + klev*ibl)] > rtt) {
      
      // Calculate subsaturation
      zsubsat = fmax(zqsice[jl + klon*(jk + klev*ibl)] - 
        zqx[jl + klon*(jk + klev*(4 + 5*ibl))], (float) 0.0);
      ...
      // due to evaporation.
      ztdmtw0 = ztp1[jl + klon*(jk + klev*ibl)] - rtt - zsubsat*(ztw1 + ztw2*(pap[jl + klon*(jk + klev*ibl)] - ztw3) - 
        ztw4*(ztp1[jl + klon*(jk + klev*ibl)] - ztw5));
      // Not implicit yet...
      // Ensure ZCONS1 is positive so that ZMELTMAX=0 if ZTDMTW0<0
      zcons1 = fabs(ptsphy*((float) 1.0 + (float) 0.5*ztdmtw0) / (*yrecldp).rtaumel);
      zmeltmax = fmax(ztdmtw0*zcons1*zrldcp, (float) 0.0);
    }
    ...
    // If rain present
    if (zqx[jl + klon*(jk + klev*(2 + 5*ibl))] > zepsec) {
      
      if (ztp1[jl + klon*(jk + klev*ibl)] <= rtt && ztp1[jl + klon*(-1 + jk + klev*ibl)] > rtt) {
        ...
        zqpretot = fmax(zqx[jl + klon*(jk + klev*(3 + 5*ibl))] + 
          zqx[jl + klon*(jk + klev*(2 + 5*ibl))], zepsec);
        prainfrac_toprfz[jl + klon*ibl] = zqx[jl + klon*(jk + klev*(2 + 5*ibl))] / zqpretot;
        if (prainfrac_toprfz[jl + klon*ibl] > (float)0.8) {
          llrainliq = true;
        } else {
          llrainliq = false;
        }
      ...
      if (ztp1[jl + klon*(jk + klev*ibl)] < rtt) {
        ...
        if (prainfrac_toprfz[jl + klon*ibl] > (float)0.8) {
          ...
          zlambda = pow(((*yrecldp).rcl_fac1 / (zrho*zqx[jl + klon*(jk + klev*(2 +5*ibl))])), 
            (*yrecldp).rcl_fac2);
          
          // Calculate freezing rate based on Bigg(1953) and Wisner(1972)
          ztemp = (*yrecldp).rcl_fzrab*(ztp1[jl + klon*(jk + klev*ibl)] - rtt);
          zfrz = ptsphy*((*yrecldp).rcl_const5r / zrho)*(exp(ztemp) -
            (float) 1.)*(pow(zlambda, (*yrecldp).rcl_const6r));
          zfrzmax = fmax(zfrz, (float) 0.0);
          ...
          zcons1 = fabs(ptsphy*((float) 1.0 + (float) 0.5*(rtt - 
            ztp1[jl + klon*(jk + klev*ibl)])) / (*yrecldp).rtaumel);
          zfrzmax = fmax((rtt - ztp1[jl + klon*(jk + klev*ibl)])*zcons1*zrldcp, (float) 0.0);
          
        }
        
        if (zfrzmax > zepsec) {
          zfrz = fmin(zqx[jl + klon*(jk + klev*(2 + 5*ibl))], zfrzmax);
    ...
    //----------------------------------------------------------------------
    // not implicit yet...
    zfrzmax = fmax(((*yrecldp).rthomo - ztp1[jl + klon*(jk + klev*ibl)])*zrldcp, (float) 0.0);
    ...
    if (ievaprain == 1) {
      ...
      zzrh = (*yrecldp).rprecrhmax + ((float) 1.0 - (*yrecldp).rprecrhmax)*zcovpmax / 
        fmax(zepsec, (float) 1.0 - za[jl + klon*(jk + klev*ibl)]);
      zzrh = fmin(fmax(zzrh, (*yrecldp).rprecrhmax), (float) 1.0);
      
      zqe = (zqx[jl + klon*(jk + klev*(4 + 5*ibl))] - za[jl + klon*(jk + klev*ibl)]*
        zqsliq[jl + klon*(jk + klev*ibl)]) / fmax(zepsec, (float) 1.0 - za[jl + klon*(jk + klev*ibl)]);
      //---------------------------------------------
      // humidity in moistest ZCOVPCLR part of domain
      //---------------------------------------------
      zqe = fmax((float) 0.0, fmin(zqe, zqsliq[jl + klon*(jk + klev*ibl)]));
      llo1 = zcovpclr > zepsec && zqxfg[2] > zepsec && zqe < zzrh*zqsliq[jl + klon*(jk + klev*ibl)];
      
      if (llo1) {
        // note: zpreclr is a rain flux
        zpreclr = zqxfg[2]*zcovpclr / copysign(fmax(fabs(zcovptot*zdtgdp), zepsilon), zcovptot*zdtgdp);
        
        //--------------------------------------
        // actual microphysics formula in zbeta
        //--------------------------------------
        
        zbeta1 = sqrt(pap[jl + klon*(jk + klev*ibl)] / paph[jl + klon*(klev + (klev + 1)*ibl)]) / 
          (*yrecldp).rvrfactor*zpreclr / fmax(zcovpclr, zepsec);
        
        zbeta = rg*(*yrecldp).rpecons*(float) 0.5*(pow(zbeta1, (float) 0.5777));
        
        zdenom = (float) 1.0 + zbeta*ptsphy*zcorqsliq;
        zdpr = zcovpclr*zbeta*(zqsliq[jl + klon*(jk + klev*ibl)] - zqe) / zdenom*zdp*zrg_r;
        zdpevap = zdpr*zdtgdp;
        ...
        //-------------------------------------------------------------
        zcovptot = fmax((*yrecldp).rcovpmin, zcovptot - fmax((float) 0.0, 
          (zcovptot - za[jl + klon*(jk + klev*ibl)])*zevap / zqxfg[2]));
      ...
      zzrh = fmin((float) 0.8, zzrh);
      
      zqe = fmax((float) 0.0, fmin(zqx[jl + klon*(jk + klev*(4 + 5*ibl))], 
        zqsliq[jl + klon*(jk + klev*ibl)]));
       
      llo1 = zcovpclr > zepsec && zqxfg[2] > zepsec && 
        zqe < zzrh*zqsliq[jl + klon*(jk + klev*ibl)];
      ...
      if (llo1) {
        ...
        zesatliq = rv / rd*((float)(r2es*exp((r3les*(ztp1[jl + klon*(jk + klev*ibl)] - rtt))/
          (ztp1[jl + klon*(jk + klev*ibl)] - r4les))));
        
        // Slope of particle size distribution
        zlambda = pow(((*yrecldp).rcl_fac1 / (zrho*zpreclr)), (*yrecldp).rcl_fac2);            // ZPRECLR=kg/kg
        
        zevap_denom = (*yrecldp).rcl_cdenom1*zesatliq - (*yrecldp).rcl_cdenom2*
          ztp1[jl + klon*(jk + klev*ibl)]*zesatliq + (*yrecldp).rcl_cdenom3*
          (pow(ztp1[jl + klon*(jk + klev*ibl)], (float) 3.))*pap[jl + klon*(jk + klev*ibl)];
        
        // Temperature dependent conductivity
        zcorr2 = (pow((ztp1[jl + klon*(jk + klev*ibl)] / (float) 273.), (float) 1.5))*(float) 393. / 
          (ztp1[jl + klon*(jk + klev*ibl)] + (float) 120.);
        zka = (*yrecldp).rcl_ka273*zcorr2;
        
        zsubsat = fmax(zzrh*zqsliq[jl + klon*(jk + klev*ibl)] - zqe, (float) 0.0);
        
        zbeta = ((float) 0.5 / zqsliq[jl + klon*(jk + klev*ibl)])*
          (pow(ztp1[jl + klon*(jk + klev*ibl)], (float) 2.))*zesatliq*(*yrecldp).rcl_const1r*(zcorr2 / 
          zevap_denom)*((float) 0.78 / (pow(zlambda, (*yrecldp).rcl_const4r)) + 
          (*yrecldp).rcl_const2r*(pow((zrho*zfallcorr), (float) 0.5)) / ((pow(zcorr2, 
          (float) 0.5))*(pow(zlambda, (*yrecldp).rcl_const3r))));
        ...
        // 2-flux treatment, abandoned due to the new prognostic precip
        //-------------------------------------------------------------
        zcovptot = fmax((*yrecldp).rcovpmin, zcovptot - fmax((float) 0.0, (zcovptot - 
          za[jl + klon*(jk + klev*ibl)])*zevap / zqxfg[2]));
    ...
    if (ievapsnow == 1) {
      ...
      // humidity in moistest ZCOVPCLR part of domain
      //---------------------------------------------
      zqe = fmax((float) 0.0, fmin(zqe, zqsice[jl + klon*(jk + klev*ibl)]));
      llo1 = zcovpclr > zepsec && zqxfg[3] > zepsec && 
        zqe < zzrh*zqsice[jl + klon*(jk + klev*ibl)];
      
      if (llo1) {
        // note: zpreclr is a rain flux a
        zpreclr = zqxfg[3]*zcovpclr / copysign(fmax(fabs(zcovptot*zdtgdp), zepsilon), zcovptot*zdtgdp);
        ...
        zbeta = rg*(*yrecldp).rpecons*(pow(zbeta1, (float) 0.5777));
        
        zdenom = (float) 1.0 + zbeta*ptsphy*zcorqsice;
        zdpr = zcovpclr*zbeta*(zqsice[jl + klon*(jk + klev*ibl)] - zqe) / zdenom*zdp*zrg_r;
        zdpevap = zdpr*zdtgdp;
        ...
        //-------------------------------------------------------------
        zcovptot = fmax((*yrecldp).rcovpmin, zcovptot - fmax((float) 0.0, (zcovptot - 
          za[jl + klon*(jk + klev*ibl)])*zevap / zqxfg[3]));
      ...
      //---------------------------------------------
      zqe = fmax((float) 0.0, fmin(zqe, zqsice[jl + klon*(jk + klev*ibl)]));
      llo1 = zcovpclr > zepsec && zqx[jl + klon*(jk + klev*(3 + 5*ibl))] > zepsec 
        && zqe < zzrh*zqsice[jl + klon*(jk + klev*ibl)];
      
      if (llo1) {
        
        // Calculate local precipitation (kg/kg)
        zpreclr = zqx[jl + klon*(jk + klev*(3 + 5*ibl))] / zcovptot;
        zvpice = ((float)(r2es*exp((r3ies*(ztp1[jl + klon*(jk + klev*ibl)] - rtt))/(ztp1[jl + klon*(jk + klev*ibl)] - r4ies))))*rv / rd;
        ...
        zaplusb = (*yrecldp).rcl_apb1*zvpice - (*yrecldp).rcl_apb2*zvpice*
          ztp1[jl + klon*(jk + klev*ibl)] + pap[jl + klon*(jk + klev*ibl)]*(*yrecldp).rcl_apb3*
          (pow(ztp1[jl + klon*(jk + klev*ibl)], (float)3));
        zcorrfac = pow(((float)1.0 / zrho), (float)0.5);
        zcorrfac2 = (pow((ztp1[jl + klon*(jk + klev*ibl)] / (float)273.0), (float)1.5))*((float)393.0 / (ztp1[jl + klon*(jk + klev*ibl)] + (float)120.0));
        
        zpr02 = zrho*zpreclr*(*yrecldp).rcl_const1s / (ztcg*zfacx1s);
        
        zterm1 = (zqsice[jl + klon*(jk + klev*ibl)] - zqe)*(pow(ztp1[jl + klon*(jk + klev*ibl)], (float)2))*
          zvpice*zcorrfac2*ztcg*(*yrecldp).rcl_const2s*zfacx1s / (zrho*zaplusb*zqsice[jl + klon*(jk + klev*ibl)]);
        zterm2 = (float)0.65*(*yrecldp).rcl_const6s*(pow(zpr02, (*yrecldp).rcl_const4s)) + 
          (*yrecldp).rcl_const3s*(pow(zcorrfac, (float)0.5))*(pow(zrho, (float)0.5))*(pow(zpr02, 
          (*yrecldp).rcl_const5s)) / (pow(zcorrfac2, (float)0.5));
        
        zdpevap = fmax(zcovpclr*zterm1*zterm2*ptsphy, (float) 0.0);
        
        //--------------------------------------------------------------------
        // Limit evaporation to snow amount
        //--------------------------------------------------------------------
        zevap = fmin(zdpevap, zevaplimice);
        zevap = fmin(zevap, zqx[jl + klon*(jk + klev*(3 + 5*ibl))]);
        ...
        //-------------------------------------------------------------
        zcovptot = fmax((*yrecldp).rcovpmin, zcovptot - fmax((float) 0.0, (zcovptot - 
          za[jl + klon*(jk + klev*ibl)])*zevap / zqx[jl + klon*(jk + klev*(3 + 5*ibl))]));
    ...
    //---------------------------
    zanew = (za[jl + klon*(jk + klev*ibl)] + zsolac) / ((float) 1.0 + zsolab);
    zanew = fmin(zanew, (float) 1.0);
    if (zanew < (*yrecldp).ramin) {
      zanew = (float) 0.0;
    }
    zda = zanew - zaorig[jl + klon*(jk + klev*ibl)];
    //---------------------------------
    ...
    //---------------------------------------
    for (jm = 0; jm <= 5 + -1; jm += 1) {
      zmax = fmax(zqx[jl + klon*(jk + klev*(jm + 5*ibl))], zepsec);
    ...
    for (jm = 0; jm <= 5 + -1; jm += 1) {
      ...
      //---------------------------
      zmm = fmax(zqx[jl + klon*(jk + klev*(jm + 5*ibl))], zepsec);
    ...
    for (jm = 0; jm <= 5 + -1; jm += 1) {
      ...
      zqxn[jm] = zqx[jl + klon*(jk + klev*(jm + 5*ibl))] + zexplicit;
    }
    ...
    for (jm = 0; jm <= 5 + -1; jm += 1) {
      zqxnm1[jm] = zqxn[jm];
      zqxn2d[jl + klon*(jk + klev*(jm + 5*ibl))] = zqxn[jm];
    }
    ...
    for (jm = 0; jm <= 5 + -1; jm += 1) {
      zpfplsx[jl + klon*(1 + jk + (klev + 1)*(jm + 5*ibl))] = 
        zfallsink[jm]*zqxn[jm]*zrdtgdp;
    }
    
    // Ensure precipitation fraction is zero if no precipitation
    zqpretot = zpfplsx[jl + klon*(1 + jk + (klev + 1)*(3 + 5*ibl))] + 
      zpfplsx[jl + klon*(1 + jk + (klev + 1)*(2 + 5*ibl))];
    if (zqpretot < zepsec) {
      zcovptot = (float) 0.0;
    }
    ...
    for (jm = 0; jm <= 5 - 1 + -1; jm += 1) {
      ...
      if (iphase[jm] == 1) {
        tendency_loc_t[jl + klon*(jk + klev*ibl)] = tendency_loc_t[jl + klon*(jk + klev*ibl)] + 
        ralvdcp*(zqxn[jm] - zqx[jl + klon*(jk + klev*(jm + 5*ibl))] - zfluxq[jm])*zqtmst;
      }
      
      if (iphase[jm] == 2) {
        tendency_loc_t[jl + klon*(jk + klev*ibl)] = tendency_loc_t[jl + klon*(jk + klev*ibl)] + 
          ralsdcp*(zqxn[jm] - zqx[jl + klon*(jk + klev*(jm + 5*ibl))] - zfluxq[jm])*zqtmst;
      }
      ...
      tendency_loc_cld[jl + klon*(jk + klev*(jm + 5*ibl))] = tendency_loc_cld[jl + klon*(jk + klev*(jm + 5*ibl))] + 
        (zqxn[jm] - zqx0[jl + klon*(jk + klev*(jm +  5*ibl))])*zqtmst;
    ...
    // 6.2 Humidity budget
    //----------------------
    tendency_loc_q[jl + klon*(jk + klev*ibl)] = tendency_loc_q[jl + klon*(jk + klev*ibl)] + 
      (zqxn[4] - zqx[jl + klon*(jk + klev*(4 + 5*ibl))])*zqtmst;
    
    //-------------------
    // 6.3 cloud cover
    //-----------------------
    tendency_loc_a[jl + klon*(jk + klev*ibl)] = tendency_loc_a[jl + klon*(jk + klev*ibl)] + zda*zqtmst;
  ...
  //--------------------------------------------------------------------
  for (jk = 0; jk <= klev + 1 + -1; jk += 1) {
    pfplsl[jl + klon*(jk + (klev + 1)*ibl)] = zpfplsx[jl + klon*(jk + (klev + 1)*(2 + 5*ibl))] + 
      zpfplsx[jl + klon*(jk + (klev + 1)*(0 + 5*ibl))];
    pfplsn[jl + klon*(jk + (klev + 1)*ibl)] = zpfplsx[jl + klon*(jk + (klev + 1)*(3 + 5*ibl))] + 
      zpfplsx[jl + klon*(jk + (klev + 1)*(1 + 5*ibl))];
  }
  ...
  for (jk = 0; jk <= klev + -1; jk += 1) {
    ...
    zgdph_r = -zrg_r*(paph[jl + klon*(1 + jk + (klev + 1)*ibl)] - 
      paph[jl + klon*(jk + (klev + 1)*ibl)])*zqtmst;
    pfsqlf[jl + klon*(1 + jk + (klev + 1)*ibl)] =  pfsqlf[jl + klon*(jk + (klev + 1)*ibl)];
    pfsqif[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfsqif[jl + klon*(jk + (klev + 1)*ibl)];
    pfsqrf[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfsqlf[jl + klon*(jk + (klev + 1)*ibl)];
    pfsqsf[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfsqif[jl + klon*(jk + (klev + 1)*ibl)];
    pfcqlng[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfcqlng[jl + klon*(jk + (klev + 1)*ibl)];
    pfcqnng[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfcqnng[jl + klon*(jk + (klev + 1)*ibl)];
    pfcqrng[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfcqlng[jl + klon*(jk + (klev + 1)*ibl)];
    pfcqsng[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfcqnng[jl + klon*(jk + (klev + 1)*ibl)];
    pfsqltur[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfsqltur[jl + klon*(jk + (klev + 1)*ibl)];
    pfsqitur[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfsqitur[jl + klon*(jk + (klev + 1)*ibl)];
    
    zalfaw = zfoealfa[jl + klon*(jk + (klev + 1)*ibl)];
    
    // Liquid , LS scheme minus detrainment
    pfsqlf[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfsqlf[jl + klon*(1 + jk + (klev + 1)*ibl)] + 
      (zqxn2d[jl + klon*(jk + klev*(0 + 5*ibl))] - zqx0[jl + klon*(jk + klev*(0 + 5*ibl))] + 
      pvfl[jl + klon*(jk + klev*ibl)]*ptsphy - zalfaw*plude[jl + klon*(jk + klev*ibl)])*zgdph_r;
    // liquid, negative numbers
    pfcqlng[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfcqlng[jl + klon*(1 + jk + (klev + 1)*ibl)] + 
      zlneg[jl + klon*(jk + klev*(0 + 5*ibl))]*zgdph_r;
    
    // liquid, vertical diffusion
    pfsqltur[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfsqltur[jl + klon*(1 + jk + (klev + 1)*ibl)] + 
      pvfl[jl + klon*(jk + klev*ibl)]*ptsphy*zgdph_r;
    
    // Rain, LS scheme
    pfsqrf[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfsqrf[jl + klon*(1 + jk + (klev + 1)*ibl)] + 
      (zqxn2d[jl + klon*(jk + klev*(2 + 5*ibl))] - zqx0[jl + klon*(jk + klev*(2 + 5*ibl))])*zgdph_r;
    // rain, negative numbers
    pfcqrng[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfcqrng[jl + klon*(1 + jk + (klev + 1)*ibl)] + 
      zlneg[jl + klon*(jk + klev*(2 + 5*ibl))]*zgdph_r;
    
    // Ice , LS scheme minus detrainment
    pfsqif[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfsqif[jl + klon*(1 + jk + (klev + 1)*ibl)] + 
      (zqxn2d[jl + klon*(jk + klev*(1 + 5*ibl))] - zqx0[jl + klon*(jk + klev*(1 + 5*ibl))] + 
      pvfi[jl + klon*(jk + klev*ibl)]*ptsphy - ((float) 1.0 - zalfaw)*plude[jl + klon*(jk + klev*ibl)])*zgdph_r;
    // ice, negative numbers
    pfcqnng[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfcqnng[jl + klon*(1 + jk + (klev + 1)*ibl)] + 
      zlneg[jl + klon*(jk + klev*(1 + 5*ibl))]*zgdph_r;
    
    // ice, vertical diffusion
    pfsqitur[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfsqitur[jl + klon*(1 + jk + (klev + 1)*ibl)] + 
      pvfi[jl + klon*(jk + klev*ibl)]*ptsphy*zgdph_r;
    
    // snow, LS scheme
    pfsqsf[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfsqsf[jl + klon*(1 + jk + (klev + 1)*ibl)] + 
      (zqxn2d[jl + klon*(jk + klev*(3 + 5*ibl))] - zqx0[jl + klon*(jk + klev*(3 + 5*ibl))])*zgdph_r;
    // snow, negative numbers
    pfcqsng[jl + klon*(1 + jk + (klev + 1)*ibl)] = pfcqsng[jl + klon*(1 + jk + (klev + 1)*ibl)] + 
      zlneg[jl + klon*(jk + klev*(3 + 5*ibl))]*zgdph_r;
  }
  ...
  //-----------------------------------
  for (jk = 0; jk <= klev + 1 + -1; jk += 1) {
    pfhpsl[jl + klon*(jk + (klev + 1)*ibl)] = -rlvtt*pfplsl[jl + klon*(jk + (klev + 1)*ibl)];
    pfhpsn[jl + klon*(jk + (klev + 1)*ibl)] = -rlstt*pfplsn[jl + klon*(jk + (klev + 1)*ibl)];
  }
}
