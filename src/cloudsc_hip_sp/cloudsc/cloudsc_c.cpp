...
__global__ void cloudsc_c(int kidia, int kfdia, int klon, float ptsphy,
  ...
  for (jm = 0; jm <= 5 + -1; jm += 1) {
    for (jk = 0; jk <= klev + 1 + -1; jk += 1) {
      zpfplsx[jk + (klev + 1)*jm] = (float) 0.0;          // precip fluxes
    }
  }

  for (jm = 0; jm <= 5 + -1; jm += 1) {
    for (jk = 0; jk <= klev + -1; jk += 1) {
      zqxn2d[jk + klev*jm] = (float) 0.0;          // end of timestep values in 2D
      zlneg[jk + klev*jm] = (float) 0.0;          // negative input check
    }
  }
  ...
  for (jk = 0; jk <= klev + -1; jk += 1) {
    ...
    //----------------------------------------
    zfoealfa[jk] = ((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2))));
    zfoeewmt[jk] =
      fmin(((float)(r2es*((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2)))*exp((r3les*(ztp1[jk] - rtt))/(ztp1[jk] - r4les)) + ((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2))))*exp((r3ies*(ztp1[jk] - rtt))/(ztp1[jk] - r4ies))))) / pap[jl + klon*(jk + klev*(ibl))], (float) 0.5);
    zqsmix[jk] = zfoeewmt[jk];
    zqsmix[jk] = zqsmix[jk] / ((float) 1.0 - retv*zqsmix[jk]);

    //---------------------------------------------
    // ice saturation T<273K
    // liquid water saturation for T>273K
    //---------------------------------------------
    zalfa = ((float)(fmax((float)0.0, copysign((float)1.0, ztp1[jk] - rtt))));
    zfoeew[jk] = fmin((zalfa*((float)(r2es*exp((r3les*(ztp1[jk] - rtt))/(ztp1[jk] - r4les)))) + ((float) 1.0 - zalfa)*((float)(r2es*exp((r3ies*(ztp1[jk] - rtt))/(ztp1[jk] - r4ies))))) / pap[jl +
      klon*(jk + klev*(ibl))], (float) 0.5);
    zfoeew[jk] = fmin((float) 0.5, zfoeew[jk]);
    zqsice[jk] = zfoeew[jk] / ((float) 1.0 - retv*zfoeew[jk]);

    //----------------------------------
    // liquid water saturation
    //----------------------------------
    zfoeeliqt[jk] =
      fmin(((float)(r2es*exp((r3les*(ztp1[jk] - rtt))/(ztp1[jk] - r4les)))) / pap[jl + klon*(jk + klev*(ibl))], (float) 0.5);
    zqsliq[jk] = zfoeeliqt[jk];
    zqsliq[jk] = zqsliq[jk] / ((float) 1.0 - retv*zqsliq[jk]);

    //   !----------------------------------
    //   ! ice water saturation
    //   !----------------------------------
    //   ZFOEEICET(JL,JK)=MIN(((float)(r2es*exp((r3ies*(ztp1[jk] - rtt))/(ztp1[jk] - r4ies))))(ZTP1(JL,JK))/PAP(JL,JK),0.5_JPRB)
  ...
  for (jk = 0; jk <= klev + -1; jk += 1) {
    ...
    //------------------------------------------
    za[jk] = fmax((float) 0.0, fmin((float) 1.0, za[jk]));
    ...
    if (zli[jk] > (*yrecldp).rlmin) {
      ...
      zicefrac[jk] = (float) 1.0 - zliqfrac[jk];
    } else {
      zliqfrac[jk] = (float) 0.0;
      zicefrac[jk] = (float) 0.0;
    }
  ...
  for (jk = 0; jk <= klev - 1 + -1; jk += 1) {
    ...
    if (zsig > (float) 0.1 && zsig < (float) 0.4 && ztp1[jk] > ztp1[1 + jk]) {
      ztrpaus = zsig;
    }
  }
  ...
  for (jk = -1 + (*yrecldp).ncldtop; jk <= klev + -1; jk += 1) {
    ...
    // liquid
    zfacw = r5les / (pow((ztp1[jk] - r4les), (float)2));
    zcor = (float) 1.0 / ((float) 1.0 - retv*zfoeeliqt[jk]);
    zdqsliqdt = zfacw*zcor*zqsliq[jk];
    zcorqsliq = (float) 1.0 + ralvdcp*zdqsliqdt;

    // ice
    zfaci = r5ies / (pow((ztp1[jk] - r4ies), (float)2));
    zcor = (float) 1.0 / ((float) 1.0 - retv*zfoeew[jk]);
    ...
    zcor = (float) 1.0 / ((float) 1.0 - retv*zfoeewmt[jk]);
    zdqsmixdt = zfac*zcor*zqsmix[jk];
    zcorqsmix = (float) 1.0 + ((float)((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2)))*ralvdcp + ((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2))))*ralsdcp))*zdqsmixdt;

    // evaporation/sublimation limits
    zevaplimmix =
      fmax((zqsmix[jk] - zqx[jk + klev*(4)]) / zcorqsmix, (float) 0.0);
    zevaplimliq =
      fmax((zqsliq[jk] - zqx[jk + klev*(4)]) / zcorqsliq, (float) 0.0);
    zevaplimice =
      fmax((zqsice[jk] - zqx[jk + klev*(4)]) / zcorqsice, (float) 0.0);

    //--------------------------------
    // in-cloud consensate amount
    //--------------------------------
    ztmpa = (float) 1.0 / fmax(za[jk], zepsec);
    ...
    zfokoop = ((float)(fmin(rkoop1 - rkoop2*ztp1[jk], (float)(r2es*exp((r3les*(ztp1[jk] - rtt))/(ztp1[jk] - r4les)))*(float)1.0/(float)(r2es*exp((r3ies*(ztp1[jk] - rtt))/(ztp1[jk] - r4ies))))));

    if (ztp1[jk] >= rtt || (*yrecldp).nssopt == 0) {
      zfac = (float) 1.0;
      zfaci = (float) 1.0;
    } else {
      zfac = za[jk] + zfokoop*((float) 1.0 - za[jk]);
    ...
    if (za[jk] > (float) 1.0 - (*yrecldp).ramin) {
      zsupsat =
        fmax((zqx[jk + klev*(4)] - zfac*zqsice[jk]) / zcorqsice, (float) 0.0);
    } else {
      // Calculate environmental humidity supersaturation
      zqp1env = (zqx[jk + klev*(4)] - za[jk]*zqsice[jk]) / fmax((float) 1.0 -
        za[jk], zepsilon);
      //& SIGN(MAX(ABS(1.0_JPRB-ZA(JL,JK)),ZEPSILON),1.0_JPRB-ZA(JL,JK))
      zsupsat = fmax(((float) 1.0 - za[jk])*(zqp1env - zfac*zqsice[jk]) / zcorqsice,
        (float) 0.0);
    }
    ...
    if (jk + 1 > (*yrecldp).ncldtop) {
      ...
      zmf = fmax((float) 0.0, (pmfu[jl + klon*(jk + klev*(ibl))] + pmfd[-1 +
      ...
      zdtdp = zrdcp*(float) 0.5*(ztp1[-1 + jk] + ztp1[jk]) / paph[jl + klon*(jk +
         (klev + 1)*(ibl))];
    ...
    if (zli[jk] > zepsec) {
      ...
      ze = zldifdt*fmax(zqsmix[jk] - zqx[jk + klev*(4)], (float) 0.0);
      zleros = za[jk]*ze;
    ...
    zwtot = pvervel[jl + klon*(jk + klev*(ibl))] + (float) 0.5*rg*(pmfu[
    ...
    ztp1[jk] = fmax(ztp1[jk], (float) 160.0);
    llflag = true;

    // Formerly a call to CUADJTQ(..., ICALL=5)
    zqp = (float) 1.0 / pap[jl + klon*(jk + klev*(ibl))];
    zqsat = ((float)(r2es*((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2)))*exp((r3les*(ztp1[jk] - rtt))/(ztp1[jk] - r4les)) + ((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2))))*exp((r3ies*(ztp1[jk] - rtt))/(ztp1[jk] - r4ies)))))*zqp;
    zqsat = fmin((float) 0.5, zqsat);
    zcor = (float) 1.0 / ((float) 1.0 - retv*zqsat);
    zqsat = zqsat*zcor;
    zcond = (zqsmix[jk] - zqsat) / ((float) 1.0 + zqsat*zcor*((float)(((float)(fmin(1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2)))*r5alvcp)*((float)1.0/pow(ztp1[jk] - r4les, (float)2)) + (((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2))))*r5alscp)*((float)1.0/pow(ztp1[jk] - r4ies, (float)2)))));
    ztp1[jk] = ztp1[jk] + ((float)((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2)))*ralvdcp + ((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2))))*ralsdcp))*zcond;
    zqsmix[jk] = zqsmix[jk] - zcond;
    zqsat = ((float)(r2es*((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2)))*exp((r3les*(ztp1[jk] - rtt))/(ztp1[jk] - r4les)) + ((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2))))*exp((r3ies*(ztp1[jk] - rtt))/(ztp1[jk] - r4ies)))))*zqp;
    zqsat = fmin((float) 0.5, zqsat);
    zcor = (float) 1.0 / ((float) 1.0 - retv*zqsat);
    zqsat = zqsat*zcor;
    zcond1 = (zqsmix[jk] - zqsat) / ((float) 1.0 + zqsat*zcor*((float)(((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2)))*r5alvcp)*((float)1.0/pow(ztp1[jk] - r4les, (float)2)) + (((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2))))*r5alscp)*((float)1.0/pow(ztp1[jk] - r4ies, (float)2)))));
    ztp1[jk] = ztp1[jk] + ((float)((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2)))*ralvdcp + ((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2))))*ralsdcp))*zcond1;
    ...
    if (zdqs > (float) 0.0) {
      ...
      zlevap = fmin(zlevap, fmax(zqsmix[jk] - zqx[jk + klev*(4)], (float) 0.0));
    ...
    if (za[jk] > zepsec && zdqs <= -(*yrecldp).rlmin) {
      ...
      if (za[jk] > (float) 0.99) {
        zcor = (float) 1.0 / ((float) 1.0 - retv*zqsmix[jk]);
        zcdmax = (zqx[jk + klev*(4)] - zqsmix[jk]) / ((float) 1.0 +
          zcor*zqsmix[jk]*((float)(((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2)))*r5alvcp)*((float)1.0/pow(ztp1[jk] - r4les, (float)2)) + (((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, (float)2))))*r5alscp)*((float)1.0/pow(ztp1[jk] - r4ies, (float)2)))));
      } else {
    ...
    if (zdqs <= -(*yrecldp).rlmin && za[jk] < (float) 1.0 - zepsec) {
      ...
      if (zsigk > (float) 0.8) {
        zrhc = (*yrecldp).ramid + ((float) 1.0 - (*yrecldp).ramid)*(pow(((zsigk -
          (float) 0.8) / (float) 0.2), (float)2));
      }
      ...
      if ((*yrecldp).nssopt == 0) {
        // No scheme
        zqe = (zqx[jk + klev*(4)] - za[jk]*zqsice[jk]) / fmax(zepsec, (float) 1.0
           - za[jk]);
      ...
      if (zqe >= zrhc*zqsice[jk]*zfac && zqe < zqsice[jk]*zfac) {
        ...
        zacond = -((float) 1.0 - za[jk])*zfac*zdqs / fmax((float)
          2.0*(zfac*zqsice[jk] - zqe), zepsec);

        zacond = fmin(zacond, (float) 1.0 - za[jk]);            //PUT THE LIMITER BACK
        ...
        zzdl =
          (float) 2.0*(zfac*zqsice[jk] - zqe) / fmax(zepsec, (float) 1.0 - za[jk]);
        // Added correction term ZFAC 15/03/2010
        if (zfac*zdqs < -zzdl) {
          // ZLCONDLIM=(ZA(JL,JK)-1.0_JPRB)*ZDQS(JL)-ZQSICE(JL,JK)+ZQX(JL,JK,NCLDQV)
          zlcondlim = (za[jk] - (float) 1.0)*zfac*zdqs - zfac*zqsice[jk] + zqx[jk +
            klev*(4)];
          zlcond2 = fmin(zlcond2, zlcondlim);
        }
        zlcond2 = fmax(zlcond2, (float) 0.0);

        if (zlcond2 < (*yrecldp).rlmin || ((float) 1.0 - za[jk]) < zepsec) {
          zlcond2 = (float) 0.0;
          zacond = (float) 0.0;
        }
    ...
    if (idepice == 1) {
      ...
      if (ztp1[jk] < rtt && zqxfg[0] > (*yrecldp).rlmin) {
        ...
        zvpice = ((float)(r2es*exp((r3ies*(ztp1[jk] - rtt))/(ztp1[jk] - r4ies))))*rv / rd;
        zvpliq = zvpice*zfokoop;
        zicenuclei = (float) 1000.0*exp((float) 12.96*(zvpliq - zvpice) / zvpliq -
          (float) 0.639);
        ...
        //------------------------------------------------
        zadd =
          rlstt*(rlstt / (rv*ztp1[jk]) - (float) 1.0) / ((float) 2.4E-2*ztp1[jk]);
        zbdd = rv*ztp1[jk]*pap[jl + klon*(jk + klev*(ibl))] / ((float)
          2.21*zvpice);
        ...
        // new value of ice:
        //------------------
        zinew = pow(((float) 0.666*zcvds*ptsphy + (pow(zice0, (float) 0.666))),
          (float) 1.5);

        //---------------------------
        // grid-mean deposition rate:
        //---------------------------
        zdepos = fmax(za[jk]*(zinew - zice0), (float) 0.0);
        ...
        zdepos = zdepos*fmin(zinfactor + ((float) 1.0 - zinfactor)*((*yrecldp)
      ...
      if (ztp1[jk] < rtt && zqxfg[0] > (*yrecldp).rlmin) {
        ...
        zcorrfac2 = (pow((ztp1[jk] / (float) 273.0), (float) 1.5))*((float) 393.0 /
          (ztp1[jk] + (float) 120.0));

        zpr02 = zrho*zice0*(*yrecldp).rcl_const1i / (ztcg*zfacx1i);

        zterm1 = (zvpliq - zvpice)*(pow(ztp1[jk], (float) 2.0))
          *zvpice*zcorrfac2*ztcg*(*yrecldp).rcl_const2i*zfacx1i / (zrho*zaplusb*zvpice)
          ;
        zterm2 = (float) 0.65*(*yrecldp).rcl_const6i*(pow(zpr02, (*yrecldp)
          .rcl_const4i)) + (*yrecldp).rcl_const3i*(pow(zcorrfac, (float) 0.5))
          *(pow(zrho, (float) 0.5))*(pow(zpr02, (*yrecldp).rcl_const5i)) /
          (pow(zcorrfac2, (float) 0.5));

        zdepos = fmax(za[jk]*zterm1*zterm2*ptsphy, (float) 0.0);
    ...
    //---------------------------------------------------------------
    if (zqpretot > zepsec) {
      zcovptot = (float) 1.0 - (((float) 1.0 - zcovptot)*((float) 1.0 - fmax(za[jk],
         za[-1 + jk])) / ((float) 1.0 - fmin(za[-1 + jk], (float) 1.0 - (float)
        1.E-06)));
      zcovptot = fmax(zcovptot, (*yrecldp).rcovpmin);
      zcovpclr = fmax((float) 0.0, zcovptot - za[jk]);          // clear sky proportion
    ...
    if (zliqcld > zepsec) {
      ...
      if (iwarmrain == 1) {
        ...
        if (zliqcld > zlcrit) {
          ...
          zrainaut = (float) 1.5*za[jk]*ptsphy*(*yrecldp).rcl_kkaau*(pow(zliqcld,
          ...
          zrainacc = (float) 2.0*za[jk]*ptsphy*(*yrecldp)
    ...
    if (zicetot > zepsec && ztp1[jk] > rtt) {
      ...
      zsubsat = fmax(zqsice[jk] - zqx[jk + klev*(4)], (float) 0.0);
    ...
    if (zqx[jk + klev*(2)] > zepsec) {
      ...
      if (ztp1[jk] < rtt) {
        ...
        if (prainfrac_toprfz[jl + klon*(ibl)] > (float)0.8) {
          ...
          zfrz = ptsphy*((*yrecldp).rcl_const5r / zrho)*(exp(ztemp) - (float) 1.)
          ...
          zcons1 = fabs(ptsphy*((float) 1.0 + (float) 0.5*(rtt - ztp1[jk])) /
            (*yrecldp).rtaumel);
          zfrzmax = fmax((rtt - ztp1[jk])*zcons1*zrldcp, (float) 0.0);
    ...
    //----------------------------------------------------------------------
    // not implicit yet...
    zfrzmax = fmax(((*yrecldp).rthomo - ztp1[jk])*zrldcp, (float) 0.0);
    ...
    if (ievaprain == 1) {
      ...
      zzrh = (*yrecldp).rprecrhmax + ((float) 1.0 - (*yrecldp).rprecrhmax)*zcovpmax /
        fmax(zepsec, (float) 1.0 - za[jk]);
      zzrh = fmin(fmax(zzrh, (*yrecldp).rprecrhmax), (float) 1.0);

      zqe = (zqx[jk + klev*(4)] - za[jk]*zqsliq[jk]) / fmax(zepsec, (float) 1.0 -
         za[jk]);
      //---------------------------------------------
      // humidity in moistest ZCOVPCLR part of domain
      //---------------------------------------------
      zqe = fmax((float) 0.0, fmin(zqe, zqsliq[jk]));
      ...
      zqe = fmax((float) 0.0, fmin(zqx[jk + klev*(4)], zqsliq[jk]));
      ...
      if (llo1) {
        ...
        zesatliq = rv / rd*((float)(r2es*exp((r3les*(ztp1[jk] - rtt))/(ztp1[jk] - r4les))));
        ...
        zevap_denom = (*yrecldp).rcl_cdenom1*zesatliq - (*yrecldp)
          ...
          (float) 3.))*pap[jl + klon*(jk + klev*(ibl))];

        // Temperature dependent conductivity
        zcorr2 = (pow((ztp1[jk] / (float) 273.), (float) 1.5))*(float) 393. /
          (ztp1[jk] + (float) 120.);
        zka = (*yrecldp).rcl_ka273*zcorr2;

        zsubsat = fmax(zzrh*zqsliq[jk] - zqe, (float) 0.0);

        zbeta = ((float) 0.5 / zqsliq[jk])*(pow(ztp1[jk], (float) 2.))
          *zesatliq*(*yrecldp).rcl_const1r*(zcorr2 / zevap_denom)*((float) 0.78 /
          (pow(zlambda, (*yrecldp).rcl_const4r)) + (*yrecldp)
          .rcl_const2r*(pow((zrho*zfallcorr), (float) 0.5)) / ((pow(zcorr2, (float)
    ...
    if (ievapsnow == 1) {
      ...
      // humidity in moistest ZCOVPCLR part of domain
      //---------------------------------------------
      zqe = fmax((float) 0.0, fmin(zqe, zqsice[jk]));
      ...
      if (llo1) {
        ...
        zaplusb = (*yrecldp).rcl_apb1*zvpice - (*yrecldp).rcl_apb2*zvpice*ztp1[jk] +
          pap[jl + klon*(jk + klev*(ibl))]*(*yrecldp).rcl_apb3*(pow(ztp1[jk],
           (float)3));
        zcorrfac = pow(((float)1.0 / zrho), (float)0.5);
        zcorrfac2 = (pow((ztp1[jk] / (float)273.0), (float)1.5))*((float)393.0 / (ztp1[jk] + (float)120.0));

        zpr02 = zrho*zpreclr*(*yrecldp).rcl_const1s / (ztcg*zfacx1s);

        zterm1 = (zqsice[jk] - zqe)*(pow(ztp1[jk], (float)2))*zvpice*zcorrfac2*ztcg*(*yrecldp)
    ...
    //---------------------------
    zanew = (za[jk] + zsolac) / ((float) 1.0 + zsolab);
  ...
  for (jk = 0; jk <= klev + -1; jk += 1) {
    ...
    pfsqif[jl + klon*(1 + jk + (klev + 1)*(ibl))] = pfsqif[jl + klon*(1
      + jk + (klev + 1)*(ibl))] + (zqxn2d[jk + klev*(1)] - zqx0[jk + klev*(-1
       + 2)] + pvfi[jl + klon*(jk + klev*(ibl))]*ptsphy - ((float) 1.0 -
