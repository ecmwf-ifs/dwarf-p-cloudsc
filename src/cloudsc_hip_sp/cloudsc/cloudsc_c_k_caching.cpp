...
__global__ void cloudsc_c(int kidia, int kfdia, int klon, float ptsphy,
  ...
  for (jk = 0; jk <= klev + 1 + -1; jk += 1) {
    ...
    if (1 <= jk + 1 && jk + 1 <= klev) {
      ...
      //----------------------------------------
      zfoealfa = ((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2))));
      zfoeewmt = fmin(((float)(r2es*((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2)))*
        exp((r3les*(ztp1[jk_i] - rtt))/(ztp1[jk_i] - r4les)) + 
	((float)1.0 - (float)(fmin(1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2))))*
	exp((r3ies*(ztp1[jk_i] - rtt))/(ztp1[jk_i] - r4ies))))) / pap[jl + klon*(jk + klev*ibl)], (float) 0.5);
      zqsmix = zfoeewmt;
      ...
      //---------------------------------------------
      zalfa = ((float)(fmax(0.0, copysign(1.0, ztp1[jk_i] - rtt))));
      zfoeew = fmin((zalfa*((float)(r2es*exp((r3les*(ztp1[jk_i] - rtt))/(ztp1[jk_i] - r4les)))) + 
        ((float) 1.0 - zalfa)*((float)(r2es*exp((r3ies*(ztp1[jk_i] - rtt))/(ztp1[jk_i] - r4ies))))) / 
        pap[jl + klon*(jk + klev*ibl)], (float) 0.5);
      zfoeew = fmin((float) 0.5, zfoeew);
      zqsice = zfoeew / ((float) 1.0 - retv*zfoeew);

      //----------------------------------
      // liquid water saturation
      //----------------------------------
      zfoeeliqt = fmin(((float)(r2es*exp((r3les*(ztp1[jk_i] - rtt))/(ztp1[jk_i] - r4les)))) / 
        pap[jl + klon*(jk + klev*ibl)], (float) 0.5);
      zqsliq = zfoeeliqt;
      ...
      //------------------------------------------
      za[jk_i] = fmax((float) 0.0, fmin((float) 1.0, za[jk_i]));
      ...
      if ((*yrecldp).ncldtop <= jk + 1 && jk + 1 <= klev) {
        ...
        zcorqsmix = (float) 1.0 + ((float)((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2)))*
          ralvdcp + ((float)1.0 - (float)(fmin(1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2))))*ralsdcp))*zdqsmixdt;

        // evaporation/sublimation limits
        zevaplimmix = fmax((zqsmix - zqx[4]) / zcorqsmix, (float) 0.0);
        zevaplimliq = fmax((zqsliq - zqx[4]) / zcorqsliq, (float) 0.0);
        zevaplimice = fmax((zqsice - zqx[4]) / zcorqsice, (float) 0.0);

        //--------------------------------
        // in-cloud consensate amount
        //--------------------------------
        ztmpa = (float) 1.0 / fmax(za[jk_i], zepsec);
        zliqcld = zqx[0]*ztmpa;
        zicecld = zqx[1]*ztmpa;
        ...
        zfokoop = ((float)(fmin(rkoop1 - rkoop2*ztp1[jk_i], (float)(r2es*exp((r3les*(ztp1[jk_i] - rtt))/
	  (ztp1[jk_i] - r4les)))*(float)1.0/(float)(r2es*exp((r3ies*(ztp1[jk_i] - rtt))/(ztp1[jk_i] - r4ies))))));
        ...
        if (za[jk_i] > (float) 1.0 - (*yrecldp).ramin) {
          zsupsat = fmax((zqx[4] - zfac*zqsice) / zcorqsice, (float) 0.0);
        } else {
          // Calculate environmental humidity supersaturation
          zqp1env = (zqx[4] - za[jk_i]*zqsice) / fmax((float) 1.0 - za[jk_i], zepsilon);
          //& SIGN(MAX(ABS(1.0_JPRB-ZA(JL,JK)),ZEPSILON),1.0_JPRB-ZA(JL,JK))
          zsupsat = fmax(((float) 1.0 - za[jk_i])*(zqp1env - zfac*zqsice) /
            zcorqsice, (float) 0.0);
        }
        ...
        if (jk + 1 > (*yrecldp).ncldtop) {
          ...
          zmf = fmax((float) 0.0, (pmfu[jl + klon*(jk + klev*ibl)] + pmfd[jl +
        ...
        if (jk + 1 < klev) {
          ...
          zmfdn = fmax((float) 0.0, (pmfu[jl + klon*(1 + jk + klev*ibl)] + pmfd[jl +
        ...
        if (zli > zepsec) {
          ...
          ze = zldifdt*fmax(zqsmix - zqx[4], (float) 0.0);
          zleros = za[jk_i]*ze;
        ...
        ztp1[jk_i] = fmax(ztp1[jk_i], (float) 160.0);
        llflag = true;

        // Formerly a call to CUADJTQ(..., ICALL=5)
        zqp = (float) 1.0 / pap[jl + klon*(jk + klev*ibl)];
        zqsat = ((float)(r2es*((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2)))*
          exp((r3les*(ztp1[jk_i] - rtt))/(ztp1[jk_i] - r4les)) + 
	  (1.0 - (float)(fmin(1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2))))*
	  exp((r3ies*(ztp1[jk_i] - rtt))/(ztp1[jk_i] - r4ies)))))*zqp;
        zqsat = fmin((float) 0.5, zqsat);
        zcor = (float) 1.0 / ((float) 1.0 - retv*zqsat);
        zqsat = zqsat*zcor;
        zcond = (zqsmix - zqsat) / ((float) 1.0 + zqsat*zcor*((float)(((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2)))*r5alvcp)*
          ((float)1.0/pow(ztp1[jk_i] - r4les, (float)2)) + (((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2))))*r5alscp)*
	  ((float)1.0/pow(ztp1[jk_i] - r4ies, (float)2)))));
        ztp1[jk_i] = ztp1[jk_i] + ((float)((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2)))*ralvdcp + 
          ((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2))))*ralsdcp))*zcond;
        zqsmix = zqsmix - zcond;
        zqsat = ((float)(r2es*((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2)))*
          exp((r3les*(ztp1[jk_i] - rtt))/(ztp1[jk_i] - r4les)) + 
          ((float)1.0 - (float)(fmin(1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2))))*
        ...
        zcor = (float) 1.0 / ((float) 1.0 - retv*zqsat);
        zqsat = zqsat*zcor;
        zcond1 = (zqsmix - zqsat) / ((float) 1.0 + zqsat*zcor*((float)(((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2)))*r5alvcp)*
          ((float)1.0/pow(ztp1[jk_i] - r4les, (float)2)) + (((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2))))*r5alscp)*
	  ((float)1.0/pow(ztp1[jk_i] - r4ies, (float)2)))));
        ztp1[jk_i] = ztp1[jk_i] + ((float)((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2)))*ralvdcp + 
          ((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2))))*ralsdcp))*zcond1;
        ...
        if (zdqs > (float) 0.0) {
          ...
          zlevap = fmin(zlevap, fmax(zqsmix - zqx[4], (float) 0.0));
        ...
        if (za[jk_i] > zepsec && zdqs <= -(*yrecldp).rlmin) {
          ...
          if (za[jk_i] > (float) 0.99) {
            ...
            zcdmax = (zqx[4] - zqsmix) / ((float) 1.0 + zcor*zqsmix*((float)(((float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2)))*r5alvcp)*
              ((float)1.0/pow(ztp1[jk_i] - r4les, (float)2)) + (((float)1.0 - (float)(fmin((float)1.0, pow((fmax(rtice, fmin(rtwat, ztp1[jk_i])) - rtice)*rtwat_rtice_r, (float)2))))*r5alscp)*
	      ((float)1.0/pow(ztp1[jk_i] - r4ies, (float)2)))));
          } else {
        ...
        if (zdqs <= -(*yrecldp).rlmin && za[jk_i] < (float) 1.0 - zepsec) {
          ...
          if ((*yrecldp).nssopt == 0) {
            // No scheme
            zqe = (zqx[4] - za[jk_i]*zqsice) / fmax(zepsec, (float) 1.0 -
              za[jk_i]);
          ...
          if (zqe >= zrhc*zqsice*zfac && zqe < zqsice*zfac) {
            ...
            zacond = -((float) 1.0 - za[jk_i])*zfac*zdqs / fmax((float)
              2.0*(zfac*zqsice - zqe), zepsec);

            zacond = fmin(zacond, (float) 1.0 - za[jk_i]);                //PUT THE LIMITER BACK
            ...
            zzdl = (float) 2.0*(zfac*zqsice - zqe) / fmax(zepsec, (float) 1.0 - za[jk_i]);
            // Added correction term ZFAC 15/03/2010
            if (zfac*zdqs < -zzdl) {
        ...
        if (idepice == 1) {
          ...
          if (ztp1[jk_i] < rtt && zqxfg[0] > (*yrecldp).rlmin) {
            ...
            zvpice = (((float)(r2es*exp((r3ies*(ztp1[jk_i] - rtt))/(ztp1[jk_i] - r4ies))))*rv) / rd;
            ...
            //---------------------------
            zdepos = fmax(za[jk_i]*(zinew - zice0), (float) 0.0);
          ...
          if (ztp1[jk_i] < rtt && zqxfg[0] > (*yrecldp).rlmin) {
            ...
            zaplusb = (*yrecldp).rcl_apb1*zvpice - (*yrecldp).rcl_apb2*zvpice*ztp1[jk_i] + 
              pap[jl + klon*(jk + klev*ibl)]*(*yrecldp).rcl_apb3*(pow(ztp1[jk_i], (float) 3.));
            zcorrfac = pow(((float) 1.0 / zrho), (float) 0.5);
            zcorrfac2 = (pow((ztp1[jk_i] / (float) 273.0), (float) 1.5))
              *((float) 393.0 / (ztp1[jk_i] + (float) 120.0));

            zpr02 = zrho*zice0*(*yrecldp).rcl_const1i / (ztcg*zfacx1i);

            zterm1 = (zvpliq - zvpice)*(pow(ztp1[jk_i], (float) 2.0))
            ...
            zdepos = fmax(za[jk_i]*zterm1*zterm2*ptsphy, (float) 0.0);
        ...
        if (zqpretot > zepsec) {
          zcovptot = (float) 1.0 - (((float) 1.0 - zcovptot)*((float) 1.0 -
            fmax(za[jk_i], za[jk_im1]))) / ((float) 1.0 - fmin(za[jk_im1], (float) 1.0 - (float) 1.E-06));              // here!!!
          zcovptot = fmax(zcovptot, (*yrecldp).rcovpmin);
          zcovpclr = fmax((float) 0.0, zcovptot - za[jk_i]);              // clear sky proportion
        ...
        if (zicetot > zepsec && ztp1[jk_i] > rtt) {
          ...
          zsubsat = fmax(zqsice - zqx[4], (float) 0.0);
        ...
        if (zqx[2] > zepsec) {
          ...
          if (ztp1[jk_i] <= rtt && ztp1[jk_im1] > rtt) {
            ...
            zqpretot = fmax(zqx[3] + zqx[2], zepsec);
          ...
          if (ztp1[jk_i] < rtt) {
            ...
            if (prainfrac_toprfz[jl + klon*ibl] > (float)0.8) {
              ...
              zlambda =
                pow(((*yrecldp).rcl_fac1 / (zrho*zqx[2])), (*yrecldp).rcl_fac2);
              ...
              zfrzmax = fmax((rtt - ztp1[jk_i])*zcons1*zrldcp, (float) 0.0);

            }

            if (zfrzmax > zepsec) {
              zfrz = fmin(zqx[2], zfrzmax);
        ...
        //----------------------------------------------------------------------
        // not implicit yet...
        zfrzmax = fmax(((*yrecldp).rthomo - ztp1[jk_i])*zrldcp, (float) 0.0);
        ...
        if (ievaprain == 1) {
          ...
          zzrh = (*yrecldp).rprecrhmax + ((float) 1.0 - (*yrecldp).rprecrhmax)
            *zcovpmax / fmax(zepsec, (float) 1.0 - za[jk_i]);
          zzrh = fmin(fmax(zzrh, (*yrecldp).rprecrhmax), (float) 1.0);

          zqe = (zqx[4] - za[jk_i]*zqsliq) / fmax(zepsec, (float) 1.0 -
            za[jk_i]);
          //---------------------------------------------
          // humidity in moistest ZCOVPCLR part of domain
          //---------------------------------------------
          zqe = fmax((float) 0.0, fmin(zqe, zqsliq));
          ...
          zqe = fmax((float) 0.0, fmin(zqx[4], zqsliq));
          ...
          if (llo1) {
            ...
            zesatliq = rv / rd*((float)(r2es*exp((r3les*(ztp1[jk_i] - rtt))/(ztp1[jk_i] - r4les))));
            ...
            zevap_denom = (*yrecldp).rcl_cdenom1*zesatliq - (*yrecldp)
              ...
              .rcl_cdenom3*(pow(ztp1[jk_i], (float) 3.))*pap[jl + klon*(jk +
              klev*ibl)];

            // Temperature dependent conductivity
            zcorr2 = (pow((ztp1[jk_i] / (float) 273.), (float) 1.5))*(float)
              393. / (ztp1[jk_i] + (float) 120.);
            zka = (*yrecldp).rcl_ka273*zcorr2;

            zsubsat = fmax(zzrh*zqsliq - zqe, (float) 0.0);

            zbeta = ((float) 0.5 / zqsliq)*(pow(ztp1[jk_i], (float) 2.))
              *zesatliq*(*yrecldp).rcl_const1r*(zcorr2 / zevap_denom)*((float) 0.78 /
              (pow(zlambda, (*yrecldp).rcl_const4r)) + (*yrecldp)
              .rcl_const2r*(pow((zrho*zfallcorr), (float) 0.5)) / ((pow(zcorr2,
        ...
        if (ievapsnow == 1) {
          ...
          // humidity in moistest ZCOVPCLR part of domain
          //---------------------------------------------
          zqe = fmax((float) 0.0, fmin(zqe, zqsice));
          ...
          if (llo1) {
            ...
            zcorrfac = pow(((float)1.0 / zrho), (float)0.5);
            zcorrfac2 =
              (pow((ztp1[jk_i] / (float)273.0), (float)1.5))*((float)393.0 / (ztp1[jk_i] + (float)120.0))
              ;

            zpr02 = zrho*zpreclr*(*yrecldp).rcl_const1s / (ztcg*zfacx1s);

            zterm1 = (zqsice - zqe)*(pow(ztp1[jk_i], (float)2))
