...
void expand_1d(float *buffer, float *field_in, int nlon, int nproma, int ngptot, int nblocks)
{
  ...
  float (*field)[nproma] = (float (*)[nproma]) field_in;
...
{
  int b, l, i, buf_start_idx, buf_idx;
  float (*buffer)[nlon] = (float (*)[nlon]) buffer_in;
  float (*field)[nlev][nproma] = (float (*)[nlev][nproma]) field_in;
...
void expand_3d(float *buffer_in, float *field_in, int nlon, int nlev, int nclv, int nproma, int ngptot, int nblocks)
{
  ...
  float (*buffer)[nlev][nlon] = (float (*)[nlev][nlon]) buffer_in;
  float (*field)[nclv][nlev][nproma] = (float (*)[nclv][nlev][nproma]) field_in;
