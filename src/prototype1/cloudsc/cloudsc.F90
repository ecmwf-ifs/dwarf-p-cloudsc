...
IF( N_VMASS > 0 )THEN
  DO JL=KIDIA,KFDIA
    ZTMP1(JL-KIDIA+1)=(-0.178_JPRB)
    ZTMP2(JL-KIDIA+1)=(-0.394_JPRB)
  ENDDO
ENDIF
...
!----------------------------------------------------------------------
  ...
  DO JL=KIDIA,KFDIA
    ...
    ! liquid
    ZFACW         = R5LES/((ZTP1(JL,JK)-R4LES)**2_JPRB)
    ...
    ! ice
    ZFACI         = R5IES/((ZTP1(JL,JK)-R4IES)**2_JPRB)
  ...
  DO JL=KIDIA,KFDIA
    ...
    IF(ZDQS(JL) <= -RLMIN .AND. ZA(JL,JK)<1.0_JPRB-ZEPSEC) THEN
      ...
      IF(ZSIGK > 0.8_JPRB) THEN
        ZRHC=RAMID+(1.0_JPRB-RAMID)*((ZSIGK-0.8_JPRB)/0.2_JPRB)**2.0_JPRB
      ENDIF
  ...
  DO JL=KIDIA,KFDIA
    ...
    IF(ZTP1(JL,JK) <= RTT) THEN
      ...
      IF (ZICECLD(JL)>ZEPSEC) THEN
        ...
        ZSNOWAUT(JL)=ZZCO*(1.0_JPRB-EXP(-(ZICECLD(JL)/ZLCRIT)**2_JPRB))
...
!       &ZCOVPTOT(JL)/(MAX(ZA(JL,JK),ZEPSEC))
      ...
      IF(ZLIQCLD(JL)/ZLCRIT < 20.0_JPRB )THEN ! Security for exp for some compilers
        ZRAINAUT(JL)=ZZCO*(1.0_JPRB-EXP(-(ZLIQCLD(JL)/ZLCRIT)**2._JPRB))
      ELSE
        ZRAINAUT(JL)=ZZCO
      ENDIF
   ...
   DO JL=KIDIA,KFDIA
    ...
    IF(LLO1) THEN
     ...
     &             PAP(JL,JK)*RCL_APB3*ZTP1(JL,JK)**3._JPRB
      ...
      ZTERM1 = (ZQSICE(JL,JK)-ZQE)*ZTP1(JL,JK)**2._JPRB*ZVPICE*ZCORRFAC2*ZTCG* &
