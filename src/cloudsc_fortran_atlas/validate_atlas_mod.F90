...
CONTAINS
  ...
  SUBROUTINE VALIDATEVAR_ATLAS(FSET, FSPACE, NAME, NLON, NGPTOTG, STATE_VAR)
    ...
    IF (FRANK == 2) THEN
        CALL LOAD_AND_EXPAND(NAME, REF_R2, NLON, FSPACE%NPROMA(), NGPTOT, NBLOCKS, NGPTOTG)
    ...
    ELSE IF (FRANK == 3) THEN
        CALL LOAD_AND_EXPAND(NAME, REF_R3, NLON, FIELD%LEVELS(), FSPACE%NPROMA(), NGPTOT, NBLOCKS, NGPTOTG)
      ...
      ELSE IF (FRANK == 4 .AND. PRESENT(STATE_VAR)) THEN
        ...
        IF (STATE_VAR /= 'CLD') THEN
            ...
            IF (STATE_VAR == 'Q') THEN
                VAR_ID = 3
            ENDIF
            CALL LOAD_AND_EXPAND(NAME//'_'//STATE_VAR, REF_R3, NLON, NLEV, FSPACE%NPROMA(), NGPTOT, NBLOCKS, NGPTOTG)
            !OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(B, BSIZE) &
            !& REDUCTION(MIN:ZMINVAL, MAX:ZMAX_VAL_ERR, +:ZSUM_ERR_ABS)
            DO B=1, NBLOCKS
              BSIZE = FSPACE%BLOCK_SIZE(B)
        ...
        ELSE IF (STATE_VAR == 'CLD') THEN
          CALL LOAD_AND_EXPAND(NAME//'_CLD', REF_R4, NLON, NLEV, NDIM, FSPACE%NPROMA(), NGPTOT, NBLOCKS, NGPTOTG)
          !OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(B, BSIZE) &
          !& REDUCTION(MIN:ZMINVAL, MAX:ZMAX_VAL_ERR, +:ZSUM_ERR_ABS)
          DO B=1, NBLOCKS
            BSIZE = FSPACE%BLOCK_SIZE(B)
