...
void cloudsc_driver(int numthreads, int numcols, int nproma) {
  ...
  d_zfoealfa = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma, q);
  d_ztp1 = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma, q);
  d_zli = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma, q);
  d_za = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma, q);
  d_zaorig = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma, q);
  d_zliqfrac = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma, q);
  d_zicefrac = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma, q);
  d_zqx = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma*nclv, q);
  d_zqx0 = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma*nclv, q);
  d_zpfplsx = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma*nclv, q);
  d_zlneg = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma*nclv, q);
  d_zqxn2d = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma*nclv, q);
  d_zqsmix = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma, q);
  d_zqsliq = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma, q);
  d_zqsice = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma, q);
  d_zfoeewmt = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma, q);
  d_zfoeew = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma, q);
  d_zfoeeliqt = cl::sycl::malloc_device<float>(nblocks*(nlev+1)*nproma, q);
  //
