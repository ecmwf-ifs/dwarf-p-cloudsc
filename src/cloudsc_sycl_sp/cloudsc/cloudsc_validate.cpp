...
void print_error(const char *name, float zminval, float zmaxval, float zmaxerr,
		 float zerrsum, float zsum, float zavgpgp, int ndim)
{
  float zrelerr, zeps = std::numeric_limits<float>::epsilon();
  int iopt = 0;
  if (zerrsum < zeps) {
    zrelerr = 0.0;
    iopt = 1;
...
//    reduction(min:zminval) reduction(max:zmaxval,zmaxerr) reduction(+:zerrsum,zsum)
  ...
  print_error(name, (float)zminval, (float)zmaxval, (float)zmaxerr, (float)zerrsum, (float)zsum, (float)zavgpgp, 2);
}
    ...
    int nlev, int nclv, int ngptot, int nblocks)
{
  ...
  float zminval, zmaxval, zdiff, zmaxerr, zerrsum, zsum, zrelerr, zavgpgp;
//  float (*field)[nclv][nlev][nlon] = (float (*)[nclv][nlev][nlon]) v_field;
//  float (*reference)[nclv][nlev][nlon] = (float (*)[nclv][nlev][nlon]) v_ref;

  zminval = +std::numeric_limits<float>::max();
  zmaxval = -std::numeric_limits<float>::max();
  zmaxerr = 0.0;
  zerrsum = 0.0;
  zsum = 0.0;
