...
MODULE CLOUDSC_STATE_TYPE_MOD
  ...
  IMPLICIT NONE
    ...
    TYPE CLOUDSC_STATE_TYPE
      ...
      CLASS(FIELD_4RB), POINTER :: FIELD_STACK
...
CONTAINS
  ...
  SUBROUTINE STATE_TYPE_INIT(SELF,NPROMA, NGPTOT, KLON, KLEV, KFLDX, NBLOCKS, NGPTOTG, USE_PACKED)
    ...
    IF (SELF%PACKED) THEN
      CALL FIELD_NEW(SELF%FIELD_STACK, LSTACK=.TRUE., UBOUNDS=[NPROMA,KLEV,NPACKED_FIELDS,NBLOCKS], &
        & PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)

      CALL GET_STACK_MEMBER(SELF%FIELD_STACK, 1, SELF%F_T)
      CALL GET_STACK_MEMBER(SELF%FIELD_STACK, 2, SELF%F_A)
      CALL GET_STACK_MEMBER(SELF%FIELD_STACK, 3, SELF%F_Q)
    ELSE
  ...
  SUBROUTINE STATE_TYPE_FINAL(SELF)
    CLASS(CLOUDSC_STATE_TYPE) :: SELF
    IF (SELF%PACKED) THEN
     CALL FIELD_DELETE(SELF%FIELD_STACK)
    ELSE
