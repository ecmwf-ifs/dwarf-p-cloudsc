...
!$omp end parallel do
  ...
  SUBROUTINE CLOUDSC_FIELD_STATE_LOAD(SELF, NPROMA, NGPTOT, NGPTOTG, USE_PACKED, USE_PINNED)
    ...
    LOGICAL, INTENT(IN), OPTIONAL :: USE_PACKED, USE_PINNED

    INTEGER(KIND=JPIM) :: KLON, NFIELDS
    INTEGER(KIND=JPIM) :: START, END, SIZE

    LOGICAL :: LLPACKED, LLPINNED

    LLPACKED = .FALSE.
    IF (PRESENT(USE_PACKED)) LLPACKED = USE_PACKED
    LLPINNED = .FALSE.
    IF (PRESENT(USE_PINNED)) LLPINNED = USE_PINNED

    ! Set this flag to enable pinning of fields in page-locked memory
    IF(USE_PINNED) INIT_PINNED_VALUE = .TRUE.
