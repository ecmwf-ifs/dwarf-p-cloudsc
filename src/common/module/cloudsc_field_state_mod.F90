...
MODULE CLOUDSC_FIELD_STATE_MOD
  ...
  USE FIELD_MODULE, ONLY: FIELD_1RB, FIELD_2RB, FIELD_3RB, FIELD_4RB, FIELD_1IM, FIELD_2IM, FIELD_1LM, FIELD_2LM
  USE FIELD_FACTORY_MODULE, ONLY: FIELD_NEW
  USE FIELD_HDF5_MODULE, ONLY: READ_HDF5_PERRANK_DATA, WRITE_HDF5_PERRANK_DATA
  ...
  CONTAINS
    ...
    PROCEDURE :: WRITE_OUTPUT => CLOUDSC_FIELD_STATE_WRITE_OUTPUT
...
CONTAINS
  ...
  SUBROUTINE LOAD_AND_EXPAND_FIELD_2D(NAME, FIELD, NLON, NPROMA, NGPTOT, NBLOCKS, NGPTOTG)
    ...
    CLASS(FIELD_1RB), POINTER :: BUFFER
    INTEGER(KIND=JPIM) :: START, END, SIZE

    CALL GET_OFFSETS(START, END, SIZE, NLON, 1, 1, NGPTOT, NGPTOTG)
    CALL FIELD_NEW(BUFFER, UBOUNDS=[NGPTOT], PERSISTENT=.TRUE.)
    CALL READ_HDF5_PERRANK_DATA(BUFFER, "input", NAME, HDFEXISTS=.TRUE.)
    CALL EXPAND(BUFFER%PTR, FIELD%PTR, SIZE, NPROMA, NGPTOT, NBLOCKS)
  ...
  SUBROUTINE LOAD_AND_EXPAND_FIELD_INT2D(NAME, FIELD, NLON, NPROMA, NGPTOT, NBLOCKS, NGPTOTG)
    ...
    INTEGER(KIND=JPIM), INTENT(IN) :: NLON, NPROMA, NGPTOT, NBLOCKS
    INTEGER(KIND=JPIM), INTENT(IN), OPTIONAL :: NGPTOTG
    CLASS(FIELD_1IM), POINTER :: BUFFER
    INTEGER(KIND=JPIM) :: START, END, SIZE
  ...
  SUBROUTINE LOAD_AND_EXPAND_FIELD_LOG2D(NAME, FIELD, NLON, NPROMA, NGPTOT, NBLOCKS, NGPTOTG)
    ...
    INTEGER(KIND=JPIM), INTENT(IN) :: NLON, NPROMA, NGPTOT, NBLOCKS
    INTEGER(KIND=JPIM), INTENT(IN), OPTIONAL :: NGPTOTG
    CLASS(FIELD_1LM), POINTER :: BUFFER
    INTEGER(KIND=JPIM) :: START, END, SIZE
  ...
  SUBROUTINE LOAD_AND_EXPAND_FIELD_3D(NAME, FIELD, NLON, NLEV, NPROMA, NGPTOT, NBLOCKS, NGPTOTG)
    ...
    INTEGER(KIND=JPIM), INTENT(IN), OPTIONAL :: NGPTOTG
    CLASS(FIELD_2RB), POINTER :: BUFFER
    INTEGER(KIND=JPIM) :: START, END, SIZE

    CALL GET_OFFSETS(START, END, SIZE, NLON, 1, NLEV, NGPTOT, NGPTOTG)
    CALL FIELD_NEW(BUFFER, UBOUNDS=[NLEV,NGPTOT], PERSISTENT=.TRUE.)
    CALL READ_HDF5_PERRANK_DATA(BUFFER, "input", NAME, HDFEXISTS=.TRUE.)
    CALL EXPAND(BUFFER%PTR, FIELD%PTR, SIZE, NPROMA, NLEV, NGPTOT, NBLOCKS)
  ...
  SUBROUTINE LOAD_AND_EXPAND_FIELD_4D(NAME, FIELD, NLON, NLEV, NDIM, NPROMA, NGPTOT, NBLOCKS, NGPTOTG)
    ...
    INTEGER(KIND=JPIM), INTENT(IN), OPTIONAL :: NGPTOTG
    CLASS(FIELD_3RB), POINTER :: BUFFER
    INTEGER(KIND=JPIM) :: START, END, SIZE

    CALL GET_OFFSETS(START, END, SIZE, NLON, NDIM, NLEV, NGPTOT, NGPTOTG)
    CALL FIELD_NEW(BUFFER, UBOUNDS=[NLEV,NDIM,NGPTOT], PERSISTENT=.TRUE.)
    CALL READ_HDF5_PERRANK_DATA(BUFFER, "input", NAME, HDFEXISTS=.TRUE.)
    CALL EXPAND(BUFFER%PTR, FIELD%PTR, SIZE, NPROMA, NLEV, NDIM, NGPTOT, NBLOCKS)
...
!$omp end parallel do
  ...
  SUBROUTINE CLOUDSC_FIELD_STATE_LOAD(SELF, NPROMA, NGPTOT, NGPTOTG, USE_PACKED)
    ...
    ! Now load fields via Field-API HDF5 IO module
  ...
  SUBROUTINE CLOUDSC_FIELD_STATE_WRITE_OUTPUT(SELF)
    CLASS(CLOUDSC_FIELD_STATE) :: SELF

    CALL WRITE_HDF5_PERRANK_DATA(SELF%AUX%F_PLUDE, 'output', "PLUDE", HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%AUX%F_PCOVPTOT, 'output', 'PCOVPTOT', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%AUX%F_PRAINFRAC_TOPRFZ, 'output', 'PRAINFRAC_TOPRFZ', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%FLUX%F_PFSQLF, 'output', 'PFSQLF', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%FLUX%F_PFSQIF, 'output', 'PFSQIF', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%FLUX%F_PFCQLNG, 'output', 'PFCQLNG', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%FLUX%F_PFCQNNG, 'output', 'PFCQNNG', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%FLUX%F_PFSQRF, 'output', 'PFSQRF', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%FLUX%F_PFSQSF, 'output', 'PFSQSF', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%FLUX%F_PFCQRNG, 'output', 'PFCQRNG', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%FLUX%F_PFCQSNG, 'output', 'PFCQSNG', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%FLUX%F_PFSQLTUR, 'output', 'PFSQLTUR', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%FLUX%F_PFSQITUR, 'output', 'PFSQITUR', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%FLUX%F_PFPLSL, 'output', 'PFPLSL', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%FLUX%F_PFPLSN, 'output', 'PFPLSN', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%FLUX%F_PFHPSL, 'output', 'PFHPSL', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%FLUX%F_PFHPSN, 'output', 'PFHPSN', HDFEXISTS=.TRUE.)

    CALL WRITE_HDF5_PERRANK_DATA(SELF%TENDENCY_LOC%F_A, 'output', 'TENDENCY_LOC_A', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%TENDENCY_LOC%F_Q, 'output', 'TENDENCY_LOC_Q', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%TENDENCY_LOC%F_T, 'output', 'TENDENCY_LOC_T', HDFEXISTS=.TRUE.)
    CALL WRITE_HDF5_PERRANK_DATA(SELF%TENDENCY_LOC%F_CLD, 'output', 'TENDENCY_LOC_CLD', HDFEXISTS=.TRUE.)
  END SUBROUTINE CLOUDSC_FIELD_STATE_WRITE_OUTPUT
