...
1002  format(1x,5i10,1x,i4,' : ',2i10,  ' : TOTAL')
      ...
      DO TID=0, SELF%NUMOMP-1
         ...
         ELSE
            ZMFLOPS = 0.0_JPRB
         ENDIF
         WRITE(0,1001) SELF%NUMOMP, NGPTOT, SELF%IGPC(TID), SELF%ICALLS(TID), NPROMA, TID, &
              & INT(TLOC*1000.0_JPRB), INT(ZMFLOPS), SELF%COREID(TID)
      ENDDO
      ...
      ELSE
         ZMFLOPS = 0.0_JPRB
      ENDIF
      WRITE(0,1002) SELF%NUMOMP, NGPTOT, INT(SUM(SELF%IGPC)), NGPBLKS, NPROMA, -1,&
