...
CONTAINS
  ...
  SUBROUTINE VALIDATE_R2_K(NAME, REF, FIELD, NLON, NLEV, NGPTOT, NBLOCKS, NGPTOTG, k)
    ! Computes and prints errors "in the L1 norm sense"
    CHARACTER(*), INTENT(IN) :: NAME
    REAL(KIND=JPRB), INTENT(INOUT) :: REF(:,:,:,:), FIELD(:,:,:,:)
    INTEGER(KIND=JPIM), INTENT(IN) :: NLON, NLEV, NBLOCKS, NGPTOT, k
    INTEGER(KIND=JPIM), INTENT(IN), OPTIONAL :: NGPTOTG
    ...
    DO B=1, NBLOCKS
      BSIZE = MIN(NLON, NGPTOT - (B-1)*NLON)  ! Field block size
      ZMINVAL(1) = MIN(ZMINVAL(1),MINVAL(FIELD(:,:,k,B)))
      ZMAX_VAL_ERR(1) = MAX(ZMAX_VAL_ERR(1),MAXVAL(FIELD(:,:,k,B)))
      DO JL=1, NLEV
        DO JK=1, bsize
          ! Difference against reference result in one-norm sense
          ZDIFF = ABS(FIELD(JK,JL,k,B) - REF(JK,JL,k,B))
          ZMAX_VAL_ERR(2) = MAX(ZMAX_VAL_ERR(2),ZDIFF)
          ZSUM_ERR_ABS(1) = ZSUM_ERR_ABS(1) + ZDIFF
          ZSUM_ERR_ABS(2) = ZSUM_ERR_ABS(2) + ABS(REF(JK,JL,k,B))
        ENDDO
      END DO
    END DO
  ...
  SUBROUTINE VALIDATE_R3_K(NAME, REF, FIELD, NLON, NLEV, NDIM, NGPTOT, NBLOCKS, NGPTOTG, k)
    ! Computes and prints errors "in the L1 norm sense"
    CHARACTER(*), INTENT(IN) :: NAME
    REAL(KIND=JPRB), INTENT(INOUT) :: REF(:,:,:,:), FIELD(:,:,:,:)
    INTEGER(KIND=JPIM), INTENT(IN) :: NLON, NLEV, NDIM, NBLOCKS, NGPTOT, k
    INTEGER(KIND=JPIM), INTENT(IN), OPTIONAL :: NGPTOTG
    ...
    DO B=1, NBLOCKS
      BSIZE = MIN(NLON, NGPTOT - (B-1)*NLON)  ! Field block size
      ZMINVAL(1) = MIN(ZMINVAL(1),MINVAL(FIELD(:,:,k+1:,B)))
      ZMAX_VAL_ERR(1) = MAX(ZMAX_VAL_ERR(1),MAXVAL(FIELD(:,:,k+1:,B)))
      DO JM=1, NDIM
        DO JL=1, NLEV
          DO JK=1, bsize
            ! Difference against reference result in one-norm sense
            ZDIFF = ABS(FIELD(JK,JL,k+JM,B) - REF(JK,JL,k+JM,B))
            ZMAX_VAL_ERR(2) = MAX(ZMAX_VAL_ERR(2),ZDIFF)
            ZSUM_ERR_ABS(1) = ZSUM_ERR_ABS(1) + ZDIFF
            ZSUM_ERR_ABS(2) = ZSUM_ERR_ABS(2) + ABS(REF(JK,JL,k+JM,B))
          END DO
        END DO
      END DO
    END DO
