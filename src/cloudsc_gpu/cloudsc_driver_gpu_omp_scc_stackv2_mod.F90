...
MODULE CLOUDSC_DRIVER_GPU_OMP_SCC_STACKV2_MOD
  ...
  USE CLOUDSC_GPU_OMP_SCC_STACKV2_MOD, ONLY: CLOUDSC_SCC_STACKV2

  IMPLICIT NONE
  
  CONTAINS
  
  SUBROUTINE CLOUDSC_DRIVER_GPU_SCC_STACKV2 (NUMOMP, NPROMA, NLEV, NGPTOT, NGPTOTG, NGPBLKS, KFLDX, PTSPHY, PT, PQ, TENDENCY_CML, TENDENCY_TMP,  &
...
!$omp parallel do
      DO JL=1,ICEND
        CALL CLOUDSC_SCC_STACKV2(1, ICEND, NPROMA, NLEV, PTSPHY, PT(:, :, IBL), PQ(:, :, IBL), BUFFER_TMP(:, :, 1, IBL),  &
...
!$omp end target data
    DEALLOCATE (ZSTACK)
  END SUBROUTINE CLOUDSC_DRIVER_GPU_SCC_STACKV2
  
END MODULE CLOUDSC_DRIVER_GPU_OMP_SCC_STACKV2_MOD
