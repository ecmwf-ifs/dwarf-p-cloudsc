...
MODULE CLOUDSC_DRIVER_GPU_OMP_SCC_STACKV3_MOD
  ...
  USE CLOUDSC_GPU_OMP_SCC_STACKV3_MOD, ONLY: CLOUDSC_SCC_STACKV3

  IMPLICIT NONE
  
  CONTAINS
  
  SUBROUTINE CLOUDSC_DRIVER_GPU_SCC_STACKV3 (NUMOMP, NPROMA, NLEV, NGPTOT, NGPTOTG, NGPBLKS, KFLDX, PTSPHY, PT, PQ, TENDENCY_CML, TENDENCY_TMP,  &
  ...
  & PFPLSN, PFHPSL, PFHPSN)
    ...
    USE, intrinsic :: ISO_C_BINDING, ONLY: C_SIZEOF, C_DOUBLE, C_INT64_T, C_PTR
    ...
    ! INTEGER(KIND=C_INT64_T), ALLOCATABLE :: ZSTACK(:, :)

    INTEGER(KIND=8) :: YLSTACK_L
    ISTSZ = NPROMA + 13*NLEV*NPROMA + 5*NLEV*NPROMA*NCLV + NPROMA*NCLV
    ALLOCATE (ZSTACK(ISTSZ, NGPBLKS))
!$omp target data map( alloc: ZSTACK )
...
!$omp target data map( to: pt, pq, buffer_tmp, pvfa, pvfl, pvfi, pdyna, pdynl, pdyni, phrsw, phrlw, pvervel, pap, paph, plsm,  &
!$omp & ldcum, ktype, plu, psnde, pmfu, pmfd, pa, pclv, psupsat, plcrit_aer, picrit_aer, pre_ice, pccn, pnice ) map( tofrom:  &
!$omp & buffer_loc, plude ) map( from: pcovptot, prainfrac_toprfz, pfsqlf, pfsqif, pfcqnng, pfcqlng, pfsqrf, pfsqsf, pfcqrng,  &
!$omp & pfcqsng, pfsqltur, pfsqitur, pfplsl, pfplsn, pfhpsl, pfhpsn )
...
!$omp target teams distribute private( YLSTACK_L ) thread_limit(128)
...
!$omp parallel do
      DO JL=1,ICEND
        CALL CLOUDSC_SCC_STACKV3(1, ICEND, NPROMA, NLEV, PTSPHY, PT(:, :, IBL), PQ(:, :, IBL), BUFFER_TMP(:, :, 1, IBL),  &
        ...
        & PSUPSAT(:, :, IBL), PLCRIT_AER(:, :, IBL), PICRIT_AER(:, :, IBL), PRE_ICE(:, :, IBL), PCCN(:, :, IBL),  &
        & PNICE(:, :, IBL), PCOVPTOT(:, :, IBL), PRAINFRAC_TOPRFZ(:, IBL), PFSQLF(:, :, IBL), PFSQIF(:, :, IBL),  &
        & PFCQNNG(:, :, IBL), PFCQLNG(:, :, IBL), PFSQRF(:, :, IBL), PFSQSF(:, :, IBL), PFCQRNG(:, :, IBL), PFCQSNG(:, :, IBL),  &
        & PFSQLTUR(:, :, IBL), PFSQITUR(:, :, IBL), PFPLSL(:, :, IBL), PFPLSN(:, :, IBL), PFHPSL(:, :, IBL), PFHPSN(:, :, IBL),  &
        & LOCAL_YRECLDP, JL=JL, YDSTACK_L=YLSTACK_L, ZSTACK=ZSTACK(:, IBL))
      END DO
!$omp end parallel do      
      !---prognostic fields
...
!$omp end target data
    DEALLOCATE (ZSTACK)
  END SUBROUTINE CLOUDSC_DRIVER_GPU_SCC_STACKV3
  
END MODULE CLOUDSC_DRIVER_GPU_OMP_SCC_STACKV3_MOD
