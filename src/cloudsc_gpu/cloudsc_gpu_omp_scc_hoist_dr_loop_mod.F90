...
MODULE CLOUDSC_HOIST_DR_LOOP_MOD
  
  CONTAINS
  
  SUBROUTINE CLOUDSC_HOIST_DR_LOOP (KIDIA, KFDIA, KLON, KLEV, PTSPHY, PT, PQ, TENDENCY_TMP, TENDENCY_LOC,  &
  & PVFA, PVFL, PVFI, PDYNA, PDYNL, PDYNI, PHRSW, PHRLW, PVERVEL, PAP, PAPH, PLSM, LDCUM, KTYPE, PLU, PLUDE, PSNDE, PMFU, PMFD,  &
  & PA, PCLV, PSUPSAT, PLCRIT_AER, PICRIT_AER, PRE_ICE, PCCN, PNICE, PCOVPTOT, PRAINFRAC_TOPRFZ, PFSQLF, PFSQIF, PFCQNNG,  &
  & PFCQLNG, PFSQRF, PFSQSF, PFCQRNG, PFCQSNG, PFSQLTUR, PFSQITUR, PFPLSL, PFPLSN, PFHPSL, PFHPSN, YRECLDP, NGPBLKS, ZFOEALFA,  &
  & ZTP1, ZLI, ZA, ZAORIG, ZLIQFRAC, ZICEFRAC, ZQX, ZQX0, ZPFPLSX, ZLNEG, ZQXN2D, ZQSMIX, ZQSLIQ, ZQSICE, ZFOEEWMT, ZFOEEW,  &
  & ZFOEELIQT, NGPTOT, NPROMA)
    !---input
    !---prognostic fields
    ...
    INTEGER(KIND=JPIM), VALUE, INTENT(IN) :: KFDIA, NGPTOT, NPROMA
    ...
    ! TYPE(TECLDP), INTENT(INOUT) :: LOCAL_YRECLDP
    ...
    REAL(KIND=JPRB), INTENT(INOUT) :: ZFOEALFA(KLON, KLEV + 1, NGPBLKS)
    ...
    REAL(KIND=JPRB), INTENT(INOUT) :: ZTP1(KLON, KLEV, NGPBLKS)
    ...
    REAL(KIND=JPRB) :: ZDA
    REAL(KIND=JPRB), INTENT(INOUT) :: ZLI(KLON, KLEV, NGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZA(KLON, KLEV, NGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZAORIG(KLON, KLEV, NGPBLKS)    ! start of scheme value for CC
    ...
    REAL(KIND=JPRB), INTENT(INOUT) :: ZLIQFRAC(KLON, KLEV, NGPBLKS)    ! cloud liquid water fraction: ql/(ql+qi)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZICEFRAC(KLON, KLEV, NGPBLKS)    ! cloud ice water fraction: qi/(ql+qi)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZQX(KLON, KLEV, NCLV, NGPBLKS)    ! water variables
    REAL(KIND=JPRB), INTENT(INOUT) :: ZQX0(KLON, KLEV, NCLV, NGPBLKS)    ! water variables at start of scheme
    ...
    REAL(KIND=JPRB), INTENT(INOUT) :: ZPFPLSX(KLON, KLEV + 1, NCLV, NGPBLKS)    ! generalized precipitation flux
    REAL(KIND=JPRB), INTENT(INOUT) :: ZLNEG(KLON, KLEV, NCLV, NGPBLKS)    ! for negative correction diagnostics
    REAL(KIND=JPRB) :: ZMELTMAX
    REAL(KIND=JPRB) :: ZFRZMAX
    REAL(KIND=JPRB) :: ZICETOT
    
    REAL(KIND=JPRB), INTENT(INOUT) :: ZQXN2D(KLON, KLEV, NCLV, NGPBLKS)    ! water variables store
    
    REAL(KIND=JPRB), INTENT(INOUT) :: ZQSMIX(KLON, KLEV, NGPBLKS)    ! diagnostic mixed phase saturation
    !REAL(KIND=JPRB) :: ZQSBIN(KLON,KLEV) ! binary switched ice/liq saturation
    REAL(KIND=JPRB), INTENT(INOUT) :: ZQSLIQ(KLON, KLEV, NGPBLKS)    ! liquid water saturation
    REAL(KIND=JPRB), INTENT(INOUT) :: ZQSICE(KLON, KLEV, NGPBLKS)    ! ice water saturation
    ...
    REAL(KIND=JPRB), INTENT(INOUT) :: ZFOEEWMT(KLON, KLEV, NGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZFOEEW(KLON, KLEV, NGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZFOEELIQT(KLON, KLEV, NGPBLKS)
    ...
    INTEGER(KIND=JPIM) :: IBL, JKGLO, ICEND
...
#include "fccld.func.h"
    ...
    ! $omp map(tofrom: &
    ! $omp   tendency_loc,plude,pcovptot,prainfrac_toprfz) &
    ! $omp map(from: &
    ...
    ! $acc data &
    ! $acc copyin(pt,pq,tendency_tmp,pvfa, &
    ...
    ! $acc copy( &
    ! $acc   tendency_loc,plude,pcovptot,prainfrac_toprfz) &
    ! $acc copyout( &
    ...
    ! $acc parallel loop gang vector_length(64) private(ZLCUST, IPHASE, IMELT, LLFALL, LLINDEX1, LLINDEX3, IORDER, ZQXN, ZQXFG, ZQXNM1, ZFLUXQ, ZSOLQA, ZSOLQB, ZQLHS, ZVQX, ZRATIO, ZSINKSUM, ZFALLSINK, ZFALLSRCE, ZCONVSRCE, ZCONVSINK, ZPSUPSATSRCE)
    DO JKGLO=1,NGPTOT,NPROMA
      ...
      ! if (IBL .eq. 1 .and. JL .eq. 1) THEN
      !        print *, "YRECLDP%NCLDTOP = ", YRECLDP%NCLDTOP
      ! endif

      IF (YRECLDP%NCLDTOP .eq. 15) THEN 
      ...
      ! $acc loop seq
      DO JK=1,KLEV
        ZTP1(JL, JK, IBL) = PT(JL, JK, IBL) + PTSPHY*TENDENCY_TMP(JL, JK, 1, IBL)
        ZQX(JL, JK, NCLDQV, IBL) = PQ(JL, JK, IBL) + PTSPHY*TENDENCY_TMP(JL, JK, 3, IBL)
        ZQX0(JL, JK, NCLDQV, IBL) = PQ(JL, JK, IBL) + PTSPHY*TENDENCY_TMP(JL, JK, 3, IBL)
        ZA(JL, JK, IBL) = PA(JL, JK, IBL) + PTSPHY*TENDENCY_TMP(JL, JK, 2, IBL)
        ZAORIG(JL, JK, IBL) = PA(JL, JK, IBL) + PTSPHY*TENDENCY_TMP(JL, JK, 2, IBL)
      END DO
      ...
      DO JM=1,NCLV - 1
        DO JK=1,KLEV
          ZQX(JL, JK, JM, IBL) = PCLV(JL, JK, JM, IBL) + PTSPHY*TENDENCY_TMP(JL, JK, 3 + JM, IBL)
          ZQX0(JL, JK, JM, IBL) = PCLV(JL, JK, JM, IBL) + PTSPHY*TENDENCY_TMP(JL, JK, 3 + JM, IBL)
        END DO
      END DO
      ...
      DO JM=1,NCLV
        DO JK=1,KLEV + 1
          ZPFPLSX(JL, JK, JM, IBL) = 0.0_JPRB            ! precip fluxes
        END DO
      END DO
      ...
      DO JM=1,NCLV
        DO JK=1,KLEV
          ZQXN2D(JL, JK, JM, IBL) = 0.0_JPRB            ! end of timestep values in 2D
          ZLNEG(JL, JK, JM, IBL) = 0.0_JPRB            ! negative input check
        END DO
      END DO
      ...
      ! $acc loop seq
      DO JK=1,KLEV
        IF (ZQX(JL, JK, NCLDQL, IBL) + ZQX(JL, JK, NCLDQI, IBL) < YRECLDP%RLMIN .or. ZA(JL, JK, IBL) < YRECLDP%RAMIN) THEN
          
          ! Evaporate small cloud liquid water amounts
          ZLNEG(JL, JK, NCLDQL, IBL) = ZLNEG(JL, JK, NCLDQL, IBL) + ZQX(JL, JK, NCLDQL, IBL)
          ZQADJ = ZQX(JL, JK, NCLDQL, IBL)*ZQTMST
          TENDENCY_LOC(JL, JK, 3, IBL) = TENDENCY_LOC(JL, JK, 3, IBL) + ZQADJ
          TENDENCY_LOC(JL, JK, 1, IBL) = TENDENCY_LOC(JL, JK, 1, IBL) - RALVDCP*ZQADJ
          ZQX(JL, JK, NCLDQV, IBL) = ZQX(JL, JK, NCLDQV, IBL) + ZQX(JL, JK, NCLDQL, IBL)
          ZQX(JL, JK, NCLDQL, IBL) = 0.0_JPRB
          
          ! Evaporate small cloud ice water amounts
          ZLNEG(JL, JK, NCLDQI, IBL) = ZLNEG(JL, JK, NCLDQI, IBL) + ZQX(JL, JK, NCLDQI, IBL)
          ZQADJ = ZQX(JL, JK, NCLDQI, IBL)*ZQTMST
          TENDENCY_LOC(JL, JK, 3, IBL) = TENDENCY_LOC(JL, JK, 3, IBL) + ZQADJ
          TENDENCY_LOC(JL, JK, 1, IBL) = TENDENCY_LOC(JL, JK, 1, IBL) - RALSDCP*ZQADJ
          ZQX(JL, JK, NCLDQV, IBL) = ZQX(JL, JK, NCLDQV, IBL) + ZQX(JL, JK, NCLDQI, IBL)
          ZQX(JL, JK, NCLDQI, IBL) = 0.0_JPRB
          
          ! Set cloud cover to zero
          ZA(JL, JK, IBL) = 0.0_JPRB
      ...
      DO JM=1,NCLV - 1
        ...
        DO JK=1,KLEV
          !DIR$ IVDEP
          IF (ZQX(JL, JK, JM, IBL) < YRECLDP%RLMIN) THEN
            ZLNEG(JL, JK, JM, IBL) = ZLNEG(JL, JK, JM, IBL) + ZQX(JL, JK, JM, IBL)
            ZQADJ = ZQX(JL, JK, JM, IBL)*ZQTMST
            TENDENCY_LOC(JL, JK, 3, IBL) = TENDENCY_LOC(JL, JK, 3, IBL) + ZQADJ
            ...
            ZQX(JL, JK, NCLDQV, IBL) = ZQX(JL, JK, NCLDQV, IBL) + ZQX(JL, JK, JM, IBL)
            ZQX(JL, JK, JM, IBL) = 0.0_JPRB
          END IF
        END DO
      END DO
      ...
      DO JK=1,KLEV
        ...
        !----------------------------------------
        ZFOEALFA(JL, JK, IBL) = FOEALFA(ZTP1(JL, JK, IBL))
        ZFOEEWMT(JL, JK, IBL) = MIN(FOEEWM(ZTP1(JL, JK, IBL)) / PAP(JL, JK, IBL), 0.5_JPRB)
        ZQSMIX(JL, JK, IBL) = ZFOEEWMT(JL, JK, IBL)
        ZQSMIX(JL, JK, IBL) = ZQSMIX(JL, JK, IBL) / (1.0_JPRB - RETV*ZQSMIX(JL, JK, IBL))
        
        !---------------------------------------------
        ! ice saturation T<273K
        ! liquid water saturation for T>273K
        !---------------------------------------------
        ZALFA = FOEDELTA(ZTP1(JL, JK, IBL))
        ZFOEEW(JL, JK, IBL) =  &
        & MIN((ZALFA*FOEELIQ(ZTP1(JL, JK, IBL)) + (1.0_JPRB - ZALFA)*FOEEICE(ZTP1(JL, JK, IBL))) / PAP(JL, JK, IBL), 0.5_JPRB)
        ZFOEEW(JL, JK, IBL) = MIN(0.5_JPRB, ZFOEEW(JL, JK, IBL))
        ZQSICE(JL, JK, IBL) = ZFOEEW(JL, JK, IBL) / (1.0_JPRB - RETV*ZFOEEW(JL, JK, IBL))
        
        !----------------------------------
        ! liquid water saturation
        !----------------------------------
        ZFOEELIQT(JL, JK, IBL) = MIN(FOEELIQ(ZTP1(JL, JK, IBL)) / PAP(JL, JK, IBL), 0.5_JPRB)
        ZQSLIQ(JL, JK, IBL) = ZFOEELIQT(JL, JK, IBL)
        ZQSLIQ(JL, JK, IBL) = ZQSLIQ(JL, JK, IBL) / (1.0_JPRB - RETV*ZQSLIQ(JL, JK, IBL))
      ...
      DO JK=1,KLEV
        ...
        !------------------------------------------
        ZA(JL, JK, IBL) = MAX(0.0_JPRB, MIN(1.0_JPRB, ZA(JL, JK, IBL)))
        
        !-------------------------------------------------------------------
        ! Calculate liq/ice fractions (no longer a diagnostic relationship)
        !-------------------------------------------------------------------
        ZLI(JL, JK, IBL) = ZQX(JL, JK, NCLDQL, IBL) + ZQX(JL, JK, NCLDQI, IBL)
        IF (ZLI(JL, JK, IBL) > YRECLDP%RLMIN) THEN
          ZLIQFRAC(JL, JK, IBL) = ZQX(JL, JK, NCLDQL, IBL) / ZLI(JL, JK, IBL)
          ZICEFRAC(JL, JK, IBL) = 1.0_JPRB - ZLIQFRAC(JL, JK, IBL)
        ELSE
          ZLIQFRAC(JL, JK, IBL) = 0.0_JPRB
          ZICEFRAC(JL, JK, IBL) = 0.0_JPRB
        END IF
      ...
      DO JK=1,KLEV - 1
        ...
        IF (ZSIG > 0.1_JPRB .and. ZSIG < 0.4_JPRB .and. ZTP1(JL, JK, IBL) > ZTP1(JL, JK + 1, IBL)) THEN
          ZTRPAUS = ZSIG
        END IF
      END DO
      ...
      DO JK=YRECLDP%NCLDTOP,KLEV
        ...
        ZGDP = RG / ZDP          ! g/dp
        ZRHO = PAP(JL, JK, IBL) / (RD*ZTP1(JL, JK, IBL))          ! p/RT air density
        ...
        ! liquid
        ZFACW = R5LES / ((ZTP1(JL, JK, IBL) - R4LES)**2)
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZFOEELIQT(JL, JK, IBL))
        ZDQSLIQDT = ZFACW*ZCOR*ZQSLIQ(JL, JK, IBL)
        ZCORQSLIQ = 1.0_JPRB + RALVDCP*ZDQSLIQDT
        
        ! ice
        ZFACI = R5IES / ((ZTP1(JL, JK, IBL) - R4IES)**2)
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZFOEEW(JL, JK, IBL))
        ZDQSICEDT = ZFACI*ZCOR*ZQSICE(JL, JK, IBL)
        ZCORQSICE = 1.0_JPRB + RALSDCP*ZDQSICEDT
        
        ! diagnostic mixed
        ZALFAW = ZFOEALFA(JL, JK, IBL)
        ZALFAWM = ZALFAW
        ZFAC = ZALFAW*ZFACW + (1.0_JPRB - ZALFAW)*ZFACI
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZFOEEWMT(JL, JK, IBL))
        ZDQSMIXDT = ZFAC*ZCOR*ZQSMIX(JL, JK, IBL)
        ZCORQSMIX = 1.0_JPRB + FOELDCPM(ZTP1(JL, JK, IBL))*ZDQSMIXDT
        
        ! evaporation/sublimation limits
        ZEVAPLIMMIX = MAX((ZQSMIX(JL, JK, IBL) - ZQX(JL, JK, NCLDQV, IBL)) / ZCORQSMIX, 0.0_JPRB)
        ZEVAPLIMLIQ = MAX((ZQSLIQ(JL, JK, IBL) - ZQX(JL, JK, NCLDQV, IBL)) / ZCORQSLIQ, 0.0_JPRB)
        ZEVAPLIMICE = MAX((ZQSICE(JL, JK, IBL) - ZQX(JL, JK, NCLDQV, IBL)) / ZCORQSICE, 0.0_JPRB)
        
        !--------------------------------
        ! in-cloud consensate amount
        !--------------------------------
        ZTMPA = 1.0_JPRB / MAX(ZA(JL, JK, IBL), ZEPSEC)
        ZLIQCLD = ZQX(JL, JK, NCLDQL, IBL)*ZTMPA
        ZICECLD = ZQX(JL, JK, NCLDQI, IBL)*ZTMPA
        ...
        IF (ZQX(JL, JK, NCLDQL, IBL) < YRECLDP%RLMIN) THEN
          ZSOLQA(NCLDQV, NCLDQL) = ZQX(JL, JK, NCLDQL, IBL)
          ZSOLQA(NCLDQL, NCLDQV) = -ZQX(JL, JK, NCLDQL, IBL)
        END IF
        
        IF (ZQX(JL, JK, NCLDQI, IBL) < YRECLDP%RLMIN) THEN
          ZSOLQA(NCLDQV, NCLDQI) = ZQX(JL, JK, NCLDQI, IBL)
          ZSOLQA(NCLDQI, NCLDQV) = -ZQX(JL, JK, NCLDQI, IBL)
        END IF
        ...
        ZFOKOOP = FOKOOP(ZTP1(JL, JK, IBL))
        
        IF (ZTP1(JL, JK, IBL) >= RTT .or. YRECLDP%NSSOPT == 0) THEN
          ZFAC = 1.0_JPRB
          ZFACI = 1.0_JPRB
        ELSE
          ZFAC = ZA(JL, JK, IBL) + ZFOKOOP*(1.0_JPRB - ZA(JL, JK, IBL))
        ...
        IF (ZA(JL, JK, IBL) > 1.0_JPRB - YRECLDP%RAMIN) THEN
          ZSUPSAT = MAX((ZQX(JL, JK, NCLDQV, IBL) - ZFAC*ZQSICE(JL, JK, IBL)) / ZCORQSICE, 0.0_JPRB)
        ELSE
          ! Calculate environmental humidity supersaturation
          ZQP1ENV = (ZQX(JL, JK, NCLDQV, IBL) - ZA(JL, JK, IBL)*ZQSICE(JL, JK, IBL)) / MAX(1.0_JPRB - ZA(JL, JK, IBL), ZEPSILON)
          !& SIGN(MAX(ABS(1.0_JPRB-ZA(JL,JK)),ZEPSILON),1.0_JPRB-ZA(JL,JK))
          ZSUPSAT = MAX((1.0_JPRB - ZA(JL, JK, IBL))*(ZQP1ENV - ZFAC*ZQSICE(JL, JK, IBL)) / ZCORQSICE, 0.0_JPRB)
        END IF
        ...
        IF (ZSUPSAT > ZEPSEC) THEN
          ...
          IF (ZTP1(JL, JK, IBL) > YRECLDP%RTHOMO) THEN
          ...
          ZSOLAC = (1.0_JPRB - ZA(JL, JK, IBL))*ZFACI
        ...
        IF (JK > YRECLDP%NCLDTOP) THEN
          ...
          ZDTDP = ZRDCP*0.5_JPRB*(ZTP1(JL, JK - 1, IBL) + ZTP1(JL, JK, IBL)) / PAPH(JL, JK, IBL)
        ...
        IF (ZLI(JL, JK, IBL) > ZEPSEC) THEN
          ...
          ZE = ZLDIFDT*MAX(ZQSMIX(JL, JK, IBL) - ZQX(JL, JK, NCLDQV, IBL), 0.0_JPRB)
          ZLEROS = ZA(JL, JK, IBL)*ZE
          ZLEROS = MIN(ZLEROS, ZEVAPLIMMIX)
          ZLEROS = MIN(ZLEROS, ZLI(JL, JK, IBL))
          ZAEROS = ZLEROS / ZLICLD            !if linear term
          
          ! Erosion is -ve LINEAR in L,A
          ZSOLAC = ZSOLAC - ZAEROS            !linear
          
          ZSOLQA(NCLDQV, NCLDQL) = ZSOLQA(NCLDQV, NCLDQL) + ZLIQFRAC(JL, JK, IBL)*ZLEROS
          ZSOLQA(NCLDQL, NCLDQV) = ZSOLQA(NCLDQL, NCLDQV) - ZLIQFRAC(JL, JK, IBL)*ZLEROS
          ZSOLQA(NCLDQV, NCLDQI) = ZSOLQA(NCLDQV, NCLDQI) + ZICEFRAC(JL, JK, IBL)*ZLEROS
          ZSOLQA(NCLDQI, NCLDQV) = ZSOLQA(NCLDQI, NCLDQV) - ZICEFRAC(JL, JK, IBL)*ZLEROS
        ...
        ZDTDP = ZRDCP*ZTP1(JL, JK, IBL) / PAP(JL, JK, IBL)
        ZDPMXDT = ZDP*ZQTMST
        ZMFDN = 0.0_JPRB
        ...
        ZQOLD = ZQSMIX(JL, JK, IBL)
        ZTOLD = ZTP1(JL, JK, IBL)
        ZTP1(JL, JK, IBL) = ZTP1(JL, JK, IBL) + ZDTFORC
        ZTP1(JL, JK, IBL) = MAX(ZTP1(JL, JK, IBL), 160.0_JPRB)
        LLFLAG = .true.
        
        ! Formerly a call to CUADJTQ(..., ICALL=5)
        ZQP = 1.0_JPRB / PAP(JL, JK, IBL)
        ZQSAT = FOEEWM(ZTP1(JL, JK, IBL))*ZQP
        ZQSAT = MIN(0.5_JPRB, ZQSAT)
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZQSAT)
        ZQSAT = ZQSAT*ZCOR
        ZCOND = (ZQSMIX(JL, JK, IBL) - ZQSAT) / (1.0_JPRB + ZQSAT*ZCOR*FOEDEM(ZTP1(JL, JK, IBL)))
        ZTP1(JL, JK, IBL) = ZTP1(JL, JK, IBL) + FOELDCPM(ZTP1(JL, JK, IBL))*ZCOND
        ZQSMIX(JL, JK, IBL) = ZQSMIX(JL, JK, IBL) - ZCOND
        ZQSAT = FOEEWM(ZTP1(JL, JK, IBL))*ZQP
        ZQSAT = MIN(0.5_JPRB, ZQSAT)
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZQSAT)
        ZQSAT = ZQSAT*ZCOR
        ZCOND1 = (ZQSMIX(JL, JK, IBL) - ZQSAT) / (1.0_JPRB + ZQSAT*ZCOR*FOEDEM(ZTP1(JL, JK, IBL)))
        ZTP1(JL, JK, IBL) = ZTP1(JL, JK, IBL) + FOELDCPM(ZTP1(JL, JK, IBL))*ZCOND1
        ZQSMIX(JL, JK, IBL) = ZQSMIX(JL, JK, IBL) - ZCOND1
        
        ZDQS = ZQSMIX(JL, JK, IBL) - ZQOLD
        ZQSMIX(JL, JK, IBL) = ZQOLD
        ZTP1(JL, JK, IBL) = ZTOLD
        ...
        IF (ZDQS > 0.0_JPRB) THEN
          ...
          ZLEVAP = ZA(JL, JK, IBL)*MIN(ZDQS, ZLICLD)
          ZLEVAP = MIN(ZLEVAP, ZEVAPLIMMIX)
          ZLEVAP = MIN(ZLEVAP, MAX(ZQSMIX(JL, JK, IBL) - ZQX(JL, JK, NCLDQV, IBL), 0.0_JPRB))
          
          ! For first guess call
          ZLEVAPL = ZLIQFRAC(JL, JK, IBL)*ZLEVAP
          ZLEVAPI = ZICEFRAC(JL, JK, IBL)*ZLEVAP
          
          ZSOLQA(NCLDQV, NCLDQL) = ZSOLQA(NCLDQV, NCLDQL) + ZLIQFRAC(JL, JK, IBL)*ZLEVAP
          ZSOLQA(NCLDQL, NCLDQV) = ZSOLQA(NCLDQL, NCLDQV) - ZLIQFRAC(JL, JK, IBL)*ZLEVAP
          
          ZSOLQA(NCLDQV, NCLDQI) = ZSOLQA(NCLDQV, NCLDQI) + ZICEFRAC(JL, JK, IBL)*ZLEVAP
          ZSOLQA(NCLDQI, NCLDQV) = ZSOLQA(NCLDQI, NCLDQV) - ZICEFRAC(JL, JK, IBL)*ZLEVAP
        ...
        IF (ZA(JL, JK, IBL) > ZEPSEC .and. ZDQS <= -YRECLDP%RLMIN) THEN
          
          ZLCOND1 = MAX(-ZDQS, 0.0_JPRB)            !new limiter
          
          !old limiter (significantly improves upper tropospheric humidity rms)
          IF (ZA(JL, JK, IBL) > 0.99_JPRB) THEN
            ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZQSMIX(JL, JK, IBL))
            ZCDMAX =  &
            & (ZQX(JL, JK, NCLDQV, IBL) - ZQSMIX(JL, JK, IBL)) / (1.0_JPRB + ZCOR*ZQSMIX(JL, JK, IBL)*FOEDEM(ZTP1(JL, JK, IBL)))
          ELSE
            ZCDMAX = (ZQX(JL, JK, NCLDQV, IBL) - ZA(JL, JK, IBL)*ZQSMIX(JL, JK, IBL)) / ZA(JL, JK, IBL)
          END IF
          ZLCOND1 = MAX(MIN(ZLCOND1, ZCDMAX), 0.0_JPRB)
          ! end old limiter
          
          ZLCOND1 = ZA(JL, JK, IBL)*ZLCOND1
        ...
        IF (ZDQS <= -YRECLDP%RLMIN .and. ZA(JL, JK, IBL) < 1.0_JPRB - ZEPSEC) THEN
          ...
          IF (YRECLDP%NSSOPT == 0) THEN
            ! No scheme
            ZQE = (ZQX(JL, JK, NCLDQV, IBL) - ZA(JL, JK, IBL)*ZQSICE(JL, JK, IBL)) / MAX(ZEPSEC, 1.0_JPRB - ZA(JL, JK, IBL))
          ...
          ELSE IF (YRECLDP%NSSOPT == 2) THEN
            ! Lohmann and Karcher
            ZQE = ZQX(JL, JK, NCLDQV, IBL)
          ELSE IF (YRECLDP%NSSOPT == 3) THEN
            ! Gierens
            ZQE = ZQX(JL, JK, NCLDQV, IBL) + ZLI(JL, JK, IBL)
          END IF
          ...
          IF (ZQE >= ZRHC*ZQSICE(JL, JK, IBL)*ZFAC .and. ZQE < ZQSICE(JL, JK, IBL)*ZFAC) THEN
            ! note: not **2 on 1-a term if ZQE is used.
            ! Added correction term ZFAC to numerator 15/03/2010
            ZACOND = -(1.0_JPRB - ZA(JL, JK, IBL))*ZFAC*ZDQS / MAX(2.0_JPRB*(ZFAC*ZQSICE(JL, JK, IBL) - ZQE), ZEPSEC)
            
            ZACOND = MIN(ZACOND, 1.0_JPRB - ZA(JL, JK, IBL))              !PUT THE LIMITER BACK
            ...
            ZZDL = 2.0_JPRB*(ZFAC*ZQSICE(JL, JK, IBL) - ZQE) / MAX(ZEPSEC, 1.0_JPRB - ZA(JL, JK, IBL))
            ! Added correction term ZFAC 15/03/2010
            IF (ZFAC*ZDQS < -ZZDL) THEN
              ! ZLCONDLIM=(ZA(JL,JK)-1.0_JPRB)*ZDQS(JL)-ZQSICE(JL,JK)+ZQX(JL,JK,NCLDQV)
              ZLCONDLIM = (ZA(JL, JK, IBL) - 1.0_JPRB)*ZFAC*ZDQS - ZFAC*ZQSICE(JL, JK, IBL) + ZQX(JL, JK, NCLDQV, IBL)
              ZLCOND2 = MIN(ZLCOND2, ZLCONDLIM)
            END IF
            ZLCOND2 = MAX(ZLCOND2, 0.0_JPRB)
            
            IF (ZLCOND2 < YRECLDP%RLMIN .or. (1.0_JPRB - ZA(JL, JK, IBL)) < ZEPSEC) THEN
              ZLCOND2 = 0.0_JPRB
              ZACOND = 0.0_JPRB
            END IF
        ...
        IF (IDEPICE == 1) THEN
          ...
          IF (ZA(JL, JK - 1, IBL) < YRECLDP%RCLDTOPCF .and. ZA(JL, JK, IBL) >= YRECLDP%RCLDTOPCF) THEN
            ZCLDTOPDIST = 0.0_JPRB
          ELSE
          ...
          !--------------------------------------------------------------
          IF (ZTP1(JL, JK, IBL) < RTT .and. ZQXFG(NCLDQL) > YRECLDP%RLMIN) THEN
            ! T<273K
            
            ZVPICE = FOEEICE(ZTP1(JL, JK, IBL))*RV / RD
            ...
            !------------------------------------------------
            ZADD = RLSTT*(RLSTT / (RV*ZTP1(JL, JK, IBL)) - 1.0_JPRB) / (2.4E-2_JPRB*ZTP1(JL, JK, IBL))
            ZBDD = RV*ZTP1(JL, JK, IBL)*PAP(JL, JK, IBL) / (2.21_JPRB*ZVPICE)
            ...
            !---------------------------
            ZDEPOS = MAX(ZA(JL, JK, IBL)*(ZINEW - ZICE0), 0.0_JPRB)
        ...
        ELSE IF (IDEPICE == 2) THEN
          ...
          IF (ZTP1(JL, JK, IBL) < RTT .and. ZQXFG(NCLDQL) > YRECLDP%RLMIN) THEN
            ...
            ZAPLUSB = YRECLDP%RCL_APB1*ZVPICE - YRECLDP%RCL_APB2*ZVPICE*ZTP1(JL, JK, IBL) + PAP(JL, JK, IBL) &
            & *YRECLDP%RCL_APB3*ZTP1(JL, JK, IBL)**3._JPRB
            ZCORRFAC = (1.0_JPRB / ZRHO)**0.5_JPRB
            ZCORRFAC2 = ((ZTP1(JL, JK, IBL) / 273.0_JPRB)**1.5_JPRB)*(393.0_JPRB / (ZTP1(JL, JK, IBL) + 120.0_JPRB))
            
            ZPR02 = ZRHO*ZICE0*YRECLDP%RCL_CONST1I / (ZTCG*ZFACX1I)
            
            ZTERM1 = (ZVPLIQ - ZVPICE)*ZTP1(JL, JK, IBL)**2.0_JPRB*ZVPICE*ZCORRFAC2*ZTCG*YRECLDP%RCL_CONST2I*ZFACX1I /  &
            & (ZRHO*ZAPLUSB*ZVPICE)
            ...
            ZDEPOS = MAX(ZA(JL, JK, IBL)*ZTERM1*ZTERM2*PTSPHY, 0.0_JPRB)
        ...
        DO JM=1,NCLV
          IF (LLFALL(JM) .or. JM == NCLDQI) THEN
            ...
            !------------------------
            IF (JK > YRECLDP%NCLDTOP) THEN
              ZFALLSRCE(JM) = ZPFPLSX(JL, JK, JM, IBL)*ZDTGDP
        ...
        IF (ZQPRETOT > ZEPSEC) THEN
          ZCOVPTOT = 1.0_JPRB - ((1.0_JPRB - ZCOVPTOT)*(1.0_JPRB - MAX(ZA(JL, JK, IBL), ZA(JL, JK - 1, IBL))) / (1.0_JPRB -  &
          & MIN(ZA(JL, JK - 1, IBL), 1.0_JPRB - 1.E-06_JPRB)))
          ZCOVPTOT = MAX(ZCOVPTOT, YRECLDP%RCOVPMIN)
          ZCOVPCLR = MAX(0.0_JPRB, ZCOVPTOT - ZA(JL, JK, IBL))            ! clear sky proportion
        ...
        IF (ZTP1(JL, JK, IBL) <= RTT) THEN
          !-----------------------------------------------------
          !     Snow Autoconversion rate follow Lin et al. 1983
          !-----------------------------------------------------
          IF (ZICECLD > ZEPSEC) THEN
            
            ZZCO = PTSPHY*YRECLDP%RSNOWLIN1*EXP(YRECLDP%RSNOWLIN2*(ZTP1(JL, JK, IBL) - RTT))
        ...
        IF (ZLIQCLD > ZEPSEC) THEN
          ...
          IF (IWARMRAIN == 1) THEN
            ...
            !------------------------------------------------------------------
            ZPRECIP = (ZPFPLSX(JL, JK, NCLDQS, IBL) + ZPFPLSX(JL, JK, NCLDQR, IBL)) / MAX(ZEPSEC, ZCOVPTOT)
          ...
          ELSE IF (IWARMRAIN == 2) THEN
            ...
            IF (ZLIQCLD > ZLCRIT) THEN
              ...
              ZRAINAUT = 1.5_JPRB*ZA(JL, JK, IBL)*PTSPHY*YRECLDP%RCL_KKAAU*ZLIQCLD**YRECLDP%RCL_KKBAUQ*ZCONST**YRECLDP%RCL_KKBAUN
              
              ZRAINAUT = MIN(ZRAINAUT, ZQXFG(NCLDQL))
              IF (ZRAINAUT < ZEPSEC) ZRAINAUT = 0.0_JPRB
              
              ZRAINACC = 2.0_JPRB*ZA(JL, JK, IBL)*PTSPHY*YRECLDP%RCL_KKAAC*(ZLIQCLD*ZRAINCLD)**YRECLDP%RCL_KKBAC
        ...
        IF (IWARMRAIN > 1) THEN
          ...
          IF (ZTP1(JL, JK, IBL) <= RTT .and. ZLIQCLD > ZEPSEC) THEN
        ...
        IF (ZICETOT > ZEPSEC .and. ZTP1(JL, JK, IBL) > RTT) THEN
          
          ! Calculate subsaturation
          ZSUBSAT = MAX(ZQSICE(JL, JK, IBL) - ZQX(JL, JK, NCLDQV, IBL), 0.0_JPRB)
          ...
          ! due to evaporation.
          ZTDMTW0 = ZTP1(JL, JK, IBL) - RTT - ZSUBSAT*(ZTW1 + ZTW2*(PAP(JL, JK, IBL) - ZTW3) - ZTW4*(ZTP1(JL, JK, IBL) - ZTW5))
          ! Not implicit yet...
        ...
        ! If rain present
        IF (ZQX(JL, JK, NCLDQR, IBL) > ZEPSEC) THEN
          
          IF (ZTP1(JL, JK, IBL) <= RTT .and. ZTP1(JL, JK - 1, IBL) > RTT) THEN
            ...
            ZQPRETOT = MAX(ZQX(JL, JK, NCLDQS, IBL) + ZQX(JL, JK, NCLDQR, IBL), ZEPSEC)
            PRAINFRAC_TOPRFZ(JL, IBL) = ZQX(JL, JK, NCLDQR, IBL) / ZQPRETOT
          ...
          IF (ZTP1(JL, JK, IBL) < RTT) THEN
            ...
            IF (PRAINFRAC_TOPRFZ(JL, IBL) > 0.8) THEN
              ...
              ZLAMBDA = (YRECLDP%RCL_FAC1 / (ZRHO*ZQX(JL, JK, NCLDQR, IBL)))**YRECLDP%RCL_FAC2
              
              ! Calculate freezing rate based on Bigg(1953) and Wisner(1972)
              ZTEMP = YRECLDP%RCL_FZRAB*(ZTP1(JL, JK, IBL) - RTT)
            ...
            ELSE
              ...
              ZCONS1 = ABS(PTSPHY*(1.0_JPRB + 0.5_JPRB*(RTT - ZTP1(JL, JK, IBL))) / YRECLDP%RTAUMEL)
              ZFRZMAX = MAX((RTT - ZTP1(JL, JK, IBL))*ZCONS1*ZRLDCP, 0.0_JPRB)
              
            END IF
            
            IF (ZFRZMAX > ZEPSEC) THEN
              ZFRZ = MIN(ZQX(JL, JK, NCLDQR, IBL), ZFRZMAX)
        ...
        !----------------------------------------------------------------------
        ! not implicit yet...
        ZFRZMAX = MAX((YRECLDP%RTHOMO - ZTP1(JL, JK, IBL))*ZRLDCP, 0.0_JPRB)
        ...
        IF (IEVAPRAIN == 1) THEN
          ...
          ZZRH = YRECLDP%RPRECRHMAX + (1.0_JPRB - YRECLDP%RPRECRHMAX)*ZCOVPMAX / MAX(ZEPSEC, 1.0_JPRB - ZA(JL, JK, IBL))
          ZZRH = MIN(MAX(ZZRH, YRECLDP%RPRECRHMAX), 1.0_JPRB)
          
          ZQE = (ZQX(JL, JK, NCLDQV, IBL) - ZA(JL, JK, IBL)*ZQSLIQ(JL, JK, IBL)) / MAX(ZEPSEC, 1.0_JPRB - ZA(JL, JK, IBL))
          !---------------------------------------------
          ! humidity in moistest ZCOVPCLR part of domain
          !---------------------------------------------
          ZQE = MAX(0.0_JPRB, MIN(ZQE, ZQSLIQ(JL, JK, IBL)))
          LLO1 = ZCOVPCLR > ZEPSEC .and. ZQXFG(NCLDQR) > ZEPSEC .and. ZQE < ZZRH*ZQSLIQ(JL, JK, IBL)
          ...
          IF (LLO1) THEN
            ...
            ZDPR = ZCOVPCLR*ZBETA*(ZQSLIQ(JL, JK, IBL) - ZQE) / ZDENOM*ZDP*ZRG_R
            ZDPEVAP = ZDPR*ZDTGDP
            ...
            !-------------------------------------------------------------
            ZCOVPTOT = MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, (ZCOVPTOT - ZA(JL, JK, IBL))*ZEVAP / ZQXFG(NCLDQR)))
        ...
        ELSE IF (IEVAPRAIN == 2) THEN
          ...
          ZQE = MAX(0.0_JPRB, MIN(ZQX(JL, JK, NCLDQV, IBL), ZQSLIQ(JL, JK, IBL)))
          ...
          IF (LLO1) THEN
            ...
            ZESATLIQ = RV / RD*FOEELIQ(ZTP1(JL, JK, IBL))
            
            ! Slope of particle size distribution
            ZLAMBDA = (YRECLDP%RCL_FAC1 / (ZRHO*ZPRECLR))**YRECLDP%RCL_FAC2              ! ZPRECLR=kg/kg
            
            ZEVAP_DENOM = YRECLDP%RCL_CDENOM1*ZESATLIQ - YRECLDP%RCL_CDENOM2*ZTP1(JL, JK, IBL)*ZESATLIQ +  &
            & YRECLDP%RCL_CDENOM3*ZTP1(JL, JK, IBL)**3._JPRB*PAP(JL, JK, IBL)
            
            ! Temperature dependent conductivity
            ZCORR2 = (ZTP1(JL, JK, IBL) / 273._JPRB)**1.5_JPRB*393._JPRB / (ZTP1(JL, JK, IBL) + 120._JPRB)
            ZKA = YRECLDP%RCL_KA273*ZCORR2
            
            ZSUBSAT = MAX(ZZRH*ZQSLIQ(JL, JK, IBL) - ZQE, 0.0_JPRB)
            
            ZBETA = (0.5_JPRB / ZQSLIQ(JL, JK, IBL))*ZTP1(JL, JK, IBL)**2._JPRB*ZESATLIQ*YRECLDP%RCL_CONST1R*(ZCORR2 /  &
        ...
        IF (IEVAPSNOW == 1) THEN
          ...
          ! humidity in moistest ZCOVPCLR part of domain
          !---------------------------------------------
          ZQE = MAX(0.0_JPRB, MIN(ZQE, ZQSICE(JL, JK, IBL)))
          LLO1 = ZCOVPCLR > ZEPSEC .and. ZQXFG(NCLDQS) > ZEPSEC .and. ZQE < ZZRH*ZQSICE(JL, JK, IBL)
          ...
          IF (LLO1) THEN
            ...
            ZDPR = ZCOVPCLR*ZBETA*(ZQSICE(JL, JK, IBL) - ZQE) / ZDENOM*ZDP*ZRG_R
            ZDPEVAP = ZDPR*ZDTGDP
            ...
            ! 2-flux treatment, abandoned due to the new prognostic precip
            !-------------------------------------------------------------
            ZCOVPTOT = MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, (ZCOVPTOT - ZA(JL, JK, IBL))*ZEVAP / ZQXFG(NCLDQS)))
        ...
        ELSE IF (IEVAPSNOW == 2) THEN
          ...
          !---------------------------------------------
          ZQE = MAX(0.0_JPRB, MIN(ZQE, ZQSICE(JL, JK, IBL)))
          LLO1 = ZCOVPCLR > ZEPSEC .and. ZQX(JL, JK, NCLDQS, IBL) > ZEPSEC .and. ZQE < ZZRH*ZQSICE(JL, JK, IBL)
          
          IF (LLO1) THEN
            
            ! Calculate local precipitation (kg/kg)
            ZPRECLR = ZQX(JL, JK, NCLDQS, IBL) / ZCOVPTOT
            ZVPICE = FOEEICE(ZTP1(JL, JK, IBL))*RV / RD
            ...
            ZAPLUSB = YRECLDP%RCL_APB1*ZVPICE - YRECLDP%RCL_APB2*ZVPICE*ZTP1(JL, JK, IBL) + PAP(JL, JK, IBL) &
            & *YRECLDP%RCL_APB3*ZTP1(JL, JK, IBL)**3
            ZCORRFAC = (1.0 / ZRHO)**0.5
            ZCORRFAC2 = ((ZTP1(JL, JK, IBL) / 273.0)**1.5)*(393.0 / (ZTP1(JL, JK, IBL) + 120.0))
            
            ZPR02 = ZRHO*ZPRECLR*YRECLDP%RCL_CONST1S / (ZTCG*ZFACX1S)
            
            ZTERM1 = (ZQSICE(JL, JK, IBL) - ZQE)*ZTP1(JL, JK, IBL)**2*ZVPICE*ZCORRFAC2*ZTCG*YRECLDP%RCL_CONST2S*ZFACX1S /  &
            & (ZRHO*ZAPLUSB*ZQSICE(JL, JK, IBL))
            ...
            ZEVAP = MIN(ZEVAP, ZQX(JL, JK, NCLDQS, IBL))
            ...
            !-------------------------------------------------------------
            ZCOVPTOT =  &
            & MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, (ZCOVPTOT - ZA(JL, JK, IBL))*ZEVAP / ZQX(JL, JK, NCLDQS, IBL)))
        ...
        !---------------------------
        ZANEW = (ZA(JL, JK, IBL) + ZSOLAC) / (1.0_JPRB + ZSOLAB)
        ZANEW = MIN(ZANEW, 1.0_JPRB)
        IF (ZANEW < YRECLDP%RAMIN) ZANEW = 0.0_JPRB
        ZDA = ZANEW - ZAORIG(JL, JK, IBL)
        !---------------------------------
        ...
        !---------------------------------------
        DO JM=1,NCLV
          ZMAX = MAX(ZQX(JL, JK, JM, IBL), ZEPSEC)
        ...
        DO JM=1,NCLV
          ...
          !---------------------------
          ZMM = MAX(ZQX(JL, JK, JM, IBL), ZEPSEC)
        ...
        DO JM=1,NCLV
          ...
          END DO
          ZQXN(JM) = ZQX(JL, JK, JM, IBL) + ZEXPLICIT
        END DO
        ...
        DO JM=1,NCLV
          ZPFPLSX(JL, JK + 1, JM, IBL) = ZFALLSINK(JM)*ZQXN(JM)*ZRDTGDP
        END DO
        
        ! Ensure precipitation fraction is zero if no precipitation
        ZQPRETOT = ZPFPLSX(JL, JK + 1, NCLDQS, IBL) + ZPFPLSX(JL, JK + 1, NCLDQR, IBL)
        IF (ZQPRETOT < ZEPSEC) THEN
          ZCOVPTOT = 0.0_JPRB
        END IF
        ...
        DO JM=1,NCLV - 1
          ...
          IF (IPHASE(JM) == 1) THEN
            ...
            & TENDENCY_LOC(JL, JK, 1, IBL) + RALVDCP*(ZQXN(JM) - ZQX(JL, JK, JM, IBL) - ZFLUXQ(JM))*ZQTMST
          END IF
          
          IF (IPHASE(JM) == 2) THEN
            TENDENCY_LOC(JL, JK, 1, IBL) =  &
            & TENDENCY_LOC(JL, JK, 1, IBL) + RALSDCP*(ZQXN(JM) - ZQX(JL, JK, JM, IBL) - ZFLUXQ(JM))*ZQTMST
          END IF
          ...
          !----------------------------------------------------------------------
          TENDENCY_LOC(JL, JK, 3 + JM, IBL) = TENDENCY_LOC(JL, JK, 3 + JM, IBL) + (ZQXN(JM) - ZQX0(JL, JK, JM, IBL))*ZQTMST
        ...
        ! 6.2 Humidity budget
        !----------------------
        TENDENCY_LOC(JL, JK, 3, IBL) = TENDENCY_LOC(JL, JK, 3, IBL) + (ZQXN(NCLDQV) - ZQX(JL, JK, NCLDQV, IBL))*ZQTMST
      ...
      ! $acc loop seq
      DO JK=1,KLEV + 1
        PFPLSL(JL, JK, IBL) = ZPFPLSX(JL, JK, NCLDQR, IBL) + ZPFPLSX(JL, JK, NCLDQL, IBL)
        PFPLSN(JL, JK, IBL) = ZPFPLSX(JL, JK, NCLDQS, IBL) + ZPFPLSX(JL, JK, NCLDQI, IBL)
      END DO
      ...
      DO JK=1,KLEV
        ...
        PFSQLF(JL, JK + 1, IBL) = PFSQLF(JL, JK + 1, IBL) + (ZQXN2D(JL, JK, NCLDQL, IBL) - ZQX0(JL, JK, NCLDQL, IBL) + PVFL(JL,  &
        & JK, IBL)*PTSPHY - ZALFAW*PLUDE(JL, JK, IBL))*ZGDPH_R
        ! liquid, negative numbers
        PFCQLNG(JL, JK + 1, IBL) = PFCQLNG(JL, JK + 1, IBL) + ZLNEG(JL, JK, NCLDQL, IBL)*ZGDPH_R
        
        ! liquid, vertical diffusion
        PFSQLTUR(JL, JK + 1, IBL) = PFSQLTUR(JL, JK + 1, IBL) + PVFL(JL, JK, IBL)*PTSPHY*ZGDPH_R
        
        ! Rain, LS scheme
        PFSQRF(JL, JK + 1, IBL) = PFSQRF(JL, JK + 1, IBL) + (ZQXN2D(JL, JK, NCLDQR, IBL) - ZQX0(JL, JK, NCLDQR, IBL))*ZGDPH_R
        ! rain, negative numbers
        PFCQRNG(JL, JK + 1, IBL) = PFCQRNG(JL, JK + 1, IBL) + ZLNEG(JL, JK, NCLDQR, IBL)*ZGDPH_R
        
        ! Ice , LS scheme minus detrainment
        PFSQIF(JL, JK + 1, IBL) = PFSQIF(JL, JK + 1, IBL) + (ZQXN2D(JL, JK, NCLDQI, IBL) - ZQX0(JL, JK, NCLDQI, IBL) + PVFI(JL,  &
        & JK, IBL)*PTSPHY - (1.0_JPRB - ZALFAW)*PLUDE(JL, JK, IBL))*ZGDPH_R
        ! ice, negative numbers
        PFCQNNG(JL, JK + 1, IBL) = PFCQNNG(JL, JK + 1, IBL) + ZLNEG(JL, JK, NCLDQI, IBL)*ZGDPH_R
        
        ! ice, vertical diffusion
        PFSQITUR(JL, JK + 1, IBL) = PFSQITUR(JL, JK + 1, IBL) + PVFI(JL, JK, IBL)*PTSPHY*ZGDPH_R
        
        ! snow, LS scheme
        PFSQSF(JL, JK + 1, IBL) = PFSQSF(JL, JK + 1, IBL) + (ZQXN2D(JL, JK, NCLDQS, IBL) - ZQX0(JL, JK, NCLDQS, IBL))*ZGDPH_R
        ! snow, negative numbers
        PFCQSNG(JL, JK + 1, IBL) = PFCQSNG(JL, JK + 1, IBL) + ZLNEG(JL, JK, NCLDQS, IBL)*ZGDPH_R
      END DO
    ...
    ! $acc exit data delete(ZFOEALFA, ZTP1, ZLI, ZA, ZAORIG, ZLIQFRAC, ZICEFRAC, ZQX, ZQX0,  &
    ! $acc &   ZPFPLSX, ZLNEG, ZQXN2D, ZQSMIX, ZQSLIQ, ZQSICE, ZFOEEWMT,  &
    ! $acc &   ZFOEEW, ZFOEELIQT)
  ...
  END SUBROUTINE CLOUDSC_HOIST_DR_LOOP
  
END MODULE CLOUDSC_HOIST_DR_LOOP_MOD
