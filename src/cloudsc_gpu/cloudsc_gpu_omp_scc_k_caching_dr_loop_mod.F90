...
MODULE CLOUDSC_GPU_OMP_SCC_K_CACHING_DR_LOOP_MOD

  ! $omp declare target(cloudsc_scc_cuf_k_caching)  
      
  CONTAINS
  
  ! ATTRIBUTES(GLOBAL) 
  SUBROUTINE CLOUDSC_SCC_CUF_K_CACHING (KIDIA, NGPTOT, NPROMA, KFDIA, KLON, KLEV, PTSPHY, PT, PQ, TENDENCY_TMP, TENDENCY_LOC, PVFA,  &
  ...
  & PFSQRF, PFSQSF, PFCQRNG, PFCQSNG, PFSQLTUR, PFSQITUR, PFPLSL, PFPLSN, PFHPSL, PFHPSN, NGPBLKS, YRECLDP)
    ...
    INTEGER(KIND=JPIM),  INTENT(IN) :: KFDIA, NPROMA, NGPTOT
    ...
    !!! Serial version !!!!!!!!!!!!!!!!!!!
    ! $omp target teams distribute parallel do simd private(ZLCUST, IPHASE, IMELT, LLFALL, LLINDEX1, LLINDEX3, IORDER, ZQXN, ZQXFG, ZQXNM1, ZFLUXQ, ZSOLQA, ZSOLQB, ZQLHS, ZVQX, ZRATIO, ZSINKSUM, ZFALLSINK, ZFALLSRCE, ZCONVSRCE, ZCONVSINK, ZPSUPSATSRCE)
    ! $omp parallel do simd private(JL, IBL) collapse(2)
    
   ! thread_limit(CEILING(REAL(NGPTOT) / REAL(NPROMA)))
   ! $omp target teams distribute parallel do simd collapse(2) private(JL, IBL, ZFOEALFA, ZTP1, ZLCUST, ZLI, ZA, ZAORIG, IPHASE, IMELT, LLFALL, LLINDEX1, &
   ! $omp  LLINDEX3, IORDER, ZLIQFRAC, ZICEFRAC, ZQX, ZQX0, ZQXN, ZQXFG, ZQXNM1, ZFLUXQ, ZPFPLSX, ZLNEG, ZQXN2D, ZQSMIX, ZQSLIQ, ZQSICE, &
   ! $omp  ZFOEEWMT, ZFOEEW, ZFOEELIQT, ZSOLQA, ZSOLQB, ZQLHS, ZVQX, ZRATIO, ZSINKSUM, ZFALLSINK, ZFALLSRCE, ZCONVSRCE, ZCONVSINK, &
   ! $omp  ZPSUPSATSRCE) 

#if 1
   !$omp target teams distribute parallel do simd collapse(2) private(JL, IBL, JK_I, JK_IP1, JK_IM1, ZTP1, ZLCUST, ZA, IPHASE, IMELT, LLFALL, LLINDEX1, LLINDEX3, &
   !$omp  IORDER, ZQX, ZQX0, ZQXN, ZQXFG, ZQXNM1, ZFLUXQ, ZPFPLSX, ZLNEG, ZQXN2D, ZSOLQA, ZSOLQB, ZQLHS, ZVQX, ZRATIO, ZSINKSUM, &
   !$omp  ZFALLSINK, ZFALLSRCE, ZCONVSRCE, ZCONVSINK, ZPSUPSATSRCE)
#else
   !$omp target teams loop collapse(2) private(JL, IBL, JK_I, JK_IP1, JK_IM1, ZTP1, ZLCUST, ZA, IPHASE, IMELT, LLFALL, LLINDEX1, LLINDEX3, &
   !$omp  IORDER, ZQX, ZQX0, ZQXN, ZQXFG, ZQXNM1, ZFLUXQ, ZPFPLSX, ZLNEG, ZQXN2D, ZSOLQA, ZSOLQB, ZQLHS, ZVQX, ZRATIO, ZSINKSUM, &
   !$omp  ZFALLSINK, ZFALLSRCE, ZCONVSRCE, ZCONVSINK, ZPSUPSATSRCE)
#endif
    ! DO JL=1,NPROMA
  ...
  END SUBROUTINE CLOUDSC_SCC_CUF_K_CACHING
    
END MODULE CLOUDSC_GPU_OMP_SCC_K_CACHING_DR_LOOP_MOD
