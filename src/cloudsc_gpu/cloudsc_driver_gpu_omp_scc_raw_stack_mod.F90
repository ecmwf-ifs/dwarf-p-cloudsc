...
MODULE CLOUDSC_DRIVER_GPU_OMP_SCC_RAW_STACK_MOD
  ...
  ! USE CLOUDSC_LOKI_MOD, ONLY: CLOUDSC_LOKI
  ...
  SUBROUTINE CLOUDSC_DRIVER_GPU_SCC_RAW_STACK (NUMOMP, NPROMA, NLEV, NGPTOT, NGPTOTG, NGPBLKS, KFLDX, PTSPHY, PT, PQ, TENDENCY_CML, TENDENCY_TMP,  &
  ...
  & PFPLSN, PFHPSL, PFHPSN)
    ...
    INTEGER :: JL
    INTEGER(KIND=JPIM) :: J_Z_JPRB_STACK_SIZE
    REAL(KIND=JPRB) :: Z_JPRB_STACK(NPROMA, 1 + 13*NLEV + 5*NLEV*NCLV + NCLV, NGPBLKS)
!$omp target data map( alloc: z_jprb_stack )
! ! $omp target allocate(z_jprb_stack)
    J_Z_JPRB_STACK_SIZE = 1 + 13*NLEV + 5*NLEV*NCLV + NCLV
...
! ! $omp target teams distribute private( Z_JPRB_STACK ) thread_limit(128)
!$omp target teams distribute thread_limit(128)
...
!$omp parallel do
      DO JL=1,ICEND
        ...
        & LOCAL_YRECLDP, J_Z_JPRB_STACK_SIZE, Z_JPRB_STACK(:, :, IBL), JL=JL)
      END DO
