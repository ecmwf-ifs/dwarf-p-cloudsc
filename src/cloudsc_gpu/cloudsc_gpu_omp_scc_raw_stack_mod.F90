...
MODULE CLOUDSC_GPU_OMP_SCC_RAW_STACK_MOD
  ...
  & PFCQSNG, PFSQLTUR, PFSQITUR, PFPLSL, PFPLSN, PFHPSL, PFHPSN, YRECLDP, K_P_JPRB_STACK_SIZE, P_JPRB_STACK, JL)
    !---input
    !---prognostic fields
...
!$omp declare target
    INTEGER(KIND=JPIM) :: JD_ZFOEALFA
    INTEGER(KIND=JPIM) :: JD_ZTP1
    INTEGER(KIND=JPIM) :: JD_ZLI
    INTEGER(KIND=JPIM) :: JD_ZA
    INTEGER(KIND=JPIM) :: JD_ZAORIG
    INTEGER(KIND=JPIM) :: JD_ZLIQFRAC
    INTEGER(KIND=JPIM) :: JD_ZICEFRAC
    INTEGER(KIND=JPIM) :: JD_ZQX
    INTEGER(KIND=JPIM) :: JD_ZQX0
    INTEGER(KIND=JPIM) :: JD_ZPFPLSX
    INTEGER(KIND=JPIM) :: JD_ZLNEG
    INTEGER(KIND=JPIM) :: JD_ZQXN2D
    INTEGER(KIND=JPIM) :: JD_ZQSMIX
    INTEGER(KIND=JPIM) :: JD_ZQSLIQ
    INTEGER(KIND=JPIM) :: JD_ZQSICE
    INTEGER(KIND=JPIM) :: JD_ZFOEEWMT
    INTEGER(KIND=JPIM) :: JD_ZFOEEW
    INTEGER(KIND=JPIM) :: JD_ZFOEELIQT
    INTEGER(KIND=JPIM) :: J_P_JPRB_STACK_USED
    INTEGER(KIND=JPIM), INTENT(IN) :: K_P_JPRB_STACK_SIZE
    REAL(KIND=JPRB), INTENT(INOUT) :: P_JPRB_STACK(KLON, K_P_JPRB_STACK_SIZE)
    JD_ZFOEALFA = 0
    JD_ZTP1 = JD_ZFOEALFA + 1 + KLEV
    JD_ZLI = JD_ZTP1 + KLEV
    JD_ZA = JD_ZLI + KLEV
    JD_ZAORIG = JD_ZA + KLEV
    JD_ZLIQFRAC = JD_ZAORIG + KLEV
    JD_ZICEFRAC = JD_ZLIQFRAC + KLEV
    JD_ZQX = JD_ZICEFRAC + KLEV
    JD_ZQX0 = JD_ZQX + KLEV*NCLV
    JD_ZPFPLSX = JD_ZQX0 + KLEV*NCLV
    JD_ZLNEG = JD_ZPFPLSX + NCLV + KLEV*NCLV
    JD_ZQXN2D = JD_ZLNEG + KLEV*NCLV
    JD_ZQSMIX = JD_ZQXN2D + KLEV*NCLV
    JD_ZQSLIQ = JD_ZQSMIX + KLEV
    JD_ZQSICE = JD_ZQSLIQ + KLEV
    JD_ZFOEEWMT = JD_ZQSICE + KLEV
    JD_ZFOEEW = JD_ZFOEEWMT + KLEV
    JD_ZFOEELIQT = JD_ZFOEEW + KLEV
    J_P_JPRB_STACK_USED = JD_ZFOEELIQT + KLEV
    !===============================================================================
...
!$loki loop seq
    DO JK=1,KLEV
      P_JPRB_STACK(JL, JD_ZTP1 + JK) = PT(JL, JK) + PTSPHY*TENDENCY_TMP_T(JL, JK)
      P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) = PQ(JL, JK) + PTSPHY*TENDENCY_TMP_Q(JL, JK)
      P_JPRB_STACK(JL, JD_ZQX0 + JK - KLEV + KLEV*NCLDQV) = PQ(JL, JK) + PTSPHY*TENDENCY_TMP_Q(JL, JK)
      P_JPRB_STACK(JL, JD_ZA + JK) = PA(JL, JK) + PTSPHY*TENDENCY_TMP_A(JL, JK)
      P_JPRB_STACK(JL, JD_ZAORIG + JK) = PA(JL, JK) + PTSPHY*TENDENCY_TMP_A(JL, JK)
    END DO
...
!$loki loop seq
      DO JK=1,KLEV
        P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + JM*KLEV) = PCLV(JL, JK, JM) + PTSPHY*TENDENCY_TMP_CLD(JL, JK, JM)
        P_JPRB_STACK(JL, JD_ZQX0 + JK - KLEV + JM*KLEV) = PCLV(JL, JK, JM) + PTSPHY*TENDENCY_TMP_CLD(JL, JK, JM)
      END DO
    END DO
...
!$loki loop seq
      DO JK=1,KLEV + 1
        P_JPRB_STACK(JL, JD_ZPFPLSX - 1 + JK + JM - KLEV + JM*KLEV) = 0.0_JPRB          ! precip fluxes
      END DO
    END DO
...
!$loki loop seq
      DO JK=1,KLEV
        P_JPRB_STACK(JL, JD_ZQXN2D + JK - KLEV + JM*KLEV) = 0.0_JPRB          ! end of timestep values in 2D
        P_JPRB_STACK(JL, JD_ZLNEG + JK - KLEV + JM*KLEV) = 0.0_JPRB          ! negative input check
      END DO
    END DO
...
!$loki loop seq
    DO JK=1,KLEV
      IF (P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQL) + P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQI) < YRECLDP%RLMIN &
      &  .or. P_JPRB_STACK(JL, JD_ZA + JK) < YRECLDP%RAMIN) THEN
        
        ! Evaporate small cloud liquid water amounts
        P_JPRB_STACK(JL, JD_ZLNEG + JK - KLEV + KLEV*NCLDQL) =  &
        & P_JPRB_STACK(JL, JD_ZLNEG + JK - KLEV + KLEV*NCLDQL) + P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQL)
        ZQADJ = P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQL)*ZQTMST
        TENDENCY_LOC_Q(JL, JK) = TENDENCY_LOC_Q(JL, JK) + ZQADJ
        TENDENCY_LOC_T(JL, JK) = TENDENCY_LOC_T(JL, JK) - RALVDCP*ZQADJ
        P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) =  &
        & P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) + P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQL)
        P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQL) = 0.0_JPRB
        
        ! Evaporate small cloud ice water amounts
        P_JPRB_STACK(JL, JD_ZLNEG + JK - KLEV + KLEV*NCLDQI) =  &
        & P_JPRB_STACK(JL, JD_ZLNEG + JK - KLEV + KLEV*NCLDQI) + P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQI)
        ZQADJ = P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQI)*ZQTMST
        TENDENCY_LOC_Q(JL, JK) = TENDENCY_LOC_Q(JL, JK) + ZQADJ
        TENDENCY_LOC_T(JL, JK) = TENDENCY_LOC_T(JL, JK) - RALSDCP*ZQADJ
        P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) =  &
        & P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) + P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQI)
        P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQI) = 0.0_JPRB
        
        ! Set cloud cover to zero
        P_JPRB_STACK(JL, JD_ZA + JK) = 0.0_JPRB
...
!$loki loop seq
      DO JK=1,KLEV
        !DIR$ IVDEP
        IF (P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + JM*KLEV) < YRECLDP%RLMIN) THEN
          P_JPRB_STACK(JL, JD_ZLNEG + JK - KLEV + JM*KLEV) =  &
          & P_JPRB_STACK(JL, JD_ZLNEG + JK - KLEV + JM*KLEV) + P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + JM*KLEV)
          ZQADJ = P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + JM*KLEV)*ZQTMST
          TENDENCY_LOC_Q(JL, JK) = TENDENCY_LOC_Q(JL, JK) + ZQADJ
          ...
          P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) =  &
          & P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) + P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + JM*KLEV)
          P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + JM*KLEV) = 0.0_JPRB
        END IF
      END DO
    END DO
...
!$loki loop seq
    DO JK=1,KLEV
      ...
      !----------------------------------------
      P_JPRB_STACK(JL, JD_ZFOEALFA + JK) = FOEALFA(P_JPRB_STACK(JL, JD_ZTP1 + JK))
      P_JPRB_STACK(JL, JD_ZFOEEWMT + JK) = MIN(FOEEWM(P_JPRB_STACK(JL, JD_ZTP1 + JK)) / PAP(JL, JK), 0.5_JPRB)
      P_JPRB_STACK(JL, JD_ZQSMIX + JK) = P_JPRB_STACK(JL, JD_ZFOEEWMT + JK)
      P_JPRB_STACK(JL, JD_ZQSMIX + JK) = P_JPRB_STACK(JL, JD_ZQSMIX + JK) / (1.0_JPRB - RETV*P_JPRB_STACK(JL, JD_ZQSMIX + JK))
      
      !---------------------------------------------
      ! ice saturation T<273K
      ! liquid water saturation for T>273K
      !---------------------------------------------
      ZALFA = FOEDELTA(P_JPRB_STACK(JL, JD_ZTP1 + JK))
      P_JPRB_STACK(JL, JD_ZFOEEW + JK) = MIN((ZALFA*FOEELIQ(P_JPRB_STACK(JL, JD_ZTP1 + JK)) + (1.0_JPRB - ZALFA) &
      & *FOEEICE(P_JPRB_STACK(JL, JD_ZTP1 + JK))) / PAP(JL, JK), 0.5_JPRB)
      P_JPRB_STACK(JL, JD_ZFOEEW + JK) = MIN(0.5_JPRB, P_JPRB_STACK(JL, JD_ZFOEEW + JK))
      P_JPRB_STACK(JL, JD_ZQSICE + JK) = P_JPRB_STACK(JL, JD_ZFOEEW + JK) / (1.0_JPRB - RETV*P_JPRB_STACK(JL, JD_ZFOEEW + JK))
      
      !----------------------------------
      ! liquid water saturation
      !----------------------------------
      P_JPRB_STACK(JL, JD_ZFOEELIQT + JK) = MIN(FOEELIQ(P_JPRB_STACK(JL, JD_ZTP1 + JK)) / PAP(JL, JK), 0.5_JPRB)
      P_JPRB_STACK(JL, JD_ZQSLIQ + JK) = P_JPRB_STACK(JL, JD_ZFOEELIQT + JK)
      P_JPRB_STACK(JL, JD_ZQSLIQ + JK) = P_JPRB_STACK(JL, JD_ZQSLIQ + JK) / (1.0_JPRB - RETV*P_JPRB_STACK(JL, JD_ZQSLIQ + JK))
...
!$loki loop seq
    DO JK=1,KLEV
      ...
      !------------------------------------------
      P_JPRB_STACK(JL, JD_ZA + JK) = MAX(0.0_JPRB, MIN(1.0_JPRB, P_JPRB_STACK(JL, JD_ZA + JK)))
      
      !-------------------------------------------------------------------
      ! Calculate liq/ice fractions (no longer a diagnostic relationship)
      !-------------------------------------------------------------------
      P_JPRB_STACK(JL, JD_ZLI + JK) =  &
      & P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQL) + P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQI)
      IF (P_JPRB_STACK(JL, JD_ZLI + JK) > YRECLDP%RLMIN) THEN
        P_JPRB_STACK(JL, JD_ZLIQFRAC + JK) = P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQL) / P_JPRB_STACK(JL, JD_ZLI + JK)
        P_JPRB_STACK(JL, JD_ZICEFRAC + JK) = 1.0_JPRB - P_JPRB_STACK(JL, JD_ZLIQFRAC + JK)
      ELSE
        P_JPRB_STACK(JL, JD_ZLIQFRAC + JK) = 0.0_JPRB
        P_JPRB_STACK(JL, JD_ZICEFRAC + JK) = 0.0_JPRB
      END IF
...
!$loki loop seq
    DO JK=1,KLEV - 1
      ...
      IF (ZSIG > 0.1_JPRB .and. ZSIG < 0.4_JPRB .and. P_JPRB_STACK(JL, JD_ZTP1 + JK) > P_JPRB_STACK(JL, JD_ZTP1 + 1 + JK)) THEN
        ZTRPAUS = ZSIG
      END IF
    END DO
...
!$loki loop seq
      DO JM=1,NCLV
        ZQXFG(JM) = P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + JM*KLEV)
      END DO
...
!$loki loop seq
      ...
      ZGDP = RG / ZDP        ! g/dp
      ZRHO = PAP(JL, JK) / (RD*P_JPRB_STACK(JL, JD_ZTP1 + JK))        ! p/RT air density
      ...
      ! liquid
      ZFACW = R5LES / ((P_JPRB_STACK(JL, JD_ZTP1 + JK) - R4LES)**2)
      ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*P_JPRB_STACK(JL, JD_ZFOEELIQT + JK))
      ZDQSLIQDT = ZFACW*ZCOR*P_JPRB_STACK(JL, JD_ZQSLIQ + JK)
      ZCORQSLIQ = 1.0_JPRB + RALVDCP*ZDQSLIQDT
      
      ! ice
      ZFACI = R5IES / ((P_JPRB_STACK(JL, JD_ZTP1 + JK) - R4IES)**2)
      ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*P_JPRB_STACK(JL, JD_ZFOEEW + JK))
      ZDQSICEDT = ZFACI*ZCOR*P_JPRB_STACK(JL, JD_ZQSICE + JK)
      ZCORQSICE = 1.0_JPRB + RALSDCP*ZDQSICEDT
      
      ! diagnostic mixed
      ZALFAW = P_JPRB_STACK(JL, JD_ZFOEALFA + JK)
      ZALFAWM = ZALFAW
      ZFAC = ZALFAW*ZFACW + (1.0_JPRB - ZALFAW)*ZFACI
      ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*P_JPRB_STACK(JL, JD_ZFOEEWMT + JK))
      ZDQSMIXDT = ZFAC*ZCOR*P_JPRB_STACK(JL, JD_ZQSMIX + JK)
      ZCORQSMIX = 1.0_JPRB + FOELDCPM(P_JPRB_STACK(JL, JD_ZTP1 + JK))*ZDQSMIXDT
      
      ! evaporation/sublimation limits
      ZEVAPLIMMIX =  &
      & MAX((P_JPRB_STACK(JL, JD_ZQSMIX + JK) - P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV)) / ZCORQSMIX, 0.0_JPRB)
      ZEVAPLIMLIQ =  &
      & MAX((P_JPRB_STACK(JL, JD_ZQSLIQ + JK) - P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV)) / ZCORQSLIQ, 0.0_JPRB)
      ZEVAPLIMICE =  &
      & MAX((P_JPRB_STACK(JL, JD_ZQSICE + JK) - P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV)) / ZCORQSICE, 0.0_JPRB)
      
      !--------------------------------
      ! in-cloud consensate amount
      !--------------------------------
      ZTMPA = 1.0_JPRB / MAX(P_JPRB_STACK(JL, JD_ZA + JK), ZEPSEC)
      ZLIQCLD = P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQL)*ZTMPA
      ZICECLD = P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQI)*ZTMPA
      ...
      IF (P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQL) < YRECLDP%RLMIN) THEN
        ZSOLQA(NCLDQV, NCLDQL) = P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQL)
        ZSOLQA(NCLDQL, NCLDQV) = -P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQL)
      END IF
      
      IF (P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQI) < YRECLDP%RLMIN) THEN
        ZSOLQA(NCLDQV, NCLDQI) = P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQI)
        ZSOLQA(NCLDQI, NCLDQV) = -P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQI)
      END IF
      ...
      ZFOKOOP = FOKOOP(P_JPRB_STACK(JL, JD_ZTP1 + JK))
      
      IF (P_JPRB_STACK(JL, JD_ZTP1 + JK) >= RTT .or. YRECLDP%NSSOPT == 0) THEN
        ZFAC = 1.0_JPRB
        ZFACI = 1.0_JPRB
      ELSE
        ZFAC = P_JPRB_STACK(JL, JD_ZA + JK) + ZFOKOOP*(1.0_JPRB - P_JPRB_STACK(JL, JD_ZA + JK))
      ...
      IF (P_JPRB_STACK(JL, JD_ZA + JK) > 1.0_JPRB - YRECLDP%RAMIN) THEN
        ZSUPSAT =  &
        & MAX((P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) - ZFAC*P_JPRB_STACK(JL, JD_ZQSICE + JK)) / ZCORQSICE, 0.0_JPRB)
      ELSE
        ! Calculate environmental humidity supersaturation
        ZQP1ENV = (P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) - P_JPRB_STACK(JL, JD_ZA + JK)*P_JPRB_STACK(JL, JD_ZQSICE + &
        &  JK)) / MAX(1.0_JPRB - P_JPRB_STACK(JL, JD_ZA + JK), ZEPSILON)
        !& SIGN(MAX(ABS(1.0_JPRB-ZA(JL,JK)),ZEPSILON),1.0_JPRB-ZA(JL,JK))
        ZSUPSAT =  &
        & MAX((1.0_JPRB - P_JPRB_STACK(JL, JD_ZA + JK))*(ZQP1ENV - ZFAC*P_JPRB_STACK(JL, JD_ZQSICE + JK)) / ZCORQSICE, 0.0_JPRB)
      END IF
      ...
      IF (ZSUPSAT > ZEPSEC) THEN
        ...
        IF (P_JPRB_STACK(JL, JD_ZTP1 + JK) > YRECLDP%RTHOMO) THEN
        ...
        ZSOLAC = (1.0_JPRB - P_JPRB_STACK(JL, JD_ZA + JK))*ZFACI
...
!$loki loop seq
        ...
        ZDTDP = ZRDCP*0.5_JPRB*(P_JPRB_STACK(JL, JD_ZTP1 - 1 + JK) + P_JPRB_STACK(JL, JD_ZTP1 + JK)) / PAPH(JL, JK)
...
!$loki loop seq
      ...
      IF (P_JPRB_STACK(JL, JD_ZLI + JK) > ZEPSEC) THEN
        ...
        ZE = ZLDIFDT*MAX(P_JPRB_STACK(JL, JD_ZQSMIX + JK) - P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV), 0.0_JPRB)
        ZLEROS = P_JPRB_STACK(JL, JD_ZA + JK)*ZE
        ZLEROS = MIN(ZLEROS, ZEVAPLIMMIX)
        ZLEROS = MIN(ZLEROS, P_JPRB_STACK(JL, JD_ZLI + JK))
        ZAEROS = ZLEROS / ZLICLD          !if linear term
        
        ! Erosion is -ve LINEAR in L,A
        ZSOLAC = ZSOLAC - ZAEROS          !linear
        
        ZSOLQA(NCLDQV, NCLDQL) = ZSOLQA(NCLDQV, NCLDQL) + P_JPRB_STACK(JL, JD_ZLIQFRAC + JK)*ZLEROS
        ZSOLQA(NCLDQL, NCLDQV) = ZSOLQA(NCLDQL, NCLDQV) - P_JPRB_STACK(JL, JD_ZLIQFRAC + JK)*ZLEROS
        ZSOLQA(NCLDQV, NCLDQI) = ZSOLQA(NCLDQV, NCLDQI) + P_JPRB_STACK(JL, JD_ZICEFRAC + JK)*ZLEROS
        ZSOLQA(NCLDQI, NCLDQV) = ZSOLQA(NCLDQI, NCLDQV) - P_JPRB_STACK(JL, JD_ZICEFRAC + JK)*ZLEROS
      ...
      ZDTDP = ZRDCP*P_JPRB_STACK(JL, JD_ZTP1 + JK) / PAP(JL, JK)
      ZDPMXDT = ZDP*ZQTMST
      ZMFDN = 0.0_JPRB
      ...
      ZQOLD = P_JPRB_STACK(JL, JD_ZQSMIX + JK)
      ZTOLD = P_JPRB_STACK(JL, JD_ZTP1 + JK)
      P_JPRB_STACK(JL, JD_ZTP1 + JK) = P_JPRB_STACK(JL, JD_ZTP1 + JK) + ZDTFORC
      P_JPRB_STACK(JL, JD_ZTP1 + JK) = MAX(P_JPRB_STACK(JL, JD_ZTP1 + JK), 160.0_JPRB)
      LLFLAG = .true.
      
      ! Formerly a call to CUADJTQ(..., ICALL=5)
      ZQP = 1.0_JPRB / PAP(JL, JK)
      ZQSAT = FOEEWM(P_JPRB_STACK(JL, JD_ZTP1 + JK))*ZQP
      ZQSAT = MIN(0.5_JPRB, ZQSAT)
      ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZQSAT)
      ZQSAT = ZQSAT*ZCOR
      ZCOND = (P_JPRB_STACK(JL, JD_ZQSMIX + JK) - ZQSAT) / (1.0_JPRB + ZQSAT*ZCOR*FOEDEM(P_JPRB_STACK(JL, JD_ZTP1 + JK)))
      P_JPRB_STACK(JL, JD_ZTP1 + JK) = P_JPRB_STACK(JL, JD_ZTP1 + JK) + FOELDCPM(P_JPRB_STACK(JL, JD_ZTP1 + JK))*ZCOND
      P_JPRB_STACK(JL, JD_ZQSMIX + JK) = P_JPRB_STACK(JL, JD_ZQSMIX + JK) - ZCOND
      ZQSAT = FOEEWM(P_JPRB_STACK(JL, JD_ZTP1 + JK))*ZQP
      ZQSAT = MIN(0.5_JPRB, ZQSAT)
      ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZQSAT)
      ZQSAT = ZQSAT*ZCOR
      ZCOND1 = (P_JPRB_STACK(JL, JD_ZQSMIX + JK) - ZQSAT) / (1.0_JPRB + ZQSAT*ZCOR*FOEDEM(P_JPRB_STACK(JL, JD_ZTP1 + JK)))
      P_JPRB_STACK(JL, JD_ZTP1 + JK) = P_JPRB_STACK(JL, JD_ZTP1 + JK) + FOELDCPM(P_JPRB_STACK(JL, JD_ZTP1 + JK))*ZCOND1
      P_JPRB_STACK(JL, JD_ZQSMIX + JK) = P_JPRB_STACK(JL, JD_ZQSMIX + JK) - ZCOND1
      
      ZDQS = P_JPRB_STACK(JL, JD_ZQSMIX + JK) - ZQOLD
      P_JPRB_STACK(JL, JD_ZQSMIX + JK) = ZQOLD
      P_JPRB_STACK(JL, JD_ZTP1 + JK) = ZTOLD
      ...
      IF (ZDQS > 0.0_JPRB) THEN
        ...
        ZLEVAP = P_JPRB_STACK(JL, JD_ZA + JK)*MIN(ZDQS, ZLICLD)
        ZLEVAP = MIN(ZLEVAP, ZEVAPLIMMIX)
        ZLEVAP =  &
        & MIN(ZLEVAP, MAX(P_JPRB_STACK(JL, JD_ZQSMIX + JK) - P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV), 0.0_JPRB))
        
        ! For first guess call
        ZLEVAPL = P_JPRB_STACK(JL, JD_ZLIQFRAC + JK)*ZLEVAP
        ZLEVAPI = P_JPRB_STACK(JL, JD_ZICEFRAC + JK)*ZLEVAP
        
        ZSOLQA(NCLDQV, NCLDQL) = ZSOLQA(NCLDQV, NCLDQL) + P_JPRB_STACK(JL, JD_ZLIQFRAC + JK)*ZLEVAP
        ZSOLQA(NCLDQL, NCLDQV) = ZSOLQA(NCLDQL, NCLDQV) - P_JPRB_STACK(JL, JD_ZLIQFRAC + JK)*ZLEVAP
        
        ZSOLQA(NCLDQV, NCLDQI) = ZSOLQA(NCLDQV, NCLDQI) + P_JPRB_STACK(JL, JD_ZICEFRAC + JK)*ZLEVAP
        ZSOLQA(NCLDQI, NCLDQV) = ZSOLQA(NCLDQI, NCLDQV) - P_JPRB_STACK(JL, JD_ZICEFRAC + JK)*ZLEVAP
      ...
      IF (P_JPRB_STACK(JL, JD_ZA + JK) > ZEPSEC .and. ZDQS <= -YRECLDP%RLMIN) THEN
        
        ZLCOND1 = MAX(-ZDQS, 0.0_JPRB)          !new limiter
        
        !old limiter (significantly improves upper tropospheric humidity rms)
        IF (P_JPRB_STACK(JL, JD_ZA + JK) > 0.99_JPRB) THEN
          ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*P_JPRB_STACK(JL, JD_ZQSMIX + JK))
          ZCDMAX = (P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) - P_JPRB_STACK(JL, JD_ZQSMIX + JK)) / (1.0_JPRB +  &
          & ZCOR*P_JPRB_STACK(JL, JD_ZQSMIX + JK)*FOEDEM(P_JPRB_STACK(JL, JD_ZTP1 + JK)))
        ELSE
          ZCDMAX = (P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) - P_JPRB_STACK(JL, JD_ZA + JK)*P_JPRB_STACK(JL, JD_ZQSMIX  &
          & + JK)) / P_JPRB_STACK(JL, JD_ZA + JK)
        END IF
        ZLCOND1 = MAX(MIN(ZLCOND1, ZCDMAX), 0.0_JPRB)
        ! end old limiter
        
        ZLCOND1 = P_JPRB_STACK(JL, JD_ZA + JK)*ZLCOND1
      ...
      IF (ZDQS <= -YRECLDP%RLMIN .and. P_JPRB_STACK(JL, JD_ZA + JK) < 1.0_JPRB - ZEPSEC) THEN
        ...
        IF (YRECLDP%NSSOPT == 0) THEN
          ! No scheme
          ZQE = (P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) - P_JPRB_STACK(JL, JD_ZA + JK)*P_JPRB_STACK(JL, JD_ZQSICE +  &
          & JK)) / MAX(ZEPSEC, 1.0_JPRB - P_JPRB_STACK(JL, JD_ZA + JK))
        ...
        ELSE IF (YRECLDP%NSSOPT == 2) THEN
          ! Lohmann and Karcher
          ZQE = P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV)
        ELSE IF (YRECLDP%NSSOPT == 3) THEN
          ! Gierens
          ZQE = P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) + P_JPRB_STACK(JL, JD_ZLI + JK)
        END IF
        ...
        IF (ZQE >= ZRHC*P_JPRB_STACK(JL, JD_ZQSICE + JK)*ZFAC .and. ZQE < P_JPRB_STACK(JL, JD_ZQSICE + JK)*ZFAC) THEN
          ! note: not **2 on 1-a term if ZQE is used.
          ! Added correction term ZFAC to numerator 15/03/2010
          ZACOND = -(1.0_JPRB - P_JPRB_STACK(JL, JD_ZA + JK))*ZFAC*ZDQS / MAX(2.0_JPRB*(ZFAC*P_JPRB_STACK(JL, JD_ZQSICE + JK) -  &
          & ZQE), ZEPSEC)
          
          ZACOND = MIN(ZACOND, 1.0_JPRB - P_JPRB_STACK(JL, JD_ZA + JK))            !PUT THE LIMITER BACK
          ...
          ZZDL = 2.0_JPRB*(ZFAC*P_JPRB_STACK(JL, JD_ZQSICE + JK) - ZQE) / MAX(ZEPSEC, 1.0_JPRB - P_JPRB_STACK(JL, JD_ZA + JK))
          ! Added correction term ZFAC 15/03/2010
          IF (ZFAC*ZDQS < -ZZDL) THEN
            ! ZLCONDLIM=(ZA(JL,JK)-1.0_JPRB)*ZDQS(JL)-ZQSICE(JL,JK)+ZQX(JL,JK,NCLDQV)
            ZLCONDLIM = (P_JPRB_STACK(JL, JD_ZA + JK) - 1.0_JPRB)*ZFAC*ZDQS - ZFAC*P_JPRB_STACK(JL, JD_ZQSICE + JK) +  &
            & P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV)
            ZLCOND2 = MIN(ZLCOND2, ZLCONDLIM)
          END IF
          ZLCOND2 = MAX(ZLCOND2, 0.0_JPRB)
          
          IF (ZLCOND2 < YRECLDP%RLMIN .or. (1.0_JPRB - P_JPRB_STACK(JL, JD_ZA + JK)) < ZEPSEC) THEN
            ZLCOND2 = 0.0_JPRB
            ZACOND = 0.0_JPRB
          END IF
      ...
      IF (IDEPICE == 1) THEN
        ...
        IF (P_JPRB_STACK(JL, JD_ZA - 1 + JK) < YRECLDP%RCLDTOPCF .and. P_JPRB_STACK(JL, JD_ZA + JK) >= YRECLDP%RCLDTOPCF) THEN
          ZCLDTOPDIST = 0.0_JPRB
        ELSE
        ...
        !--------------------------------------------------------------
        IF (P_JPRB_STACK(JL, JD_ZTP1 + JK) < RTT .and. ZQXFG(NCLDQL) > YRECLDP%RLMIN) THEN
          ! T<273K
          
          ZVPICE = FOEEICE(P_JPRB_STACK(JL, JD_ZTP1 + JK))*RV / RD
          ...
          !------------------------------------------------
          ZADD = RLSTT*(RLSTT / (RV*P_JPRB_STACK(JL, JD_ZTP1 + JK)) - 1.0_JPRB) / (2.4E-2_JPRB*P_JPRB_STACK(JL, JD_ZTP1 + JK))
          ZBDD = RV*P_JPRB_STACK(JL, JD_ZTP1 + JK)*PAP(JL, JK) / (2.21_JPRB*ZVPICE)
          ...
          !---------------------------
          ZDEPOS = MAX(P_JPRB_STACK(JL, JD_ZA + JK)*(ZINEW - ZICE0), 0.0_JPRB)
          ...
          ZDEPOS = ZDEPOS*MIN(ZINFACTOR + (1.0_JPRB - ZINFACTOR)*(YRECLDP%RDEPLIQREFRATE + ZCLDTOPDIST / YRECLDP%RDEPLIQREFDEPTH) &
          & , 1.0_JPRB)
      ...
      ELSE IF (IDEPICE == 2) THEN
        ...
        IF (P_JPRB_STACK(JL, JD_ZTP1 + JK) < RTT .and. ZQXFG(NCLDQL) > YRECLDP%RLMIN) THEN
          ...
          ZAPLUSB = YRECLDP%RCL_APB1*ZVPICE - YRECLDP%RCL_APB2*ZVPICE*P_JPRB_STACK(JL, JD_ZTP1 + JK) + PAP(JL, JK) &
          & *YRECLDP%RCL_APB3*P_JPRB_STACK(JL, JD_ZTP1 + JK)**3._JPRB
          ZCORRFAC = (1.0_JPRB / ZRHO)**0.5_JPRB
          ZCORRFAC2 = ((P_JPRB_STACK(JL, JD_ZTP1 + JK) / 273.0_JPRB)**1.5_JPRB)*(393.0_JPRB / (P_JPRB_STACK(JL, JD_ZTP1 + JK) +  &
          & 120.0_JPRB))
          
          ZPR02 = ZRHO*ZICE0*YRECLDP%RCL_CONST1I / (ZTCG*ZFACX1I)
          
          ZTERM1 = (ZVPLIQ - ZVPICE)*P_JPRB_STACK(JL, JD_ZTP1 + JK)**2.0_JPRB*ZVPICE*ZCORRFAC2*ZTCG*YRECLDP%RCL_CONST2I*ZFACX1I / &
          &  (ZRHO*ZAPLUSB*ZVPICE)
          ...
          ZDEPOS = MAX(P_JPRB_STACK(JL, JD_ZA + JK)*ZTERM1*ZTERM2*PTSPHY, 0.0_JPRB)
...
!$loki loop seq
      DO JM=1,NCLV
        IF (LLFALL(JM) .or. JM == NCLDQI) THEN
          ...
          !------------------------
          IF (JK > YRECLDP%NCLDTOP) THEN
            ZFALLSRCE(JM) = P_JPRB_STACK(JL, JD_ZPFPLSX - 1 + JK + JM - KLEV + JM*KLEV)*ZDTGDP
      ...
      IF (ZQPRETOT > ZEPSEC) THEN
        ZCOVPTOT = 1.0_JPRB - ((1.0_JPRB - ZCOVPTOT)*(1.0_JPRB - MAX(P_JPRB_STACK(JL, JD_ZA + JK), P_JPRB_STACK(JL, JD_ZA - 1 +  &
        & JK))) / (1.0_JPRB - MIN(P_JPRB_STACK(JL, JD_ZA - 1 + JK), 1.0_JPRB - 1.E-06_JPRB)))
        ZCOVPTOT = MAX(ZCOVPTOT, YRECLDP%RCOVPMIN)
        ZCOVPCLR = MAX(0.0_JPRB, ZCOVPTOT - P_JPRB_STACK(JL, JD_ZA + JK))          ! clear sky proportion
      ...
      IF (P_JPRB_STACK(JL, JD_ZTP1 + JK) <= RTT) THEN
        !-----------------------------------------------------
        !     Snow Autoconversion rate follow Lin et al. 1983
        !-----------------------------------------------------
        IF (ZICECLD > ZEPSEC) THEN
          
          ZZCO = PTSPHY*YRECLDP%RSNOWLIN1*EXP(YRECLDP%RSNOWLIN2*(P_JPRB_STACK(JL, JD_ZTP1 + JK) - RTT))
      ...
      IF (ZLIQCLD > ZEPSEC) THEN
        ...
        IF (IWARMRAIN == 1) THEN
          ...
          !------------------------------------------------------------------
          ZPRECIP = (P_JPRB_STACK(JL, JD_ZPFPLSX - 1 + JK + NCLDQS - KLEV + KLEV*NCLDQS) + P_JPRB_STACK(JL, JD_ZPFPLSX - 1 + JK + &
          &  NCLDQR - KLEV + KLEV*NCLDQR)) / MAX(ZEPSEC, ZCOVPTOT)
        ...
        ELSE IF (IWARMRAIN == 2) THEN
          ...
          IF (ZLIQCLD > ZLCRIT) THEN
            ...
            ZRAINAUT = 1.5_JPRB*P_JPRB_STACK(JL, JD_ZA + JK) &
            & *PTSPHY*YRECLDP%RCL_KKAAU*ZLIQCLD**YRECLDP%RCL_KKBAUQ*ZCONST**YRECLDP%RCL_KKBAUN
            
            ZRAINAUT = MIN(ZRAINAUT, ZQXFG(NCLDQL))
            IF (ZRAINAUT < ZEPSEC) ZRAINAUT = 0.0_JPRB
            
            ZRAINACC = 2.0_JPRB*P_JPRB_STACK(JL, JD_ZA + JK)*PTSPHY*YRECLDP%RCL_KKAAC*(ZLIQCLD*ZRAINCLD)**YRECLDP%RCL_KKBAC
      ...
      IF (IWARMRAIN > 1) THEN
        ...
        IF (P_JPRB_STACK(JL, JD_ZTP1 + JK) <= RTT .and. ZLIQCLD > ZEPSEC) THEN
      ...
      IF (ZICETOT > ZEPSEC .and. P_JPRB_STACK(JL, JD_ZTP1 + JK) > RTT) THEN
        
        ! Calculate subsaturation
        ZSUBSAT = MAX(P_JPRB_STACK(JL, JD_ZQSICE + JK) - P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV), 0.0_JPRB)
        ...
        ! due to evaporation.
        ZTDMTW0 = P_JPRB_STACK(JL, JD_ZTP1 + JK) - RTT - ZSUBSAT*(ZTW1 + ZTW2*(PAP(JL, JK) - ZTW3) - ZTW4*(P_JPRB_STACK(JL,  &
        & JD_ZTP1 + JK) - ZTW5))
        ! Not implicit yet...
...
!$loki loop seq
      ...
      ! If rain present
      IF (P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQR) > ZEPSEC) THEN
        
        IF (P_JPRB_STACK(JL, JD_ZTP1 + JK) <= RTT .and. P_JPRB_STACK(JL, JD_ZTP1 - 1 + JK) > RTT) THEN
          ...
          ZQPRETOT =  &
          & MAX(P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQS) + P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQR), ZEPSEC)
          PRAINFRAC_TOPRFZ(JL) = P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQR) / ZQPRETOT
        ...
        IF (P_JPRB_STACK(JL, JD_ZTP1 + JK) < RTT) THEN
          ...
          IF (PRAINFRAC_TOPRFZ(JL) > 0.8) THEN
            ...
            ZLAMBDA = (YRECLDP%RCL_FAC1 / (ZRHO*P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQR)))**YRECLDP%RCL_FAC2
            
            ! Calculate freezing rate based on Bigg(1953) and Wisner(1972)
            ZTEMP = YRECLDP%RCL_FZRAB*(P_JPRB_STACK(JL, JD_ZTP1 + JK) - RTT)
          ...
          ELSE
            ...
            ZCONS1 = ABS(PTSPHY*(1.0_JPRB + 0.5_JPRB*(RTT - P_JPRB_STACK(JL, JD_ZTP1 + JK))) / YRECLDP%RTAUMEL)
            ZFRZMAX = MAX((RTT - P_JPRB_STACK(JL, JD_ZTP1 + JK))*ZCONS1*ZRLDCP, 0.0_JPRB)
            
          END IF
          
          IF (ZFRZMAX > ZEPSEC) THEN
            ZFRZ = MIN(P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQR), ZFRZMAX)
      ...
      !----------------------------------------------------------------------
      ! not implicit yet...
      ZFRZMAX = MAX((YRECLDP%RTHOMO - P_JPRB_STACK(JL, JD_ZTP1 + JK))*ZRLDCP, 0.0_JPRB)
      ...
      IF (IEVAPRAIN == 1) THEN
        ...
        ZZRH =  &
        & YRECLDP%RPRECRHMAX + (1.0_JPRB - YRECLDP%RPRECRHMAX)*ZCOVPMAX / MAX(ZEPSEC, 1.0_JPRB - P_JPRB_STACK(JL, JD_ZA + JK))
        ZZRH = MIN(MAX(ZZRH, YRECLDP%RPRECRHMAX), 1.0_JPRB)
        
        ZQE = (P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) - P_JPRB_STACK(JL, JD_ZA + JK)*P_JPRB_STACK(JL, JD_ZQSLIQ + JK) &
        & ) / MAX(ZEPSEC, 1.0_JPRB - P_JPRB_STACK(JL, JD_ZA + JK))
        !---------------------------------------------
        ! humidity in moistest ZCOVPCLR part of domain
        !---------------------------------------------
        ZQE = MAX(0.0_JPRB, MIN(ZQE, P_JPRB_STACK(JL, JD_ZQSLIQ + JK)))
        LLO1 = ZCOVPCLR > ZEPSEC .and. ZQXFG(NCLDQR) > ZEPSEC .and. ZQE < ZZRH*P_JPRB_STACK(JL, JD_ZQSLIQ + JK)
        ...
        IF (LLO1) THEN
          ...
          ZDPR = ZCOVPCLR*ZBETA*(P_JPRB_STACK(JL, JD_ZQSLIQ + JK) - ZQE) / ZDENOM*ZDP*ZRG_R
          ZDPEVAP = ZDPR*ZDTGDP
          ...
          !-------------------------------------------------------------
          ZCOVPTOT =  &
          & MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, (ZCOVPTOT - P_JPRB_STACK(JL, JD_ZA + JK))*ZEVAP / ZQXFG(NCLDQR)))
      ...
      ELSE IF (IEVAPRAIN == 2) THEN
        ...
        ZQE = MAX(0.0_JPRB, MIN(P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV), P_JPRB_STACK(JL, JD_ZQSLIQ + JK)))
        ...
        IF (LLO1) THEN
          ...
          ZESATLIQ = RV / RD*FOEELIQ(P_JPRB_STACK(JL, JD_ZTP1 + JK))
          
          ! Slope of particle size distribution
          ZLAMBDA = (YRECLDP%RCL_FAC1 / (ZRHO*ZPRECLR))**YRECLDP%RCL_FAC2            ! ZPRECLR=kg/kg
          
          ZEVAP_DENOM = YRECLDP%RCL_CDENOM1*ZESATLIQ - YRECLDP%RCL_CDENOM2*P_JPRB_STACK(JL, JD_ZTP1 + JK)*ZESATLIQ +  &
          & YRECLDP%RCL_CDENOM3*P_JPRB_STACK(JL, JD_ZTP1 + JK)**3._JPRB*PAP(JL, JK)
          
          ! Temperature dependent conductivity
          ZCORR2 =  &
          & (P_JPRB_STACK(JL, JD_ZTP1 + JK) / 273._JPRB)**1.5_JPRB*393._JPRB / (P_JPRB_STACK(JL, JD_ZTP1 + JK) + 120._JPRB)
          ZKA = YRECLDP%RCL_KA273*ZCORR2
          
          ZSUBSAT = MAX(ZZRH*P_JPRB_STACK(JL, JD_ZQSLIQ + JK) - ZQE, 0.0_JPRB)
          
          ZBETA = (0.5_JPRB / P_JPRB_STACK(JL, JD_ZQSLIQ + JK))*P_JPRB_STACK(JL, JD_ZTP1 + JK) &
          & **2._JPRB*ZESATLIQ*YRECLDP%RCL_CONST1R*(ZCORR2 / ZEVAP_DENOM)*(0.78_JPRB / (ZLAMBDA**YRECLDP%RCL_CONST4R) +  &
          & YRECLDP%RCL_CONST2R*(ZRHO*ZFALLCORR)**0.5_JPRB / (ZCORR2**0.5_JPRB*ZLAMBDA**YRECLDP%RCL_CONST3R))
      ...
      IF (IEVAPSNOW == 1) THEN
        ...
        & YRECLDP%RPRECRHMAX + (1.0_JPRB - YRECLDP%RPRECRHMAX)*ZCOVPMAX / MAX(ZEPSEC, 1.0_JPRB - P_JPRB_STACK(JL, JD_ZA + JK))
        ZZRH = MIN(MAX(ZZRH, YRECLDP%RPRECRHMAX), 1.0_JPRB)
        ZQE = (P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV) - P_JPRB_STACK(JL, JD_ZA + JK)*P_JPRB_STACK(JL, JD_ZQSICE + JK) &
        & ) / MAX(ZEPSEC, 1.0_JPRB - P_JPRB_STACK(JL, JD_ZA + JK))
        
        !---------------------------------------------
        ! humidity in moistest ZCOVPCLR part of domain
        !---------------------------------------------
        ZQE = MAX(0.0_JPRB, MIN(ZQE, P_JPRB_STACK(JL, JD_ZQSICE + JK)))
        LLO1 = ZCOVPCLR > ZEPSEC .and. ZQXFG(NCLDQS) > ZEPSEC .and. ZQE < ZZRH*P_JPRB_STACK(JL, JD_ZQSICE + JK)
        ...
        IF (LLO1) THEN
          ...
          ZDPR = ZCOVPCLR*ZBETA*(P_JPRB_STACK(JL, JD_ZQSICE + JK) - ZQE) / ZDENOM*ZDP*ZRG_R
          ZDPEVAP = ZDPR*ZDTGDP
          ...
          !-------------------------------------------------------------
          ZCOVPTOT =  &
          & MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, (ZCOVPTOT - P_JPRB_STACK(JL, JD_ZA + JK))*ZEVAP / ZQXFG(NCLDQS)))
      ...
      ELSE IF (IEVAPSNOW == 2) THEN
        ...
        !---------------------------------------------
        ZQE = MAX(0.0_JPRB, MIN(ZQE, P_JPRB_STACK(JL, JD_ZQSICE + JK)))
        LLO1 = ZCOVPCLR > ZEPSEC .and. P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQS) > ZEPSEC .and. ZQE <  &
        & ZZRH*P_JPRB_STACK(JL, JD_ZQSICE + JK)
        
        IF (LLO1) THEN
          
          ! Calculate local precipitation (kg/kg)
          ZPRECLR = P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQS) / ZCOVPTOT
          ZVPICE = FOEEICE(P_JPRB_STACK(JL, JD_ZTP1 + JK))*RV / RD
          ...
          ZAPLUSB = YRECLDP%RCL_APB1*ZVPICE - YRECLDP%RCL_APB2*ZVPICE*P_JPRB_STACK(JL, JD_ZTP1 + JK) + PAP(JL, JK) &
          & *YRECLDP%RCL_APB3*P_JPRB_STACK(JL, JD_ZTP1 + JK)**3
          ZCORRFAC = (1.0 / ZRHO)**0.5
          ZCORRFAC2 = ((P_JPRB_STACK(JL, JD_ZTP1 + JK) / 273.0)**1.5)*(393.0 / (P_JPRB_STACK(JL, JD_ZTP1 + JK) + 120.0))
          
          ZPR02 = ZRHO*ZPRECLR*YRECLDP%RCL_CONST1S / (ZTCG*ZFACX1S)
          
          ZTERM1 = (P_JPRB_STACK(JL, JD_ZQSICE + JK) - ZQE)*P_JPRB_STACK(JL, JD_ZTP1 + JK) &
          & **2*ZVPICE*ZCORRFAC2*ZTCG*YRECLDP%RCL_CONST2S*ZFACX1S / (ZRHO*ZAPLUSB*P_JPRB_STACK(JL, JD_ZQSICE + JK))
          ...
          ZEVAP = MIN(ZEVAP, P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQS))
          ...
          ! 2-flux treatment, abandoned due to the new prognostic precip
          !-------------------------------------------------------------
          ZCOVPTOT = MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, (ZCOVPTOT - P_JPRB_STACK(JL, JD_ZA + JK))*ZEVAP /  &
          & P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQS)))
...
!$loki loop seq
      ...
      !---------------------------
      ZANEW = (P_JPRB_STACK(JL, JD_ZA + JK) + ZSOLAC) / (1.0_JPRB + ZSOLAB)
      ZANEW = MIN(ZANEW, 1.0_JPRB)
      IF (ZANEW < YRECLDP%RAMIN) ZANEW = 0.0_JPRB
      ZDA = ZANEW - P_JPRB_STACK(JL, JD_ZAORIG + JK)
      !---------------------------------
...
!$loki loop seq
      DO JM=1,NCLV
        ZMAX = MAX(P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + JM*KLEV), ZEPSEC)
...
!$loki loop seq
        ...
        !---------------------------
        ZMM = MAX(P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + JM*KLEV), ZEPSEC)
...
!$loki loop seq
        ...
        END DO
        ZQXN(JM) = P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + JM*KLEV) + ZEXPLICIT
      END DO
...
!$loki loop seq
      DO JM=1,NCLV
        ZQXNM1(JM) = ZQXN(JM)
        P_JPRB_STACK(JL, JD_ZQXN2D + JK - KLEV + JM*KLEV) = ZQXN(JM)
      END DO
...
!$loki loop seq
      DO JM=1,NCLV
        P_JPRB_STACK(JL, JD_ZPFPLSX + JK + JM - KLEV + JM*KLEV) = ZFALLSINK(JM)*ZQXN(JM)*ZRDTGDP
      END DO
      
      ! Ensure precipitation fraction is zero if no precipitation
      ZQPRETOT = P_JPRB_STACK(JL, JD_ZPFPLSX + JK + NCLDQS - KLEV + KLEV*NCLDQS) + P_JPRB_STACK(JL, JD_ZPFPLSX + JK + NCLDQR -  &
      & KLEV + KLEV*NCLDQR)
      IF (ZQPRETOT < ZEPSEC) THEN
        ZCOVPTOT = 0.0_JPRB
      END IF
...
!$loki loop seq
      DO JM=1,NCLV - 1
        ...
        IF (IPHASE(JM) == 1) THEN
          ...
          & TENDENCY_LOC_T(JL, JK) + RALVDCP*(ZQXN(JM) - P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + JM*KLEV) - ZFLUXQ(JM))*ZQTMST
        END IF
        
        IF (IPHASE(JM) == 2) THEN
          TENDENCY_LOC_T(JL, JK) =  &
          & TENDENCY_LOC_T(JL, JK) + RALSDCP*(ZQXN(JM) - P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + JM*KLEV) - ZFLUXQ(JM))*ZQTMST
        END IF
        ...
        !----------------------------------------------------------------------
        TENDENCY_LOC_CLD(JL, JK, JM) =  &
        & TENDENCY_LOC_CLD(JL, JK, JM) + (ZQXN(JM) - P_JPRB_STACK(JL, JD_ZQX0 + JK - KLEV + JM*KLEV))*ZQTMST
      ...
      & TENDENCY_LOC_Q(JL, JK) + (ZQXN(NCLDQV) - P_JPRB_STACK(JL, JD_ZQX + JK - KLEV + KLEV*NCLDQV))*ZQTMST
...
!$loki loop seq
    DO JK=1,KLEV + 1
      PFPLSL(JL, JK) = P_JPRB_STACK(JL, JD_ZPFPLSX - 1 + JK + NCLDQR - KLEV + KLEV*NCLDQR) + P_JPRB_STACK(JL, JD_ZPFPLSX - 1 + JK &
      &  + NCLDQL - KLEV + KLEV*NCLDQL)
      PFPLSN(JL, JK) = P_JPRB_STACK(JL, JD_ZPFPLSX - 1 + JK + NCLDQS - KLEV + KLEV*NCLDQS) + P_JPRB_STACK(JL, JD_ZPFPLSX - 1 + JK &
      &  + NCLDQI - KLEV + KLEV*NCLDQI)
    END DO
...
!$loki loop seq
    DO JK=1,KLEV
      ...
      PFSQLF(JL, JK + 1) = PFSQLF(JL, JK + 1) + (P_JPRB_STACK(JL, JD_ZQXN2D + JK - KLEV + KLEV*NCLDQL) - P_JPRB_STACK(JL, JD_ZQX0 &
      &  + JK - KLEV + KLEV*NCLDQL) + PVFL(JL, JK)*PTSPHY - ZALFAW*PLUDE(JL, JK))*ZGDPH_R
      ! liquid, negative numbers
      PFCQLNG(JL, JK + 1) = PFCQLNG(JL, JK + 1) + P_JPRB_STACK(JL, JD_ZLNEG + JK - KLEV + KLEV*NCLDQL)*ZGDPH_R
      
      ! liquid, vertical diffusion
      PFSQLTUR(JL, JK + 1) = PFSQLTUR(JL, JK + 1) + PVFL(JL, JK)*PTSPHY*ZGDPH_R
      
      ! Rain, LS scheme
      PFSQRF(JL, JK + 1) = PFSQRF(JL, JK + 1) + (P_JPRB_STACK(JL, JD_ZQXN2D + JK - KLEV + KLEV*NCLDQR) - P_JPRB_STACK(JL, JD_ZQX0 &
      &  + JK - KLEV + KLEV*NCLDQR))*ZGDPH_R
      ! rain, negative numbers
      PFCQRNG(JL, JK + 1) = PFCQRNG(JL, JK + 1) + P_JPRB_STACK(JL, JD_ZLNEG + JK - KLEV + KLEV*NCLDQR)*ZGDPH_R
      
      ! Ice , LS scheme minus detrainment
      PFSQIF(JL, JK + 1) = PFSQIF(JL, JK + 1) + (P_JPRB_STACK(JL, JD_ZQXN2D + JK - KLEV + KLEV*NCLDQI) - P_JPRB_STACK(JL, JD_ZQX0 &
      &  + JK - KLEV + KLEV*NCLDQI) + PVFI(JL, JK)*PTSPHY - (1.0_JPRB - ZALFAW)*PLUDE(JL, JK))*ZGDPH_R
      ! ice, negative numbers
      PFCQNNG(JL, JK + 1) = PFCQNNG(JL, JK + 1) + P_JPRB_STACK(JL, JD_ZLNEG + JK - KLEV + KLEV*NCLDQI)*ZGDPH_R
      
      ! ice, vertical diffusion
      PFSQITUR(JL, JK + 1) = PFSQITUR(JL, JK + 1) + PVFI(JL, JK)*PTSPHY*ZGDPH_R
      
      ! snow, LS scheme
      PFSQSF(JL, JK + 1) = PFSQSF(JL, JK + 1) + (P_JPRB_STACK(JL, JD_ZQXN2D + JK - KLEV + KLEV*NCLDQS) - P_JPRB_STACK(JL, JD_ZQX0 &
      &  + JK - KLEV + KLEV*NCLDQS))*ZGDPH_R
      ! snow, negative numbers
      PFCQSNG(JL, JK + 1) = PFCQSNG(JL, JK + 1) + P_JPRB_STACK(JL, JD_ZLNEG + JK - KLEV + KLEV*NCLDQS)*ZGDPH_R
    END DO
