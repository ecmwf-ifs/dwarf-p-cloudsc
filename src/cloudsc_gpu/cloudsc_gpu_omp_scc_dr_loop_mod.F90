...
MODULE CLOUDSC_GPU_OMP_SCC_DR_LOOP_MOD
!  USE NLEV_MOD, ONLY : NLEV

! $omp declare target(cloudsc_scc_cuf)

CONTAINS
!ATTRIBUTES(GLOBAL)  
SUBROUTINE CLOUDSC_SCC_CUF (NGPTOT, NPROMA, KIDIA, KFDIA, NLEV, KLON, KGPBLKS, PTSPHY, PT, PQ,  & !! 8
  ...
  & PFCQSNG, PFSQLTUR, PFSQITUR, PFPLSL, PFPLSN, PFHPSL, PFHPSN, YRECLDP )
    ...
    INTEGER(KIND=JPIM), INTENT(IN), VALUE :: KLON, NLEV    ! Number of grid points
...
#include "fccld.func.h"
    ...
    ! &
    ! $omp map(alloc: &
    ! $omp   ZLCUST, IPHASE, IMELT, LLFALL, LLINDEX1, LLINDEX3, & 
    ! $omp   IORDER, ZQXN, ZQXFG, ZQXNM1, ZFLUXQ, ZSOLQA, ZSOLQB, ZQLHS, ZVQX, &
    ! $omp   ZRATIO, ZSINKSUM, ZFALLSINK, ZFALLSRCE, ZCONVSRCE, ZCONVSINK, ZPSUPSATSRCE) 
 ...
 ! $omp parallel do simd private(JL, JBLK) collapse(2) 
 ! $omp target teams distribute parallel do simd private(JL, JBLK) collapse(2)
   ! $omp target teams distribute parallel do simd private(ZLCUST, IPHASE, IMELT, LLFALL, LLINDEX1, LLINDEX3, IORDER, ZQXN, ZQXFG, ZQXNM1, ZFLUXQ, ZSOLQA, ZSOLQB, ZQLHS, ZVQX, ZRATIO, ZSINKSUM, ZFALLSINK, ZFALLSRCE, ZCONVSRCE, ZCONVSINK, ZPSUPSATSRCE)
   !$omp target teams distribute parallel do simd collapse(2) private(JL, JBLK, ZFOEALFA, ZTP1, ZLCUST, ZLI, ZA, ZAORIG, IPHASE, IMELT, LLFALL, LLINDEX1, &
...
END MODULE CLOUDSC_GPU_OMP_SCC_DR_LOOP_MOD
