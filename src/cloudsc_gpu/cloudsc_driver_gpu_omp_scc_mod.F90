...
CONTAINS
  ...
  SUBROUTINE CLOUDSC_DRIVER_GPU_SCC( &
    ...
    REAL(KIND=JPRB) :: ZFOEALFA(NPROMA, NLEV + 1)
    REAL(KIND=JPRB) :: ZTP1(NPROMA, NLEV)
    REAL(KIND=JPRB) :: ZLI(NPROMA, NLEV), ZA(NPROMA, NLEV)
    REAL(KIND=JPRB) :: ZAORIG(NPROMA, NLEV)
    REAL(KIND=JPRB) :: ZLIQFRAC(NPROMA, NLEV)
    REAL(KIND=JPRB) :: ZICEFRAC(NPROMA, NLEV)
    REAL(KIND=JPRB) :: ZQX(NPROMA, NLEV, NCLV)
    REAL(KIND=JPRB) :: ZQX0(NPROMA, NLEV, NCLV)
    REAL(KIND=JPRB) :: ZPFPLSX(NPROMA, NLEV + 1, NCLV)
    REAL(KIND=JPRB) :: ZLNEG(NPROMA, NLEV, NCLV)
    REAL(KIND=JPRB) :: ZQXN2D(NPROMA, NLEV, NCLV)
    REAL(KIND=JPRB) :: ZQSMIX(NPROMA, NLEV)
    REAL(KIND=JPRB) :: ZQSLIQ(NPROMA, NLEV)
    REAL(KIND=JPRB) :: ZQSICE(NPROMA, NLEV)
    REAL(KIND=JPRB) :: ZFOEEWMT(NPROMA, NLEV)
    REAL(KIND=JPRB) :: ZFOEEW(NPROMA, NLEV)
    REAL(KIND=JPRB) :: ZFOEELIQT(NPROMA, NLEV)
...
!!$omp parallel do private(ZLCUST, IPHASE, IMELT, LLFALL, LLINDEX1, LLINDEX3, IORDER, ZQXN, ZQXFG, ZQXNM1, ZFLUXQ, ZSOLQA, ZSOLQB, ZQLHS, ZVQX, ZRATIO, ZSINKSUM, ZFALLSINK, ZFALLSRCE, ZCONVSRCE, ZCONVSINK, ZPSUPSATSRCE)
      DO JL=1, ICEND
