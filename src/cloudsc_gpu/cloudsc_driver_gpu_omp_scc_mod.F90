...
CONTAINS
  ...
  SUBROUTINE CLOUDSC_DRIVER_GPU_SCC( &
    ...
    ! now created beforehand without using OMP private clause
    REAL(KIND=JPRB) :: ZLCUST(NPROMA, NCLV, NGPBLKS)
    INTEGER(KIND=JPIM) :: IPHASE(NPROMA, NCLV, NGPBLKS)
    INTEGER(KIND=JPIM) :: IMELT(NPROMA, NCLV, NGPBLKS)
    LOGICAL :: LLFALL(NPROMA, NCLV, NGPBLKS)
    LOGICAL :: LLINDEX1(NPROMA, NCLV, NGPBLKS)
    LOGICAL :: LLINDEX3(NPROMA, NCLV, NCLV, NGPBLKS) !
    INTEGER(KIND=JPIM) :: IORDER(NPROMA, NCLV, NGPBLKS)
    REAL(KIND=JPRB) :: ZQXN(NPROMA, NCLV, NGPBLKS)
    REAL(KIND=JPRB) :: ZQXFG(NPROMA, NCLV, NGPBLKS)
    REAL(KIND=JPRB) :: ZQXNM1(NPROMA, NCLV, NGPBLKS)
    REAL(KIND=JPRB) :: ZFLUXQ(NPROMA, NCLV, NGPBLKS)
    REAL(KIND=JPRB) :: ZSOLQA(NPROMA, NCLV, NCLV, NGPBLKS) !
    REAL(KIND=JPRB) :: ZSOLQB(NPROMA, NCLV, NCLV, NGPBLKS) !
    REAL(KIND=JPRB) :: ZQLHS(NPROMA, NCLV, NCLV, NGPBLKS) !
    REAL(KIND=JPRB) :: ZVQX(NPROMA, NCLV, NGPBLKS)
    REAL(KIND=JPRB) :: ZRATIO(NPROMA, NCLV, NGPBLKS)
    REAL(KIND=JPRB) :: ZSINKSUM(NPROMA, NCLV, NGPBLKS)
    REAL(KIND=JPRB) :: ZFALLSINK(NPROMA, NCLV, NGPBLKS)
    REAL(KIND=JPRB) :: ZFALLSRCE(NPROMA, NCLV, NGPBLKS)
    REAL(KIND=JPRB) :: ZCONVSRCE(NPROMA, NCLV, NGPBLKS)
    REAL(KIND=JPRB) :: ZCONVSINK(NPROMA, NCLV, NGPBLKS)
    REAL(KIND=JPRB) :: ZPSUPSATSRCE(NPROMA, NCLV, NGPBLKS)
...
1003 format(5x,'NUMPROC=',i0,', NUMOMP=',i0,', NGPTOTG=',i0,', NPROMA=',i0,', NGPBLKS=',i0)
    ...
    RTICE_=RTICE
    RTICECU_=RTICECU
    RTWAT_RTICE_R_=RTWAT_RTICE_R
    RTWAT_RTICECU_R_=RTWAT_RTICECU_R
    RKOOP1_=RKOOP1
    RKOOP2_=RKOOP2
    NCLDQV_=NCLDQV
    NCLDQL_=NCLDQL
...
!$omp map(alloc: &
!$omp   ZLCUST, IPHASE, IMELT, LLFALL, LLINDEX1, LLINDEX3, IORDER, ZQXN, ZQXFG, &
!$omp   ZQXNM1, ZFLUXQ, ZSOLQA, ZSOLQB, ZQLHS, ZVQX, ZRATIO, ZSINKSUM, ZFALLSINK, &
!$omp   ZFALLSRCE, ZCONVSRCE, ZCONVSINK, ZPSUPSATSRCE)
...
!$omp parallel do
      DO JL=1, ICEND
       ...
       CALL CLOUDSC_SCC &
        ...
        & ZLCUST(:,:,IBL), IPHASE(:,:,IBL), IMELT(:,:,IBL), LLFALL(:,:,IBL), &
        & LLINDEX1(:,:,IBL), LLINDEX3(:,:,:,IBL), IORDER(:,:,IBL), ZQXN(:,:,IBL), &
        & ZQXFG(:,:,IBL), ZQXNM1(:,:,IBL), ZFLUXQ(:,:,IBL), ZSOLQA(:,:,:,IBL), &
        & ZSOLQB(:,:,:,IBL), ZQLHS(:,:,:,IBL), ZVQX(:,:,IBL), ZRATIO(:,:,IBL), &
        & ZSINKSUM(:,:,IBL), ZFALLSINK(:,:,IBL), ZFALLSRCE(:,:,IBL), &
        & ZCONVSRCE(:,:,IBL), ZCONVSINK(:,:,IBL), ZPSUPSATSRCE(:,:,IBL), &
