...
!...Timing variables
REAL(KIND=JPRB) :: T0, T1, T2, T3, T4, INIT_TIME
REAL(KIND=JPRB), ALLOCATABLE :: BUFFER(:,:,:)
INTEGER(KIND=JPIM) :: ITER, NITER, BUFFER_BLKS
...
OPEN(UNIT=12, FILE='timings.dat', STATUS="REPLACE", ACTION="WRITE")
CLOSE(12)

NITER = 200
DO ITER=1,NITER
   BUFFER_BLKS = 10*ITER*GLOBAL_STATE%NBLOCKS
   ALLOCATE(BUFFER(NPROMA,GLOBAL_STATE%KLEV,BUFFER_BLKS))
   BUFFER = 0._JPRB
   ...
   INIT_TIME = OMP_GET_WTIME() - T4
   
   T2 = OMP_GET_WTIME()
   ! Call the driver to perform the parallel loop over our kernel
   CALL CLOUDSC_DRIVER_GPU_SCC_FIELD( &
        & NUMOMP, NPROMA, GLOBAL_STATE%KLEV, NGPTOT, GLOBAL_STATE%NBLOCKS, NGPTOTG, &
        & GLOBAL_STATE%KFLDX, GLOBAL_STATE%PTSPHY, GLOBAL_STATE, BUFFER_BLKS, BUFFER, USE_PACKED &
        & )
   T3 = OMP_GET_WTIME()
   ...
   OPEN(UNIT=12, FILE='timings.dat', POSITION="APPEND", STATUS="OLD", ACTION="WRITE")
     WRITE (12, '(I4,F10.3,F10.3,F10.3,F10.3)') ITER, INIT_TIME, T1-T0, T3-T2, T1-T4
   CLOSE(12)

   PRINT '(A,I4,A,F10.3,A,F10.3,A,F10.3,A,F10.3)', "ITERATION: ", ITER, " INIT TIME (s): ", INIT_TIME, &
   &                                               " FINAL TIME (s): ", T1-T0, " KERNEL TIME (s): ", T3-T2, &
   &                                               " TOTAL TIME (s): ", T1-T4
   PRINT *
ENDDO
