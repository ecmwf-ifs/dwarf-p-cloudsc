...
#ifdef CLOUDSC_GPU_SCC_DBLK
USE CLOUDSC_DRIVER_GPU_SCC_DBLK_MOD, ONLY: CLOUDSC_DRIVER_GPU_SCC_DBLK
#endif
...
#ifdef CLOUDSC_GPU_SCC_DBLK_K_CACHING
USE CLOUDSC_DRIVER_GPU_SCC_DBLK_K_CACHING_MOD, ONLY: CLOUDSC_DRIVER_GPU_SCC_DBLK_K_CACHING
#endif
...
#ifdef CLOUDSC_GPU_SCC_DBLK
print '(1X,A42)', 'Executing CLOUDSC-GPU, "SCC-DBLK" variant...'

 ! Call the driver to perform the parallel loop over our kernel
CALL CLOUDSC_DRIVER_GPU_SCC_DBLK(NUMOMP, NPROMA, GLOBAL_STATE%KLEV, NGPTOT, GLOBAL_STATE%NBLOCKS, NGPTOTG, &
     & GLOBAL_STATE%KFLDX, GLOBAL_STATE%PTSPHY, &
     & GLOBAL_STATE%PT, GLOBAL_STATE%PQ, &
     & GLOBAL_STATE%B_CML,   GLOBAL_STATE%B_TMP, GLOBAL_STATE%B_LOC, &
     & GLOBAL_STATE%PVFA,    GLOBAL_STATE%PVFL,  GLOBAL_STATE%PVFI, &
...
#if defined(CLOUDSC_GPU_SCC_DBLK_K_CACHING)
print '(1X,A42)', 'Executing CLOUDSC-GPU, "SCC-dblk-k-caching" variant...'

 ! Call the driver to perform the parallel loop over our kernel
CALL CLOUDSC_DRIVER_GPU_SCC_DBLK_K_CACHING(NUMOMP, NPROMA, GLOBAL_STATE%KLEV, NGPTOT, GLOBAL_STATE%NBLOCKS, NGPTOTG, &
     & GLOBAL_STATE%KFLDX, GLOBAL_STATE%PTSPHY, &
     & GLOBAL_STATE%PT, GLOBAL_STATE%PQ, &
     & GLOBAL_STATE%B_CML,   GLOBAL_STATE%B_TMP, GLOBAL_STATE%B_LOC, &
     & GLOBAL_STATE%PVFA,    GLOBAL_STATE%PVFL,  GLOBAL_STATE%PVFI, &
