...
MODULE CLOUDSC_GPU_SCC_CUF_K_CACHING_MOD
  
  CONTAINS
  
  ATTRIBUTES(GLOBAL) SUBROUTINE CLOUDSC_SCC_CUF_K_CACHING (KIDIA, KFDIA, KLON, KLEV, PTSPHY, PT, PQ, TENDENCY_TMP, TENDENCY_LOC, PVFA,  &
  ...
  & PFSQRF, PFSQSF, PFCQRNG, PFCQSNG, PFSQLTUR, PFSQITUR, PFPLSL, PFPLSN, PFHPSL, PFHPSN, YRECLDP, NGPBLKS)
    ...
    IF (IBL <= NGPBLKS .and. JL <= KLON) THEN
      ...
      ZFOEEWMT = MIN(FOEEWM(ZTP1(JK_I)) / PAP(JL, JK, IBL), 0.5_JPRB)
      ZQSMIX = ZFOEEWMT
      ...
      ZFOEEW =  &
      & MIN((ZALFA*FOEELIQ(ZTP1(JK_I)) + (1.0_JPRB - ZALFA)*FOEEICE(ZTP1(JK_I))) / PAP(JL, JK, IBL), 0.5_JPRB)
      ...
      !----------------------------------
      ZFOEELIQT = MIN(FOEELIQ(ZTP1(JK_I)) / PAP(JL, JK, IBL), 0.5_JPRB)
      ZQSLIQ = ZFOEELIQT
      ...
      IF (YRECLDP%NCLDTOP<=JK .AND. JK<=KLEV) THEN
        ...
        IF (ZA(JK_I) > ZEPSEC .and. ZDQS <= -YRECLDP%RLMIN) THEN
          ...
          IF (ZA(JK_I) > 0.99_JPRB) THEN
            ...
            ZCDMAX =  &
            & (ZQX(NCLDQV) - ZQSMIX) / (1.0_JPRB + ZCOR*ZQSMIX*FOEDEM(ZTP1(JK_I)))
          ELSE
...
END MODULE CLOUDSC_GPU_SCC_CUF_K_CACHING_MOD
