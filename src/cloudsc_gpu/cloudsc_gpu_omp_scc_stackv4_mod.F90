...
MODULE CLOUDSC_GPU_OMP_SCC_STACKV3_MOD

  CONTAINS
  
  SUBROUTINE CLOUDSC_SCC_STACKV3 (KIDIA, KFDIA, KLON, KLEV, PTSPHY, PT, PQ, TENDENCY_TMP_T, TENDENCY_TMP_Q, TENDENCY_TMP_A,  &
  ...
  & PFCQSNG, PFSQLTUR, PFSQITUR, PFPLSL, PFPLSN, PFHPSL, PFHPSN, YRECLDP, YDSTACK_L, ZSTACK, JL)
    !---input
    !---prognostic fields
    ...
    USE, intrinsic :: ISO_C_BINDING, ONLY: C_SIZEOF, C_INT64_T, C_PTR
    ...
    REAL(KIND=JPRB), POINTER :: ZFOEALFA(:, :) ! (KLON, KLEV + 1)
    ...
    REAL(KIND=JPRB), POINTER :: ZTP1(:, :) ! (KLON, KLEV)
    ...
    REAL(KIND=JPRB) :: ZDA
    REAL(KIND=JPRB), POINTER :: ZLI(:, :), ZA(:,:) ! (KLON, KLEV), ZA(KLON, KLEV)
    REAL(KIND=JPRB), POINTER :: ZAORIG(:, :) ! (KLON, KLEV)    ! start of scheme value for CC
    ...
    REAL(KIND=JPRB), POINTER :: ZLIQFRAC(:, :) ! (KLON, KLEV)    ! cloud liquid water fraction: ql/(ql+qi)
    REAL(KIND=JPRB), POINTER :: ZICEFRAC(:, :) ! (KLON, KLEV)    ! cloud ice water fraction: qi/(ql+qi)
    REAL(KIND=JPRB), POINTER :: ZQX(:, :, :) ! (KLON, KLEV, NCLV)    ! water variables
    REAL(KIND=JPRB), POINTER :: ZQX0(:, :, :) ! (KLON, KLEV, NCLV)    ! water variables at start of scheme
    ...
    REAL(KIND=JPRB), POINTER :: ZPFPLSX(:, :, :) ! (KLON, KLEV + 1, NCLV)    ! generalized precipitation flux
    REAL(KIND=JPRB), POINTER :: ZLNEG(:, :, :) ! (KLON, KLEV, NCLV)    ! for negative correction diagnostics
    REAL(KIND=JPRB) :: ZMELTMAX
    REAL(KIND=JPRB) :: ZFRZMAX
    REAL(KIND=JPRB) :: ZICETOT
    
    REAL(KIND=JPRB), POINTER :: ZQXN2D(:, :, :) ! (KLON, KLEV, NCLV)    ! water variables store
    
    REAL(KIND=JPRB), POINTER :: ZQSMIX(:, :) ! (KLON, KLEV)    ! diagnostic mixed phase saturation
    !REAL(KIND=JPRB) :: ZQSBIN(KLON,KLEV) ! binary switched ice/liq saturation
    REAL(KIND=JPRB), POINTER :: ZQSLIQ(:, :) ! (KLON, KLEV)    ! liquid water saturation
    REAL(KIND=JPRB), POINTER :: ZQSICE(:, :) ! (KLON, KLEV)    ! ice water saturation
    ...
    REAL(KIND=JPRB), POINTER :: ZFOEEWMT(:, :) ! (KLON, KLEV)
    REAL(KIND=JPRB), POINTER :: ZFOEEW(:, :) ! (KLON, KLEV)
    REAL(KIND=JPRB), POINTER :: ZFOEELIQT(: ,:) ! (KLON, KLEV)
...
!$omp declare target
    ...
    ! REAL(KIND=JPRB), CONTIGUOUS, TARGET, INTENT(INOUT) :: ZSTACK(:)
    TYPE(C_PTR), CONTIGUOUS, TARGET, INTENT(INOUT) :: ZSTACK(:)

    YLSTACK_L = YDSTACK_L
    ZFOEALFA(1:KLON, 1:KLEV+1) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*(KLEV + 1))
    YLSTACK_L = YLSTACK_L + KLON*(KLEV + 1)
    ZTP1(1:KLON, 1:KLEV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV)
    YLSTACK_L = YLSTACK_L + KLON*KLEV
    ZLI(1:KLON, 1:KLEV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV)
    YLSTACK_L = YLSTACK_L + KLON*KLEV
    ZA(1:KLON, 1:KLEV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV)
    YLSTACK_L = YLSTACK_L + KLON*KLEV
    ZAORIG(1:KLON, 1:KLEV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV)
    YLSTACK_L = YLSTACK_L + KLON*KLEV
    ZLIQFRAC(1:KLON, 1:KLEV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV)
    YLSTACK_L = YLSTACK_L + KLON*KLEV
    ZICEFRAC(1:KLON, 1:KLEV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV)
    YLSTACK_L = YLSTACK_L + KLON*KLEV
    ZQX(1:KLON, 1:KLEV, 1:NCLV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV*NCLV)
    YLSTACK_L = YLSTACK_L + KLON*KLEV*NCLV
    ZQX0(1:KLON, 1:KLEV, 1:NCLV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV*NCLV)
    YLSTACK_L = YLSTACK_L + KLON*KLEV*NCLV
    ZPFPLSX(1:KLON, 1:KLEV + 1, 1:NCLV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*(KLEV + 1)*NCLV)
    YLSTACK_L = YLSTACK_L + KLON*(KLEV + 1)*NCLV
    ZLNEG(1:KLON, 1:KLEV, 1:NCLV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV*NCLV) 
    YLSTACK_L = YLSTACK_L + KLON*KLEV*NCLV
    ZQXN2D(1:KLON, 1:KLEV, 1:NCLV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV*NCLV)
    YLSTACK_L = YLSTACK_L + KLON*KLEV*NCLV
    ZQSMIX(1:KLON, 1:KLEV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV) 
    YLSTACK_L = YLSTACK_L + KLON*KLEV
    ZQSLIQ(1:KLON, 1:KLEV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV) 
    YLSTACK_L = YLSTACK_L + KLON*KLEV
    ZQSICE(1:KLON, 1:KLEV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV)
    YLSTACK_L = YLSTACK_L + KLON*KLEV
    ZFOEEWMT(1:KLON, 1:KLEV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV)
    YLSTACK_L = YLSTACK_L + KLON*KLEV
    ZFOEEW(1:KLON, 1:KLEV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV)
    YLSTACK_L = YLSTACK_L + KLON*KLEV
    ZFOEELIQT(1:KLON, 1:KLEV) => ZSTACK(YLSTACK_L:YLSTACK_L+KLON*KLEV)
    YLSTACK_L = YLSTACK_L + KLON*KLEV

    ! REAL(KIND=JPRB), POINTER :: ZFOEALFA(KLON, KLEV + 1)
    ! REAL(KIND=JPRB), POINTER :: ZTP1(KLON, KLEV)
    ! REAL(KIND=JPRB), POINTER :: ZLI(KLON, KLEV), ZA(KLON, KLEV)
    ! REAL(KIND=JPRB), POINTER :: ZAORIG(KLON, KLEV)    ! start of scheme value for CC
    ! REAL(KIND=JPRB), POINTER :: ZLIQFRAC(KLON, KLEV)    ! cloud liquid water fraction: ql/(ql+qi)
    ! REAL(KIND=JPRB), POINTER :: ZICEFRAC(KLON, KLEV)    ! cloud ice water fraction: qi/(ql+qi)
    ! REAL(KIND=JPRB), POINTER :: ZQX(KLON, KLEV, NCLV)    ! water variables
    ...
    ! YLSTACK_L = YDSTACK_L
    ! IP_ZFOEALFA = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*(KLEV + 1)
    ! IP_ZTP1 = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV
    ! IP_ZLI = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV
    ! IP_ZA = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV
    ! IP_ZAORIG = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV
    ! IP_ZLIQFRAC = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV
    ! IP_ZICEFRAC = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV
    ! IP_ZQX = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV*NCLV
    ! IP_ZQX0 = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV*NCLV
    ! IP_ZPFPLSX = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*(KLEV + 1)*NCLV
    ! IP_ZLNEG = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV*NCLV
    ! IP_ZQXN2D = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV*NCLV
    ! IP_ZQSMIX = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV
    ! IP_ZQSLIQ = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV
    ! IP_ZQSICE = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV
    ! IP_ZFOEEWMT = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV
    ! IP_ZFOEEW = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV
    ! IP_ZFOEELIQT = LOC(ZSTACK(YLSTACK_L))
    ! YLSTACK_L = YLSTACK_L + KLON*KLEV
    !===============================================================================
...
!$loki loop seq
      ...
      IF (ZQPRETOT > ZEPSEC) THEN
        ZCOVPTOT = 1.0_JPRB - ((1.0_JPRB - ZCOVPTOT)*(1.0_JPRB - MAX(ZA(JL, JK), ZA(JL, JK - 1))) / (1.0_JPRB - MIN(ZA(JL, JK - 1 &
        & ), 1.0_JPRB - 1.E-06_JPRB)))
...
!$loki loop seq
  ...
  END SUBROUTINE CLOUDSC_SCC_STACKV3

END MODULE CLOUDSC_GPU_OMP_SCC_STACKV3_MOD
