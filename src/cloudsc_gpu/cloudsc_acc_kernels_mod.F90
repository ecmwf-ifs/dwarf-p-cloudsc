MODULE CLOUDSC_ACC_KERNELS_MOD

CONTAINS

SUBROUTINE CLOUDSC_ACC_KERNELS &
 !---input
...
REAL(KIND=JPRB)   ,INTENT(INOUT) :: TENDENCY_CML_T(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: TENDENCY_CML_A(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: TENDENCY_CML_Q(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: TENDENCY_CML_CLD(KLON,KLEV,NCLV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: TENDENCY_TMP_T(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: TENDENCY_TMP_A(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: TENDENCY_TMP_Q(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: TENDENCY_TMP_CLD(KLON,KLEV,NCLV)
...
!          ZEVAP=0.0_JPRB
  ...
  ! ------------------------------
  DO JL=KIDIA,KFDIA
    ZLDIFDT(JL)=YRECLDP%RCLDIFF*PTSPHY !original version
    !Increase by factor of 5 for convective points
    IF(KTYPE(JL) > 0 .AND. PLUDE(JL,JK) > ZEPSEC)&
       & ZLDIFDT(JL)=YRECLDP%RCLDIFF_CONVI*ZLDIFDT(JL)  
  ENDDO
...
!    ZLEVAP = MAX(ZA(JL,JK)+ZACUST(JL),1.0_JPRB)*MIN(ZDQS(JL),ZLICLD(JL)+ZLFINALSUM(JL))
  ...
  DO JL=KIDIA,KFDIA
    IF(ZA(JL,JK) > ZEPSEC.AND.ZDQS(JL) <= -YRECLDP%RLMIN) THEN
...
! end old limiter
      ...
      IF(ZLCOND1(JL) < YRECLDP%RLMIN) ZLCOND1(JL)=0.0_JPRB
  ...
  DO JL=KIDIA,KFDIA
    ...
    IF(ZDQS(JL) <= -YRECLDP%RLMIN .AND. ZA(JL,JK)<1.0_JPRB-ZEPSEC) THEN
      ...
      ELSEIF (YRECLDP%NSSOPT==1) THEN 
        ! Tompkins 
        ZQE=(ZQX(JL,JK,NCLDQV)-ZA(JL,JK)*ZQSICE(JL,JK))/&
            & MAX(ZEPSEC,1.0_JPRB-ZA(JL,JK))  
        ZQE=MAX(0.0_JPRB,ZQE)
      ELSEIF (YRECLDP%NSSOPT==2) THEN 
        ! Lohmann and Karcher
        ZQE=ZQX(JL,JK,NCLDQV)  
      ELSEIF (YRECLDP%NSSOPT==3) THEN 
        ! Gierens
     ...
     &               ZFAC*ZQSICE(JL,JK)+ZQX(JL,JK,NCLDQV)
        ...
        IF(ZLCOND2(JL) < YRECLDP%RLMIN .OR. (1.0_JPRB-ZA(JL,JK))<ZEPSEC ) THEN
          ZLCOND2(JL) = 0.0_JPRB
          ZACOND      = 0.0_JPRB
        ENDIF
  ...
  DO JL=KIDIA,KFDIA
    ...
    !--------------------------------------------------------------
    IF (ZTP1(JL,JK)<RTT .AND. ZQXFG(JL,NCLDQL)>YRECLDP%RLMIN) THEN  ! T<273K
      ...
      !-----------------------------------------------------
      ZICE0=MAX(ZICECLD(JL), ZICENUCLEI(JL)*YRECLDP%RICEINIT/ZRHO(JL))
...
!      ZDEPOS = ZDEPOS*MIN(RDEPLIQREFRATE+ZCLDTOPDIST(JL)/RDEPLIQREFDEPTH,1.0_JPRB)
      ...
      ZDEPOS = ZDEPOS*MIN(ZINFACTOR + (1.0_JPRB-ZINFACTOR)* &
                  & (YRECLDP%RDEPLIQREFRATE+ZCLDTOPDIST(JL)/YRECLDP%RDEPLIQREFDEPTH),1.0_JPRB)
  ...
  ELSEIF (IDEPICE == 2) THEN
    ...
    DO JL=KIDIA,KFDIA
      ...
      IF (ZTP1(JL,JK)<RTT .AND. ZQXFG(JL,NCLDQL)>YRECLDP%RLMIN) THEN  ! T<273K
        ...
        ZAPLUSB   = YRECLDP%RCL_APB1*ZVPICE-YRECLDP%RCL_APB2*ZVPICE*ZTP1(JL,JK)+ &
       &             PAP(JL,JK)*YRECLDP%RCL_APB3*ZTP1(JL,JK)**3._JPRB
        ZCORRFAC  = (1.0_JPRB/ZRHO(JL))**0.5_JPRB
        ZCORRFAC2 = ((ZTP1(JL,JK)/273.0_JPRB)**1.5_JPRB) &
       &             *(393.0_JPRB/(ZTP1(JL,JK)+120.0_JPRB))

        ZPR02  = ZRHO(JL)*ZICE0*YRECLDP%RCL_CONST1I/(ZTCG*ZFACX1I)

        ZTERM1 = (ZVPLIQ-ZVPICE)*ZTP1(JL,JK)**2.0_JPRB*ZVPICE*ZCORRFAC2*ZTCG* &
       &          YRECLDP%RCL_CONST2I*ZFACX1I/(ZRHO(JL)*ZAPLUSB*ZVPICE)
        ZTERM2 = 0.65_JPRB*YRECLDP%RCL_CONST6I*ZPR02**YRECLDP%RCL_CONST4I+YRECLDP%RCL_CONST3I &
       &          *ZCORRFAC**0.5_JPRB*ZRHO(JL)**0.5_JPRB &
       &          *ZPR02**YRECLDP%RCL_CONST5I/ZCORRFAC2**0.5_JPRB
  ...
  DO JL=KIDIA,KFDIA
    IF (ZQPRETOT(JL)>ZEPSEC) THEN
      ...
      ZCOVPTOT(JL) = MAX(ZCOVPTOT(JL),YRECLDP%RCOVPMIN)
...
!       &ZCOVPTOT(JL)/(MAX(ZA(JL,JK),ZEPSEC))
    ...
    ELSEIF (IWARMRAIN == 2) THEN
     ...
     &                  YRECLDP%RCL_KKAau * ZLIQCLD(JL)**YRECLDP%RCL_KKBauq * ZCONST**YRECLDP%RCL_KKBaun
     ...
     &                 YRECLDP%RCL_KKAac * (ZLIQCLD(JL)*ZRAINCLD(JL))**YRECLDP%RCL_KKBac
  ...
  DO JL=KIDIA,KFDIA
    IF(ZTP1(JL,JK) <= RTT .AND. ZLIQCLD(JL)>ZEPSEC) THEN
      ...
      IF (ZSNOWCLD(JL)>ZEPSEC .AND. ZCOVPTOT(JL)>0.01_JPRB) THEN
        ...
        ZSNOWRIME(JL) = 0.3_JPRB*ZCOVPTOT(JL)*PTSPHY*YRECLDP%RCL_CONST7S*ZFALLCORR &
     &                  *(ZRHO(JL)*ZSNOWCLD(JL)*YRECLDP%RCL_CONST1S)**YRECLDP%RCL_CONST8S
...
!      ENDIF
    ...
    IF (ZQX(JL,JK,NCLDQR) > ZEPSEC) THEN
      ...
      IF (ZTP1(JL,JK) < RTT) THEN
        ...
        IF (LLRAINLIQ(JL)) THEN 
          ...
          ZLAMBDA = (YRECLDP%RCL_FAC1/(ZRHO(JL)*ZQX(JL,JK,NCLDQR)))**YRECLDP%RCL_FAC2

          ! Calculate freezing rate based on Bigg(1953) and Wisner(1972)
          ZTEMP = YRECLDP%RCL_FZRAB * (ZTP1(JL,JK)-RTT)
          ZFRZ  = PTSPHY * (YRECLDP%RCL_CONST5R/ZRHO(JL)) * (EXP(ZTEMP)-1._JPRB) &
                  & * ZLAMBDA**YRECLDP%RCL_CONST6R
  ...
  DO JL=KIDIA,KFDIA 
    ! not implicit yet... 
    ZFRZMAX(JL)=MAX((YRECLDP%RTHOMO-ZTP1(JL,JK))*ZRLDCP,0.0_JPRB)
  ENDDO
 ...
 IF (IEVAPRAIN == 1) THEN
  ...
  DO JL=KIDIA,KFDIA
    ...
    ZZRH=YRECLDP%RPRECRHMAX+(1.0_JPRB-YRECLDP%RPRECRHMAX)*ZCOVPMAX(JL)/MAX(ZEPSEC,1.0_JPRB-ZA(JL,JK))
    ...
    IF(LLO1) THEN
      ...
      ZBETA1 = SQRT(PAP(JL,JK)/&
       & PAPH(JL,KLEV+1))/YRECLDP%RVRFACTOR*ZPRECLR/&
      ...
      !-------------------------------------------------------------
      ZCOVPTOT(JL) = MAX(YRECLDP%RCOVPMIN,ZCOVPTOT(JL)-MAX(0.0_JPRB, &
 ...
 ELSEIF (IEVAPRAIN == 2) THEN
  ...
  DO JL=KIDIA,KFDIA
    ...
    IF(LLO1) THEN
      ...
      ZLAMBDA = (YRECLDP%RCL_FAC1/(ZRHO(JL)*ZPRECLR))**YRECLDP%RCL_FAC2 ! ZPRECLR=kg/kg
     ...
     & YRECLDP%RCL_CONST1R*(ZCORR2/ZEVAP_DENOM)*(0.78_JPRB/(ZLAMBDA**YRECLDP%RCL_CONST4R)+ &
     & YRECLDP%RCL_CONST2R*(ZRHO(JL)*ZFALLCORR)**0.5_JPRB/ &
  ...
  ELSEIF (IEVAPSNOW == 2) THEN
   ...
   DO JL=KIDIA,KFDIA
    ...
    IF(LLO1) THEN
      ...
      ZAPLUSB   = YRECLDP%RCL_APB1*ZVPICE-YRECLDP%RCL_APB2*ZVPICE*ZTP1(JL,JK)+ &
     &             PAP(JL,JK)*YRECLDP%RCL_APB3*ZTP1(JL,JK)**3
      ZCORRFAC  = (1.0/ZRHO(JL))**0.5
      ZCORRFAC2 = ((ZTP1(JL,JK)/273.0)**1.5)*(393.0/(ZTP1(JL,JK)+120.0))

      ZPR02 = ZRHO(JL)*ZPRECLR*YRECLDP%RCL_CONST1S/(ZTCG*ZFACX1S)

      ZTERM1 = (ZQSICE(JL,JK)-ZQE)*ZTP1(JL,JK)**2*ZVPICE*ZCORRFAC2*ZTCG* &
     &          YRECLDP%RCL_CONST2S*ZFACX1S/(ZRHO(JL)*ZAPLUSB*ZQSICE(JL,JK))
      ZTERM2 = 0.65*YRECLDP%RCL_CONST6S*ZPR02**YRECLDP%RCL_CONST4S+YRECLDP%RCL_CONST3S*ZCORRFAC**0.5 &
     &          *ZRHO(JL)**0.5*ZPR02**YRECLDP%RCL_CONST5S/ZCORRFAC2**0.5
...
END SUBROUTINE CLOUDSC_ACC_KERNELS

END MODULE CLOUDSC_ACC_KERNELS_MOD
