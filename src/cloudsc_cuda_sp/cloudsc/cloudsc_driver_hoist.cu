...
void cloudsc_driver(int numthreads, int numcols, int nproma) {
  ...
  cudaMalloc(&d_zfoealfa, sizeof(float) * nblocks*(nlev+1)*nproma);
  cudaMalloc(&d_ztp1, sizeof(float) * nblocks*(nlev+1)*nproma);
  cudaMalloc(&d_zli, sizeof(float) * nblocks*(nlev+1)*nproma);
  cudaMalloc(&d_za, sizeof(float) * nblocks*(nlev+1)*nproma);
  cudaMalloc(&d_zaorig, sizeof(float) * nblocks*(nlev+1)*nproma);
  cudaMalloc(&d_zliqfrac, sizeof(float) * nblocks*(nlev+1)*nproma);
  cudaMalloc(&d_zicefrac, sizeof(float) * nblocks*(nlev+1)*nproma);
  cudaMalloc(&d_zqx, sizeof(float) * nblocks*(nlev+1)*nproma*nclv);
  cudaMalloc(&d_zqx0, sizeof(float) * nblocks*(nlev+1)*nproma*nclv);
  cudaMalloc(&d_zpfplsx, sizeof(float) * nblocks*(nlev+1)*nproma*nclv);
  cudaMalloc(&d_zlneg, sizeof(float) * nblocks*(nlev+1)*nproma*nclv);
  cudaMalloc(&d_zqxn2d, sizeof(float) * nblocks*(nlev+1)*nproma*nclv);
  cudaMalloc(&d_zqsmix, sizeof(float) * nblocks*(nlev+1)*nproma);
  cudaMalloc(&d_zqsliq, sizeof(float) * nblocks*(nlev+1)*nproma);
  cudaMalloc(&d_zqsice, sizeof(float) * nblocks*(nlev+1)*nproma);
  cudaMalloc(&d_zfoeewmt, sizeof(float) * nblocks*(nlev+1)*nproma);
  cudaMalloc(&d_zfoeew, sizeof(float) * nblocks*(nlev+1)*nproma);
  cudaMalloc(&d_zfoeeliqt, sizeof(float) * nblocks*(nlev+1)*nproma);
  //
