...
__global__ void __launch_bounds__(128, 1) cloudsc_c(int kidia, int kfdia, int klon, float ptsphy,
  ...
  for (jk = 0; jk <= klev + -1; jk += 1) {
    ...
    //----------------------------------------
    zfoealfa[jk] = ((float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2))));
    zfoeewmt[jk] =
      fminf(((float)(r2es*((float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2)))*expf((r3les*(ztp1[jk] - rtt))/(ztp1[jk] - r4les)) + (1.0 - (float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2))))*expf((r3ies*(ztp1[jk] - rtt))/(ztp1[jk] - r4ies))))) / pap[jl + klon*(jk + klev*(ibl))], (float) 0.5);
    ...
    //---------------------------------------------
    zalfa = ((float)(fmaxf(0.0, copysign(1.0, ztp1[jk] - rtt))));
    zfoeew[jk] = fminf((zalfa*((float)(r2es*expf((r3les*(ztp1[jk] - rtt))/(ztp1[jk] - r4les)))) + ((float) 1.0 - zalfa)*((float)(r2es*expf((r3ies*(ztp1[jk] - rtt))/(ztp1[jk] - r4ies))))) / pap[jl +
      klon*(jk + klev*(ibl))], (float) 0.5);
    zfoeew[jk] = fminf((float) 0.5, zfoeew[jk]);
    ...
    //----------------------------------
    zfoeeliqt[jk] =
      fminf(((float)(r2es*expf((r3les*(ztp1[jk] - rtt))/(ztp1[jk] - r4les)))) / pap[jl + klon*(jk + klev*(ibl))], (float) 0.5);
    ...
    //   !----------------------------------
    //   ZFOEEICET(JL,JK)=MIN(((float)(r2es*expf((r3ies*(ztp1[jk] - rtt))/(ztp1[jk] - r4ies))))(ZTP1(JL,JK))/PAP(JL,JK),0.5_JPRB)
  ...
  for (jk = 0; jk <= klev + -1; jk += 1) {
    ...
    //------------------------------------------
    za[jk] = fmaxf((float) 0.0, fminf((float) 1.0, za[jk]));
  ...
  for (jk = -1 + (*yrecldp).ncldtop; jk <= klev + -1; jk += 1) {
    ...
    zcorqsmix = (float) 1.0 + ((float)((float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2)))*ralvdcp + (1.0 - (float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2))))*ralsdcp))*zdqsmixdt;

    // evaporation/sublimation limits
    zevaplimmix =
      fmaxf((zqsmix[jk] - zqx[jk + klev*(4)]) / zcorqsmix, (float) 0.0);
    zevaplimliq =
      fmaxf((zqsliq[jk] - zqx[jk + klev*(4)]) / zcorqsliq, (float) 0.0);
    zevaplimice =
      fmaxf((zqsice[jk] - zqx[jk + klev*(4)]) / zcorqsice, (float) 0.0);

    //--------------------------------
    // in-cloud consensate amount
    //--------------------------------
    ztmpa = (float) 1.0 / fmaxf(za[jk], zepsec);
    ...
    zfokoop = ((float)(fminf(rkoop1 - rkoop2*ztp1[jk], (float)(r2es*expf((r3les*(ztp1[jk] - rtt))/(ztp1[jk] - r4les)))*1.0/(float)(r2es*expf((r3ies*(ztp1[jk] - rtt))/(ztp1[jk] - r4ies))))));
    ...
    if (za[jk] > (float) 1.0 - (*yrecldp).ramin) {
      zsupsat =
        fmaxf((zqx[jk + klev*(4)] - zfac*zqsice[jk]) / zcorqsice, (float) 0.0);
    } else {
      // Calculate environmental humidity supersaturation
      zqp1env = (zqx[jk + klev*(4)] - za[jk]*zqsice[jk]) / fmaxf((float) 1.0 -
        za[jk], zepsilon);
      //& SIGN(MAX(ABS(1.0_JPRB-ZA(JL,JK)),ZEPSILON),1.0_JPRB-ZA(JL,JK))
      zsupsat = fmaxf(((float) 1.0 - za[jk])*(zqp1env - zfac*zqsice[jk]) / zcorqsice,
        (float) 0.0);
    }
    ...
    if (jk + 1 > (*yrecldp).ncldtop) {
      ...
      zmf = fmaxf((float) 0.0, (pmfu[jl + klon*(jk + klev*(ibl))] + pmfd[-1 +
    ...
    if (zli[jk] > zepsec) {
      ...
      ze = zldifdt*fmaxf(zqsmix[jk] - zqx[jk + klev*(4)], (float) 0.0);
      zleros = za[jk]*ze;
      zleros = fminf(zleros, zevaplimmix);
      zleros = fminf(zleros, zli[jk]);
    ...
    ztp1[jk] = fmaxf(ztp1[jk], (float) 160.0);
    llflag = true;

    // Formerly a call to CUADJTQ(..., ICALL=5)
    zqp = (float) 1.0 / pap[jl + klon*(jk + klev*(ibl))];
    zqsat = ((float)(r2es*((float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2)))*expf((r3les*(ztp1[jk] - rtt))/(ztp1[jk] - r4les)) + (1.0 - (float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2))))*expf((r3ies*(ztp1[jk] - rtt))/(ztp1[jk] - r4ies)))))*zqp;
    zqsat = fminf((float) 0.5, zqsat);
    zcor = (float) 1.0 / ((float) 1.0 - retv*zqsat);
    zqsat = zqsat*zcor;
    zcond = (zqsmix[jk] - zqsat) / ((float) 1.0 + zqsat*zcor*((float)(((float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2)))*r5alvcp)*(1.0/powf(ztp1[jk] - r4les, 2)) + ((1.0 - (float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2))))*r5alscp)*(1.0/powf(ztp1[jk] - r4ies, 2)))));
    ztp1[jk] = ztp1[jk] + ((float)((float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2)))*ralvdcp + (1.0 - (float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2))))*ralsdcp))*zcond;
    zqsmix[jk] = zqsmix[jk] - zcond;
    zqsat = ((float)(r2es*((float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2)))*expf((r3les*(ztp1[jk] - rtt))/(ztp1[jk] - r4les)) + (1.0 - (float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2))))*expf((r3ies*(ztp1[jk] - rtt))/(ztp1[jk] - r4ies)))))*zqp;
    zqsat = fminf((float) 0.5, zqsat);
    zcor = (float) 1.0 / ((float) 1.0 - retv*zqsat);
    zqsat = zqsat*zcor;
    zcond1 = (zqsmix[jk] - zqsat) / ((float) 1.0 + zqsat*zcor*((float)(((float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2)))*r5alvcp)*(1.0/powf(ztp1[jk] - r4les, 2)) + ((1.0 - (float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2))))*r5alscp)*(1.0/powf(ztp1[jk] - r4ies, 2)))));
    ztp1[jk] = ztp1[jk] + ((float)((float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2)))*ralvdcp + (1.0 - (float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2))))*ralsdcp))*zcond1;
    ...
    if (zdqs > (float) 0.0) {
      ...
      zlevap = za[jk]*fminf(zdqs, zlicld);
      zlevap = fminf(zlevap, zevaplimmix);
      zlevap = fminf(zlevap, fmaxf(zqsmix[jk] - zqx[jk + klev*(4)], (float) 0.0));
    ...
    if (za[jk] > zepsec && zdqs <= -(*yrecldp).rlmin) {
      ...
      if (za[jk] > (float) 0.99) {
        ...
        zcdmax = (zqx[jk + klev*(4)] - zqsmix[jk]) / ((float) 1.0 +
          zcor*zqsmix[jk]*((float)(((float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2)))*r5alvcp)*(1.0/powf(ztp1[jk] - r4les, 2)) + ((1.0 - (float)(fminf(1.0, powf((fmaxf(rtice, fminf(rtwat, ztp1[jk])) - rtice)*rtwat_rtice_r, 2))))*r5alscp)*(1.0/powf(ztp1[jk] - r4ies, 2)))));
      } else {
    ...
    if (zdqs <= -(*yrecldp).rlmin && za[jk] < (float) 1.0 - zepsec) {
      ...
      if ((*yrecldp).nssopt == 0) {
        // No scheme
        zqe = (zqx[jk + klev*(4)] - za[jk]*zqsice[jk]) / fmaxf(zepsec, (float) 1.0
           - za[jk]);
      ...
      if (zqe >= zrhc*zqsice[jk]*zfac && zqe < zqsice[jk]*zfac) {
        ...
        zacond = -((float) 1.0 - za[jk])*zfac*zdqs / fmaxf((float)
          2.0*(zfac*zqsice[jk] - zqe), zepsec);

        zacond = fminf(zacond, (float) 1.0 - za[jk]);            //PUT THE LIMITER BACK
        ...
        zzdl =
          (float) 2.0*(zfac*zqsice[jk] - zqe) / fmaxf(zepsec, (float) 1.0 - za[jk]);
        // Added correction term ZFAC 15/03/2010
        if (zfac*zdqs < -zzdl) {
    ...
    if (idepice == 1) {
      ...
      if (ztp1[jk] < rtt && zqxfg[0] > (*yrecldp).rlmin) {
        ...
        zvpice = ((float)(r2es*expf((r3ies*(ztp1[jk] - rtt))/(ztp1[jk] - r4ies))))*rv / rd;
        ...
        //---------------------------
        zdepos = fmaxf(za[jk]*(zinew - zice0), (float) 0.0);
      ...
      if (ztp1[jk] < rtt && zqxfg[0] > (*yrecldp).rlmin) {
        ...
        zaplusb = (*yrecldp).rcl_apb1*zvpice - (*yrecldp).rcl_apb2*zvpice*ztp1[jk] +
          pap[jl + klon*(jk + klev*(ibl))]*(*yrecldp).rcl_apb3*(powf(ztp1[jk],
           (float) 3.));
        zcorrfac = powf(((float) 1.0 / zrho), (float) 0.5);
        zcorrfac2 = (powf((ztp1[jk] / (float) 273.0), (float) 1.5))*((float) 393.0 /
          (ztp1[jk] + (float) 120.0));

        zpr02 = zrho*zice0*(*yrecldp).rcl_const1i / (ztcg*zfacx1i);

        zterm1 = (zvpliq - zvpice)*(powf(ztp1[jk], (float) 2.0))
        ...
        zdepos = fmaxf(za[jk]*zterm1*zterm2*ptsphy, (float) 0.0);
    ...
    //---------------------------------------------------------------
    if (zqpretot > zepsec) {
      zcovptot = (float) 1.0 - (((float) 1.0 - zcovptot)*((float) 1.0 - fmaxf(za[jk],
         za[-1 + jk])) / ((float) 1.0 - fminf(za[-1 + jk], (float) 1.0 - (float)
        1.E-06)));
      zcovptot = fmaxf(zcovptot, (*yrecldp).rcovpmin);
      zcovpclr = fmaxf((float) 0.0, zcovptot - za[jk]);          // clear sky proportion
    ...
    if (ztp1[jk] <= rtt) {
      ...
      if (zicecld > zepsec) {
        ...
        zzco = ptsphy*(*yrecldp).rsnowlin1*expf((*yrecldp).rsnowlin2*(ztp1[jk] - rtt));

        if ((*yrecldp).laericeauto) {
          zlcrit = picrit_aer[jl + klon*(jk + klev*(ibl))];
          // 0.3 = N**0.333 with N=0.027
          zzco = zzco*(powf(((*yrecldp).rnice / pnice[jl + klon*(jk + klev*(
            ibl))]), (float) 0.333));
        } else {
    ...
    if (zliqcld > zepsec) {
      ...
      if (iwarmrain == 1) {
        ...
        if ((*yrecldp).laerliqcoll) {
          ...
          zcfpr = zcfpr*(powf(((*yrecldp).rccn / pccn[jl + klon*(jk + klev*(
            ibl))]), (float) 0.333));
        }
        ...
        if (zliqcld > zlcrit) {
          ...
          zrainaut = (float) 1.5*za[jk]*ptsphy*(*yrecldp).rcl_kkaau*(powf(zliqcld,
            (*yrecldp).rcl_kkbauq))*(powf(zconst, (*yrecldp).rcl_kkbaun));
    ...
    if (zicetot > zepsec && ztp1[jk] > rtt) {
      ...
      zsubsat = fmaxf(zqsice[jk] - zqx[jk + klev*(4)], (float) 0.0);
    ...
    if (zqx[jk + klev*(2)] > zepsec) {
      ...
      if (ztp1[jk] <= rtt && ztp1[-1 + jk] > rtt) {
        ...
        zqpretot = fmaxf(zqx[jk + klev*(3)] + zqx[jk + klev*(2)], zepsec);
      ...
      if (ztp1[jk] < rtt) {
        ...
        if (prainfrac_toprfz[jl + klon*(ibl)] > 0.8) {
          ...
          zlambda = powf(((*yrecldp).rcl_fac1 / (zrho*zqx[jk + klev*(2)])),
            (*yrecldp).rcl_fac2);
          ...
          zcons1 = fabsf(ptsphy*((float) 1.0 + (float) 0.5*(rtt - ztp1[jk])) /
            (*yrecldp).rtaumel);
          zfrzmax = fmaxf((rtt - ztp1[jk])*zcons1*zrldcp, (float) 0.0);

        }

        if (zfrzmax > zepsec) {
          zfrz = fminf(zqx[jk + klev*(2)], zfrzmax);
    ...
    //----------------------------------------------------------------------
    // not implicit yet...
    zfrzmax = fmaxf(((*yrecldp).rthomo - ztp1[jk])*zrldcp, (float) 0.0);
    ...
    if (ievaprain == 1) {
      ...
      zzrh = (*yrecldp).rprecrhmax + ((float) 1.0 - (*yrecldp).rprecrhmax)*zcovpmax /
        fmaxf(zepsec, (float) 1.0 - za[jk]);
      zzrh = fminf(fmaxf(zzrh, (*yrecldp).rprecrhmax), (float) 1.0);

      zqe = (zqx[jk + klev*(4)] - za[jk]*zqsliq[jk]) / fmaxf(zepsec, (float) 1.0 -
         za[jk]);
      //---------------------------------------------
      // humidity in moistest ZCOVPCLR part of domain
      //---------------------------------------------
      zqe = fmaxf((float) 0.0, fminf(zqe, zqsliq[jk]));
      llo1 = zcovpclr > zepsec && zqxfg[2] > zepsec && zqe < zzrh*zqsliq[jk];

      if (llo1) {
        // note: zpreclr is a rain flux
        zpreclr = zqxfg[2]*zcovpclr / copysign(fmaxf(fabsf(zcovptot*zdtgdp),
      ...
      zqe = fmaxf((float) 0.0, fminf(zqx[jk + klev*(4)], zqsliq[jk]));
      ...
      if (llo1) {
        ...
        zesatliq = rv / rd*((float)(r2es*expf((r3les*(ztp1[jk] - rtt))/(ztp1[jk] - r4les))));
        ...
        zevap_denom = (*yrecldp).rcl_cdenom1*zesatliq - (*yrecldp)
          .rcl_cdenom2*ztp1[jk]*zesatliq + (*yrecldp).rcl_cdenom3*(powf(ztp1[jk],
          (float) 3.))*pap[jl + klon*(jk + klev*(ibl))];

        // Temperature dependent conductivity
        zcorr2 = (powf((ztp1[jk] / (float) 273.), (float) 1.5))*(float) 393. /
          (ztp1[jk] + (float) 120.);
        zka = (*yrecldp).rcl_ka273*zcorr2;

        zsubsat = fmaxf(zzrh*zqsliq[jk] - zqe, (float) 0.0);

        zbeta = ((float) 0.5 / zqsliq[jk])*(powf(ztp1[jk], (float) 2.))
          *zesatliq*(*yrecldp).rcl_const1r*(zcorr2 / zevap_denom)*((float) 0.78 /
          (powf(zlambda, (*yrecldp).rcl_const4r)) + (*yrecldp)
          .rcl_const2r*(powf((zrho*zfallcorr), (float) 0.5)) / ((powf(zcorr2, (float)
          0.5))*(powf(zlambda, (*yrecldp).rcl_const3r))));
    ...
    if (ievapsnow == 1) {
      ...
      // humidity in moistest ZCOVPCLR part of domain
      //---------------------------------------------
      zqe = fmaxf((float) 0.0, fminf(zqe, zqsice[jk]));
      llo1 = zcovpclr > zepsec && zqxfg[3] > zepsec && zqe < zzrh*zqsice[jk];

      if (llo1) {
        // note: zpreclr is a rain flux a
        zpreclr = zqxfg[3]*zcovpclr / copysign(fmaxf(fabsf(zcovptot*zdtgdp),
          zepsilon), zcovptot*zdtgdp);
      ...
      if (llo1) {
        ...
        zcorrfac2 = (powf((ztp1[jk] / 273.0), 1.5))*(393.0 / (ztp1[jk] + 120.0));

        zpr02 = zrho*zpreclr*(*yrecldp).rcl_const1s / (ztcg*zfacx1s);

        zterm1 = (zqsice[jk] - zqe)*(powf(ztp1[jk], 2))*zvpice*zcorrfac2*ztcg*(*yrecldp)
        ...
        zevap = fminf(zevap, zqx[jk + klev*(3)]);
    ...
    //---------------------------------------
    for (jm = 0; jm <= 5 + -1; jm += 1) {
      zmax = fmaxf(zqx[jk + klev*jm], zepsec);
    ...
    for (jm = 0; jm <= 5 + -1; jm += 1) {
      ...
      //---------------------------
      zmm = fmaxf(zqx[jk + klev*jm], zepsec);
